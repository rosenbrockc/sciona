...
def _cmd_index_build(args: argparse.Namespace) -> None:
    ...
    from sciona.indexer.builder import IndexBuilder
    ...
    if prover == Prover.LEAN4:
        ...
        from sciona.indexer.coq_source import CoqDeclarationSource
...
def _cmd_skill_index(args: argparse.Namespace) -> None:
    ...
    if catalog.size == 0:
        print(
            "Error: no primitives found. Run 'sciona skill ingest' first.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _cmd_skill_search(args: argparse.Namespace) -> None:
    ...
    if not index_dir.exists():
        print(
            f"Error: skill index not found at {index_dir}. Run 'sciona skill index' first.",
            file=sys.stderr,
        )
        sys.exit(1)
