...
def _benchmark_validation_metadata(summary: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "flow_avg_planner_tool_dispatches": dict(
            summary.get("flow_avg_planner_tool_dispatches", {}) or {}
        ),
        "flow_avg_planner_tool_latency_ms": dict(
            summary.get("flow_avg_planner_tool_latency_ms", {}) or {}
        ),
        "flow_avg_planner_escalations": dict(
            summary.get("flow_avg_planner_escalations", {}) or {}
        ),
...
async def _cmd_release_validate(args: argparse.Namespace) -> None:
    ...
    from sciona.config import AgeomConfig
    from sciona.telemetry import configure_dashboard_output, finish_run, merge_run_metadata, start_run
        ...
        merge_run_metadata(
            {
                "release_validation": {
                    ...
                    "top_benchmark_warning_subcheck": str(
                        summary.get("top_benchmark_warning_subcheck", "") or ""
                    ),
                    "failure_summary": str(summary.get("failure_summary", "") or ""),
