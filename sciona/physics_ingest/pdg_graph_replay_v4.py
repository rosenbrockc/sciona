"""Versioned graph replay with two-premise substitution and unity rules."""
import json
...
def replay_derivation(nodes, expressions, rules, *, edges=None):
    ...
    while pending:
        ...
        for identity in ready:
            ...
            try:
                actual,local=replay_rational_step(rule,[computed[i] for i in signature['inputs']],feeds,expressions[output])
            except ValueError as error:
                raise ValueError(f'{identity} ({rule.name}): {error}') from error
            ...
            elif rule.name=='substitute LHS of expr 1 into expr 2':
                premise=computed[signature['inputs'][0]]
                mapping={premise.rhs:premise.lhs}
            elif rule.name=='substitute RHS of expr 1 into expr 2':
                premise=computed[signature['inputs'][0]]
                mapping={premise.lhs:premise.rhs}
            else: mapping={}
    ...
    return {
        'runner_version':'pdg-graph-replay.v4',
