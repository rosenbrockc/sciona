"""Five-fold native Sofia and full-training query refits.

Raw features enter each fit, which standardizes using only its references.
"""
import numpy as np
from sciona.otto_sofia import fit_predict


def crossfit_sofia(training, labels, folds, training_ids, query, query_ids, *, seed, regularization, iterations, ddof, r_library):
    ...
    for fold in range(5):
        held=f==fold
        oof[held]=fit_predict(x[~held],y[~held],x[held],seed=seed,regularization=regularization,iterations=iterations,ddof=ddof,r_library=r_library)
        coverage[held]+=1
    if not np.all(coverage==1):raise ValueError('Incomplete out-of-fold coverage')
    predicted=fit_predict(x,y,q,seed=seed,regularization=regularization,iterations=iterations,ddof=ddof,r_library=r_library)
