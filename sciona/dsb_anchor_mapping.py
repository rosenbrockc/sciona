"""Selected source label mapping, with explicit local RNG and Python 3 grid sizes.

Copyright (c) 2023 lfz; MIT notice: docs/licenses/DSB2017-MIT.txt.
Source commit 0ac3eb9f383bf0127c587e0502c59ac84d9ba6a2.
Used through the validated assign_anchor_labels wrapper. Source detector anchors
are even integers; wrapper uses float geometry for physical lengths.
"""
import random
import numpy as np
...
class LabelMapping(object):
    ...
    def __init__(self, config, phase):
        ...
        self.rng = config.get('rng') or random.Random(config.get('random_state'))
    ...
    def __call__(self, input_size, target, bboxes):
        ...
        if self.phase == 'train' and self.num_neg > 0:
            ...
            neg_idcs = self.rng.sample(range(len(neg_z)), min(num_neg, len(neg_z)))
        ...
        if len(iz) == 0:
            ...
            idx = self.rng.sample(range(len(iz)), 1)[0]
...
def select_samples(bbox, anchor, th, oz, oh, ow):
    ...
    if min_overlap > max_overlap:
        ...
        centers = np.concatenate([oz[iz].reshape((-1, 1)), oh[ih].reshape((-1, 1)), ow[iw].reshape((-1, 1))], axis=1)
        r0 = anchor / 2
        s0 = centers - r0
        e0 = centers + r0
        r1 = d / 2
