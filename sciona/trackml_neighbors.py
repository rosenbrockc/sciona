"""TrackML layer neighbor search; BSD-2-Clause source adaptation.

Derived from edwinst/trackml_solution; see docs/licenses/TrackML-BSD-2-Clause.txt.
Legacy DataFrame.as_matrix(columns=...) uses equivalent column selection/to_numpy.
Source fitting assumes each detector layer has hits and is performed once.
"""
from collections import OrderedDict
import numpy as np
import pandas as pd
from sklearn.neighbors import NearestNeighbors
from sciona.trackml_layer_functions import createLayerFunctionInterpolators
...
class CapNeighbors:
    ...
    class Cap:
        ...
        def __init__(self, cap_id, cap_group, cap_z, layer_functions=None):
            self.cap_z = cap_z
            self.nb_coords = self.transformToNeighborCoords(cap_group.loc[:, ['x', 'y', 'z']].to_numpy())
...
class Neighbors:
    default_params = OrderedDict([('nb__cyl_first_dz_0', 1000.0), ('nb__cyl_first_dz_1', 200.0), ('nb__cyl_first_dz_4', 400.0), ('nb__cap_first_dr2_0', 100.0), ('nb__cap_first_dr2_1', 200.0), ('nb__cap_first_inc', 5.0), ('nb__min_radius', 1.0), ('nb__nbins_radius', 100), ('nb__cyl_scale', 5.0)])
    ...
    def findIntersectionNeighborhood(self, xi, yi, zi, cyl_closer, next_id, radius):
        ...
        int_finite = np.isfinite(xi) & np.isfinite(yi) & np.isfinite(zi)
        cyl_finite = int_finite & cyl_closer
        nb_dfs += self._findNeighborhoodsGroupedByIdAndRadius(xi, yi, zi, cyl_finite, next_id, radius, self._findCylinderNeighborhood)
        cap_finite = int_finite & ~cyl_closer
        nb_dfs += self._findNeighborhoodsGroupedByIdAndRadius(xi, yi, zi, cap_finite, next_id, radius, self._findCapNeighborhood)
    ...
    def findIntersectionNeighborhoodK(self, xi, yi, zi, cyl_closer, next_id, k):
        ...
        int_finite = np.isfinite(xi) & np.isfinite(yi) & np.isfinite(zi)
        cyl_finite = int_finite & cyl_closer
        nb_dfs += self._findNeighborhoodsGroupedById(xi, yi, zi, cyl_finite, next_id, self._findCylinderNeighborhoodK, k=k)
        cap_finite = int_finite & ~cyl_closer
        nb_dfs += self._findNeighborhoodsGroupedById(xi, yi, zi, cap_finite, next_id, self._findCapNeighborhoodK, k=k)
        nb_df = pd.concat(nb_dfs, ignore_index=True) if nb_dfs else None
        return nb_df
    ...
    def findFirstHitNeighborhood(self):
        ...
        for cyl_id, cyl in enumerate(self.cyln.cylinders):
            ...
            if parname in self.params:
                dfs.append(cyl.centralNeighborhood(absz=self.params[parname]))
        ...
        for i in range(center):
            ...
            for cap_id in (center + i, center - i - 1):
                r2 = r2_first[cap_id] + self.params['nb__cap_first_inc']
                if parname in self.params:
                    r2 = self.params[parname]
                if r2 > 0:
