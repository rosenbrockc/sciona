"""Full pinned competition Inception-ResNet-v2, including its inference auxiliary head.
...
class InceptionResNetV2(nn.Module):
    def __init__(self):
        super().__init__()
        self.topology = json.loads(Path(__file__).with_name('adversarial_inception_resnet_topology.json').read_text())
        ...
        for i, node in enumerate(self.topology['nodes']):
            if node['op'] == 'conv':
                ...
                self.layers[str(i)] = nn.Linear(node['in_features'], node['out_features'])
    ...
    def forward(self, x):
        ...
        for i, node in enumerate(self.topology['nodes']):
            ...
            op = node['op']
            if op in ('conv', 'linear'): out = self.layers[str(i)](args[0])
            elif op == 'flatten':
                # Source Slim flatten is NHWC; preserve ordering even beyond 1x1.
                out = args[0].permute(0,2,3,1).reshape(args[0].shape[0],-1)
            ...
            elif op == 'scale': out = node['factor'] * args[0]
            elif op == 'add': out = args[0] + args[1]
            elif op == 'relu': out = torch.relu(args[0])
            elif op == 'squeeze':
...
def build_inception_resnet(*, seed, state=None):
