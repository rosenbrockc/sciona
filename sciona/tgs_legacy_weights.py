"""Data-only reader for the pinned, pre-zip ResNet34 archive.
...
def read_legacy_resnet34(data):
    ...
    with tarfile.open(fileobj=io.BytesIO(data), mode='r:') as archive:
        ...
        if (len(members) != 4 or {m.name for m in members} != {'sys_info', 'pickle', 'tensors', 'storages'}
                or any(not m.isfile() or m.size > 100_000_000 for m in members)):
        ...
        if f.read(1) or not isinstance(result, OrderedDict) or len(result) != 182:
