"""TrackML candidate storage and greedy hit commitment; BSD-2-Clause adaptation.

Derived from edwinst/trackml_solution; see docs/licenses/TrackML-BSD-2-Clause.txt.
Event context and hit identifiers are supplied at runtime.
"""
from collections import OrderedDict
import numpy as np
import pandas as pd

def zeroUsedHits(hit_matrix, used, min_nhits=3, max_nloss=None, max_loss_fraction=1.0, max_nrows=None, reserve_skipped=False):
...
class Candidates:
    ...
    def __init__(self, event, nmax_per_crossing=4, fit_columns=None):
        ...
        self._fit_dtype = np.float32
        self._ncross_dtype = np.int8
        self._track_id_dtype = np.int32
        self._idx_dtype = np.int32
        self._col_dtype = np.int32
    ...
    def submit(self, fill=True, used=None, min_track_id=1, min_nhits=3, max_nloss=None, max_loss_fraction=1.0, max_nrows=None, reserve_skipped=False):
        ...
        if self.open is not None:
            ...
            zeroUsedHits(masked_candidates, used, min_nhits=min_nhits, max_nloss=max_nloss, max_loss_fraction=max_loss_fraction, max_nrows=max_nrows, reserve_skipped=reserve_skipped)
            ...
            if sorted_hit_id.size and sorted_hit_id[0] == 0:
                ...
                indices = indices[1:]
            track_id = self._track_id_dtype(min_track_id) + np.floor_divide(indices, self._candidates.shape[1]).astype(self._track_id_dtype)
        else:
        ...
        submission_df = pd.DataFrame(data=OrderedDict([('event_id', np.int16(self._event.event_id)), ('hit_id', np.concatenate([sorted_hit_id, missing_hit_id])), ('track_id', np.concatenate([track_id, np.zeros(len(missing_hit_id), dtype=self._track_id_dtype)]))]))
    ...
    def analysisDataframe(self):
        ...
        fit_df = pd.DataFrame(data=self._fit[:, :self._nfit * max_ncross.astype(self._col_dtype)], columns=['%d_%s' % (i_cross, name) for i_cross in range(max_ncross) for name in self._fit_columns])
    ...
    def __str__(self, mask=None, show_coords=False, show_r2=False, show_pid=False, show_fit=False, show_df=False, break_cross=False, hit_in_cyl=None, hit_layer_id=None):
        ...
        text = 'candidates: ' + str(n) + ' (' + str(nopen) + ' open / ' + str(n - nopen) + ' closed), shape ' + str(self._candidates.shape) + '\n'
        for i in range(n):
            if mask is not None:
                if not mask[i]:
                    continue
            ...
            for j in range(self.ncross[i]):
                ...
                for hit_id in hit_ids:
                    ...
                    if show_r2:
                        hit_text += '(r2=%.1f)' % np.sqrt(np.sum([np.square(coord[hit_id]) for coord in hit_coords[:2]], axis=0))
