"""Five-fold thirty-run multiclass boosting and full-training query refits.
...
"""
import numpy as np
from sciona.otto_raw_boosting import fit_predict
...
def crossfit_boosting(training, labels, folds, training_ids, query, query_ids, *, seed, controls):
    ...
    return dict(oof=oof,query=predicted,folds=5,oof_rows=len(x),query_rows=len(q),bag_runs=30,total_model_fits=180)
