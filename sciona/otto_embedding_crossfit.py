...
"""
import numpy as np
from sciona.otto_embedding_boosting import fit_predict


def crossfit_boosting(training, labels, folds, training_ids, query, query_ids, *, embedding, variant, seed, controls):
    ...
    for fold in range(5):
        held=f==fold
        oof[held]=fit_predict(x[~held],y[~held],[i for i,h in zip(training_ids,held) if not h],x[held],[i for i,h in zip(training_ids,held) if h],embedding=embedding,variant=variant,seed=seed,controls=controls)
        coverage[held]+=1
    if not np.all(coverage==1):raise ValueError('Incomplete out-of-fold coverage')
    predicted=fit_predict(x,y,training_ids,q,query_ids,embedding=embedding,variant=variant,seed=seed,controls=controls)
