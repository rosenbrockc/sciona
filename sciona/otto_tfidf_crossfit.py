"""Five-fold TF-IDF nearest-class distances with explicit independent weighting.

Historical preprocessing scope remains unresolved; this module explicitly fits
IDF on each excluding fold and on full training for query inference.
"""
import numpy as np
...
def crossfit_tfidf_distances(training, labels, folds, training_ids, query, query_ids, *, metric, smooth_idf, sublinear_tf, norm, chunk_size=128):
    ...
    def compute(reference, targets, evaluation):
        weighting=fit_weighting(reference,smooth_idf=smooth_idf,sublinear_tf=sublinear_tf,norm=norm)
        return class_distances(weighting.transform(reference),targets,weighting.transform(evaluation),metric=metric,chunk_size=chunk_size)[:,0,:]
    oof=np.empty((len(x),9),dtype=np.float64)
    coverage=np.zeros(len(x),dtype=np.int64)
    for fold in range(5):
        held=f==fold
        oof[held]=compute(x[~held],y[~held],x[held])
        coverage[held]+=1
    if not np.all(coverage==1):raise ValueError('Incomplete out-of-fold coverage')
    predicted=compute(x,y,q)
