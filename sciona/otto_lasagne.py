"""Isolated native Lasagne CPU ensembles with reference-only standardization."""
...
def fit_predict(reference,labels,query,*,variant,seed,controls):
    ...
    if variant not in ('pair','six_log'):raise ValueError('Explicit CPU ensemble variant required')
    runs=2 if variant=='pair' else 6
    ...
    with tempfile.TemporaryDirectory(prefix='otto-private-lasagne-') as temporary:
        ...
        process=None
        try:
            process=subprocess.Popen([sys.executable,'-m','sciona.otto_lasagne_worker',str(root)],stdout=subprocess.PIPE,stderr=subprocess.PIPE,env=env,start_new_session=True)
