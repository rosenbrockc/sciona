...
# Pinned source 08a15beb36f9cbed4c3990e74c625c1332b61fe8.
# Adaptations: package-local decoder import; pretrained defaults off to avoid implicit downloads.
# Runtime checkpoint injection and training lifecycle are implemented separately.
import torch.nn as nn
import timm
import segmentation_models_pytorch as smp
from segmentation_models_pytorch.base.initialization import initialize_decoder

from sciona.contrails_temporal_decoder import UnetDecoder
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        features4 = self.encoder(x)  # => list of 5 features
        features = []
        for x in features4:
            batch_size, nch, h, w = x.shape
            features.append(x[:, :, :(h // 2), :(w // 2)])
