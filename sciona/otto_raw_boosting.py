"""Thirty seeded multiclass raw-feature boosters for Otto model 21.
...
def fit_predict(reference,labels,query,*,seed,controls):
    ...
    result=np.zeros((len(q),9),dtype=np.float64)
    for run in range(30):
        ...
        if scores.shape!=result.shape or not np.isfinite(scores).all() or (scores<0).any() or not np.allclose(scores.sum(axis=1),1.):
            raise ValueError('Invalid multiclass probabilities')
        result+=scores.astype(np.float64)/30.
    return result
