...
class SubprocessCLIClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._max_retries = _env_int("SCIONA_SUBPROCESS_MAX_RETRIES", 1, min_value=0)
    ...
    def _log_subprocess_event(self, event_type: str, payload: dict[str, object]) -> None:
        try:
            from sciona.telemetry import get_current_stage, log_event
...
def _codex_mcp_disable_overrides() -> tuple[str, ...]:
    ...
    """
    if not _env_bool("SCIONA_CODEX_SHIM_DISABLE_MCP", True):
        return ()

    config_path = Path(
        os.getenv("SCIONA_CODEX_CONFIG_PATH", "~/.codex/config.toml")
    ).expanduser()
    ...
    *,
    ...
    raise ValueError(
        (
            ...
            "SCIONA_ALLOW_LEGACY_SUBPROCESS_PROVIDERS=true to opt in."
        )
    )
    ...
    *,
    ...
    if normalized == "anthropic":
        if not anthropic_api_key:
            raise ValueError("SCIONA_ANTHROPIC_API_KEY not set")
    ...
    if normalized == "codex":
        if not openai_api_key:
            raise ValueError("SCIONA_OPENAI_API_KEY not set")
    ...
    if normalized == "claude_shim":
        from sciona.hunter.cli_socket_shim import CLISocketShimClient
    ...
    if normalized == "gemini_shim":
        from sciona.hunter.gemini_shim import GeminiShimClient
