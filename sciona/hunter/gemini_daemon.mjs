...
async function loadGeminiRuntime(cliRoot, model, cwd) {
  ...
  const sessionId = `sciona-shim-${process.pid}`;
...
async function main() {
  ...
  const cliRoot = process.env.SCIONA_GEMINI_CLI_ROOT || "";
  const fakeMode = process.env.SCIONA_GEMINI_DAEMON_FAKE === "1";
  ...
  if (!fakeMode) {
    if (!cliRoot) {
      throw new Error("SCIONA_GEMINI_CLI_ROOT is not set");
    }
      ...
      try {
        ...
        if (fakeMode) {
          ...
          const response = await baseLlmClient.generateContent({
            ...
            promptId: `sciona-shim-${process.pid}-${requestCount}`,
          });
