"""Five-fold two/six-run native Lasagne neural bagging and full-training query refits.
...
"""
import numpy as np
from sciona.otto_lasagne import fit_predict
...
def crossfit_lasagne(training, labels, folds, training_ids, query, query_ids, *, variant, seed, controls):
    ...
    return dict(oof=oof,query=predicted,folds=5,oof_rows=len(x),query_rows=len(q),bag_runs=2 if variant=='pair' else 6,total_model_fits=12 if variant=='pair' else 36)
