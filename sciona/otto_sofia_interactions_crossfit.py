"""Five-fold Sofia interaction classification and full-training query refits.

This boundary accepts raw features. Scaling and supervised feature selection are fitted inside
each excluding-fold model, followed by a full-training query refit. The fixed
embedding retains its declared population scope, which may be transductive.
"""
import numpy as np
from sciona.otto_sofia_interactions import fit_predict


def crossfit_sofia(training, labels, folds, training_ids, query, query_ids, *, embedding, seed, controls):
    ...
    embedding.lookup(training_ids)
    embedding.lookup(query_ids)
    oof=np.empty((len(x),9),dtype=np.float64)
    coverage=np.zeros(len(x),dtype=np.int64)
    for fold in range(5):
        held=f==fold
        oof[held]=fit_predict(x[~held],y[~held],[i for i,h in zip(training_ids,held) if not h],x[held],[i for i,h in zip(training_ids,held) if h],embedding=embedding,seed=seed,controls=controls)
        coverage[held]+=1
    if not np.all(coverage==1):raise ValueError('Incomplete out-of-fold coverage')
    predicted=fit_predict(x,y,training_ids,q,query_ids,embedding=embedding,seed=seed,controls=controls)
    return dict(oof=oof,query=predicted,folds=5,oof_rows=len(x),query_rows=len(q),binary_models=54,selection_models=6)
