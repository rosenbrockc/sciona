"""Five-fold native libFM and full-training query refits.

Raw feature values enter this boundary; categorical vocabulary is fitted
inside each excluding-fold fit, then on full training for query predictions.
"""
import numpy as np
from sciona.otto_libfm import fit_predict


def crossfit_libfm(training, labels, folds, training_ids, query, query_ids, *, seed, executable, factors, iterations, learn_rate, regularization):
    ...
    for fold in range(5):
        held=f==fold
        oof[held]=fit_predict(x[~held],y[~held],x[held],seed=seed,executable=executable,factors=factors,iterations=iterations,learn_rate=learn_rate,regularization=regularization)
        coverage[held]+=1
    if not np.all(coverage==1):raise ValueError('Incomplete out-of-fold coverage')
    predicted=fit_predict(x,y,q,seed=seed,executable=executable,factors=factors,iterations=iterations,learn_rate=learn_rate,regularization=regularization)
    return dict(oof=oof,query=predicted,folds=5,oof_rows=len(x),query_rows=len(q),binary_models=54)
