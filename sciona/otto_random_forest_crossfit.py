"""Five-fold native R forest and full-training query refits.
...
"""
import numpy as np
from sciona.otto_random_forest import fit_predict


def crossfit_forest(training, labels, folds, training_ids, query, query_ids, *, seed, ntree, mtry, nodesize, r_library):
    ...
    for fold in range(5):
        held=f==fold
        oof[held]=fit_predict(x[~held],y[~held],x[held],seed=seed,ntree=ntree,mtry=mtry,nodesize=nodesize,r_library=r_library)
        coverage[held]+=1
    if not np.all(coverage==1):raise ValueError('Incomplete out-of-fold coverage')
    predicted=fit_predict(x,y,q,seed=seed,ntree=ntree,mtry=mtry,nodesize=nodesize,r_library=r_library)
    return dict(oof=oof,query=predicted,folds=5,oof_rows=len(x),query_rows=len(q),forest_models=6)
