"""Record the installed CPU M5 uncertainty dependency closure and retain notices."""
import hashlib
...
ROOT_PACKAGES=('numpy','scipy','scikit-learn','lightgbm','pandas')
...
def main():
    import numpy,scipy,sklearn,lightgbm,pandas
    versions=dependency_closure()
    for name,module in [('numpy',numpy),('scipy',scipy),('scikit-learn',sklearn),('lightgbm',lightgbm),('pandas',pandas)]:
    ...
    for name in versions:
        ...
        for file in sorted(dist.files or [],key=str):
            ...
            data=path.read_bytes();digest=hashlib.sha256(data).hexdigest();target='M5U-notice-'+digest[:16]+'.txt'
    ...
    requirements=ROOT/'requirements/m5u-execution.txt'
    ...
    (ROOT/'docs/reviews/competition_m5_uncertainty_environment.json').write_text(json.dumps(report,indent=2)+'\n')
