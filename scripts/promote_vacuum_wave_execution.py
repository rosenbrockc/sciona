#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected vacuum_wave realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.vacuum_wave_execution import build_vacuum_wave_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_vacuum_wave_execution import validate

REFERENCE='mit-18013a-vacuum-wave-equation'
URL='https://www.ocw.mit.edu/ans7870/18/18.013a/textbook/HTML/chapter29/section04.html'
LIMITATIONS=[
    'Automated Tier 3 only; no human-reviewed Tier 1 or Tier 2 usage claim.',
    'Formal symbolic differential transformation of C2 Cartesian electric fields on a caller-established common open space-time region.',
    'Wave-equation equality follows only under vacuum Maxwell premises: constant positive mu/epsilon, zero charge and current.',
    'Returned expressions do not certify Maxwell compliance; wave-equation satisfaction alone does not imply the separately returned Gauss constraint.',
    'Original vector AST/LaTeX and SI dimensions explicitly corrected; original draft unchanged and no literal source parity claim.',
    'Caller establishes physical units, smoothness and branch/pole exclusions. Complex field representations permitted; no PDE or initial/boundary-value solver.',
    'Protected expression AST strings; formal derivatives preserved for composed functions. No throughput claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/vacuum_wave_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/vacuum_wave_test_review.json').read_text())
    if tests['test_results']!=dict(tests=32,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    if report['graph_digest']!=digest or report['full_runner_cases']!=6 or report['symbolic_derivative_checks']!=6 or not report['formal_derivatives_preserved'] or not report['maxwell_counterexample_preserved'] or not report['cache_ast_strings_preserved'] or report['sympy_version']!='1.14.0':raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['vacuum_wave_execution.json','vacuum_wave_test_review.json','vacuum_wave_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('vacuum_wave-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'vacuum_wave-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.vacuum_wave_corrected_execution'
        ...
        description='Construct symbolic vector vacuum wave-equation sides and separate Gauss divergence constraint from arbitrary C2 Cartesian electric fields. Reviewed six-step Maxwell derivation and corrected SI dimensions. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'wave'},dict(version_id=version_id,node_id='wave',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='vacuum_wave-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['laplacian_srepr','scaled_acceleration_srepr','divergence_srepr']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; vector derivatives, curl-curl identity and SI dimensions explicitly reconstructed for C2 Cartesian fields under vacuum Maxwell premises. Original draft remains unchanged.'))})
        title='MIT 18.013A: The Wave Equation'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Vacuum Maxwell wave equation from time differentiation and curl operations in the absence of charge and current.'})
            ...
            evidence_id=uuid5(selected,'vacuum_wave-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='vacuum_wave-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'vacuum-wave-c2-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='C2 electric-field components in SI V/m on a common open Cartesian space-time region. Positive constant vacuum mu/epsilon; zero charge/current required for conditional equality of wave-equation sides. Separate Gauss constraint returned. No Maxwell compliance certification or PDE solver.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
        for selected in [spec.version_id,version_id]:
            if db.execute('SELECT count(*) AS n FROM artifact_io_specs WHERE version_id=%s',(selected,)).fetchone()['n']!=7:
                raise ValueError('Unexpected canonical ports')
        if db.execute('SELECT count(*) AS n FROM atom_io_specs WHERE version_id=%s',(spec.version_id,)).fetchone()['n']!=7:
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=4,top_level_output_arity=3,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
