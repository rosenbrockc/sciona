#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected helmholtz realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.helmholtz_execution import build_helmholtz_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_helmholtz_execution import validate

REFERENCE='mit-6013-harmonic-helmholtz'
...
LIMITATIONS=[
    ...
    'Time-independent C2 spatial amplitude in Cartesian coordinates, real constant omega and positive constant c.',
    'Zero/negative frequency supported; complex harmonic convention exp(+i omega t).',
    'Residuals computed for arbitrary amplitudes; no PDE solution, boundary condition or Maxwell divergence certification.',
    'Caller establishes common open domain, smoothness and consistent field/coordinate/coefficient units.',
    'Malformed source differential operators reconstructed; no literal source AST parity claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/helmholtz_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/helmholtz_test_review.json').read_text())
    if tests['test_results']!=dict(tests=36,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    if report['graph_digest']!=digest or report['full_runner_cases']!=6 or report['symbolic_derivative_checks']!=6 or not report['formal_derivatives_preserved'] or not report['nonzero_residual_preserved'] or not report['cache_ast_strings_preserved'] or report['sympy_version']!='1.14.0':raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['helmholtz_execution.json','helmholtz_test_review.json','helmholtz_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('helmholtz-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'helmholtz-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.helmholtz_corrected_execution'
        ...
        description='Construct time-harmonic vector field, spatial Laplacian and Helmholtz residual with exact symbolic derivatives. Preserve non-solutions and zero frequency. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'harmonic'},dict(version_id=version_id,node_id='harmonic',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='helmholtz-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['harmonic_field_srepr','laplacian_srepr','helmholtz_residual_srepr']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; scalar-factor differential ASTs reconstructed as time-harmonic wave to Helmholtz reduction. Original draft unchanged.'))})
        title='MIT 6.013 Electromagnetics and Applications: Time-Harmonic Helmholtz Equation'
