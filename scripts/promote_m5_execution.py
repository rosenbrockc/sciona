...
RUNNER = 'm5-execution-community.v1'
from scripts.review_m5_execution import LIMITATIONS
...
def review(root):
    from scripts.review_m5_execution import audit
    import sciona.atoms.ml.m5_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_m5_semantic_review.json').read_text())
    ...
    for name in ('competition_m5_semantic_review.json','competition_m5_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_m5_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_m5_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_m5_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if {n.node_id for n in original.nodes} != {'temporal_unrolling','recursive_feature_engineering','objective_function_optimization','magic_multiplier','hierarchical_reconciliation'}:
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community independent M5 CPU pipeline: semantic-role preprocessing, six pooled Tweedie families, 28-day recursive/nonrecursive forecasting and equal family mean. Full model-count synthetic qualification; no historical accuracy claim.',
        ...
        references=[]
        for reference,url,title in [('m5-winner-archive-18e4e77','https://github.com/Mcompetitions/M5-methods/tree/18e4e776cfa1ded3b387fabe008c6c2c44922007','M5 organizers archive of winning methods: A1')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Primary A1 software provenance for semantic-role preprocessing, six pooled Tweedie families and 28-day equal-mean forecasts. Independent mathematical reconstruction; archive reuse grant and historical native parity remain unqualified.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_m5_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_m5_publication_gates.json':sha(gates_path)})
