"""Read back served HuBMAP versions and execute the catalog graph against source."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_hubmap_serialized as comparison


def verify(root, source_root, library_root, notebook):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_hubmap_serialized.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('325c30ff-ea2a-515c-8d40-110a5fd6991d',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='hubmap-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        assert len(bindings) == len(graph.nodes) == 6
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='hubmap-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert {r['name'] for r in outputs} == {'initial_metrics', 'retraining_metrics', 'predictions'}
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='6f290e9ea9794b85f83ff3e52034a41a1c400f84d2f3f5182beb65d837664468', dependency_role='cdg', optional=False)]
    ...
    with patch.object(comparison, 'build_hubmap_lifecycle_graph', return_value=graph):
        result = comparison.validate(root, source_root, library_root, notebook)
    assert result == expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=5,
        served_cdg_count=served, catalog_graph_source_comparison='passed', inference_calls=result['inference_calls'],
...
if __name__ == '__main__':
    ...
    report = verify(root, args.source_root, args.library_root, args.notebook)
    (root / 'docs/reviews/competition_hubmap_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
