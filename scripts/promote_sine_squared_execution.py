#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected sine_squared realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.sine_squared_execution import build_sine_squared_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_sine_squared_execution import validate

REFERENCE='openstax-sine-power-reduction'
...
LIMITATIONS=[
    ...
    'Provably finite real symbolic angle in dimensionless radians; caller establishes assumptions.',
    'Squared identity with all13source transitions and Euler/Pythagorean prerequisites; independent endpoint ODE certificate.',
    'Source angle alias, renaming feed and dependency order corrected; no literal source AST parity.',
    'No principal-square-root sine-sign inference or arbitrary complex-angle extension.',
    'Symbolic outputs; no claim of stable float64 evaluation of1-cos(2*x) by downstream consumers.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/sine_squared_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/sine_squared_test_review.json').read_text())
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['sine_squared_execution.json','sine_squared_test_review.json','sine_squared_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('sine_squared-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'sine_squared-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.sine_squared_corrected_execution'
        ...
        description='Construct sine-squared and half-angle expressions with full source reconstruction and independent endpoint certificate. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'formula'},dict(version_id=version_id,node_id='formula',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='sine_squared-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['sine_squared_srepr','half_angle_srepr','certificate_json']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; angle alias, renaming feed and dependency order corrected with all13transitions retained. Original draft unchanged.'))})
        title='OpenStax Precalculus2e: Double-Angle, Half-Angle, and Reduction Formulas'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Power reduction identity sin(x)^2=(1-cos(2*x))/2 follows from double-angle cosine.'})
