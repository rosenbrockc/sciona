"""Atomically publish the source-compared CHAMPS realization at automated Tier 3.
...
COMMITS = {'winning_solver': '4a42e18b5b88043fb40ec15289216a1d88789698'}
RUNNER = 'champs-execution-community.v1'
from scripts.review_champs_execution import LIMITATIONS
...
def review(root):
    from scripts.review_champs_execution import audit
    import sciona.atoms.ml.champs_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_champs_semantic_review.json').read_text())
    ...
    graph=build_champs_graph()
    digest,nodes,edges=encode_execution_graph(graph)
    full=json.loads((reviews/'competition_champs_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_champs_review_gates.json').read_text())
    ...
    for name in ('competition_champs_semantic_review.json','competition_champs_review_gates.json'):
...
def promote(root, apply=False):
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community CHAMPS corrected full graph-transformer solution: raw chemistry, thirteen full models, training and complete checkpoint reload, subtype scaling and trimmed ensemble. Explicit caller schedules; automated synthetic evidence, no historical accuracy claim.',
        ...
        references=[]
        reference='champs-winning-solver-'+COMMITS['winning_solver'][:12]
        url='https://github.com/boschresearch/BCAI_kaggle_CHAMPS/tree/'+COMMITS['winning_solver']
        title='Pinned CHAMPS first-place graph-transformer solution (MIT; bundled MIT/Apache notices)'
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Pinned primary Bosch CHAMPS software; full configured graph-transformer execution with explicit corrections and retained MIT/Apache notices.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_champs_graph_execution.json').read_text())['code_sha256'],
