#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected brewster_angle realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.brewster_angle_execution import build_brewster_angle_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_brewster_angle_execution import validate

REFERENCE='openstax-brewster-polarization'
...
LIMITATIONS=[
    ...
    'Positive incident/transmitted indices, planar isotropic lossless nonmagnetic interface, p-polarization.',
    'Equal indices give pi/4 convention; no unique reflection zero.',
    'Independent radians outputs may round near-grazing to float64pi/2; no exact Fresnel claim from rounded angles.',
    'Positive representable output angles required; subnormals accepted, underflow0 rejected; no broadcasting.',
    'Corrected reciprocal ratio, degree/radian and malformed source ASTs; no literal parity claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/brewster_angle_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/brewster_angle_test_review.json').read_text())
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['brewster_angle_execution.json','brewster_angle_test_review.json','brewster_angle_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('brewster_angle-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'brewster_angle-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.brewster_angle_corrected_execution'
        ...
        description='Compute Brewster incidence and complementary refraction angles with corrected incident/transmitted ratio and independent rounding. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'interface'},dict(version_id=version_id,node_id='interface',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='brewster_angle-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['incidence_angle','refraction_angle']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; reciprocal source endpoint corrected, radians restored and angle identities reconstructed. Original draft unchanged.'))})
        title='OpenStax University Physics Volume3: Polarization'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Brewster law uses transmitted/incident index ratio and p-polarization under lossless dielectric assumptions.'})
            ...
            evidence_id=uuid5(selected,'brewster_angle-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='brewster_angle-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'brewster-positive-index-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Positive real indices at lossless isotropic nonmagnetic interface. Independent incidence/refraction radians, p-polarized zero reflection in exact model. Equal indices convention; floating-point near-grazing rounding is not exact optical certification.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
