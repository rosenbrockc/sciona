"""Atomically publish the source-compared Text Pair realization at automated Tier 3.
...
RUNNER = 'text_pair-execution-community.v1'
from scripts.review_text_pair_execution import LIMITATIONS
...
def review(root):
    from scripts.review_text_pair_execution import audit
    import sciona.atoms.ml.text_pair_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_text_pair_semantic_review.json').read_text())
    ...
    full=json.loads((reviews/'competition_text_pair_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_text_pair_review_gates.json').read_text())
    ...
    for name in ('competition_text_pair_semantic_review.json','competition_text_pair_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_text_pair_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_text_pair_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_text_pair_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community generic binary text-pair pipeline with normalization, lexical similarities, training-only LSA and pair-graph context, logistic/random-forest score blend and held-out F1 threshold. Synthetic evidence; no historical winning solution or calibrated-probability claim.',
        ...
        for reference,url,title in [
            ('text-pair-sklearn-tfidf','https://scikit-learn.org/1.8/modules/generated/sklearn.feature_extraction.text.TfidfVectorizer.html','Scikit-learn TF-IDF vocabulary and transform documentation'),
            ('text-pair-sklearn-svd','https://scikit-learn.org/1.8/modules/generated/sklearn.decomposition.TruncatedSVD.html','Scikit-learn truncated SVD and latent semantic analysis')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Library documentation supporting explicit training-only lexical/LSA features; independently tested binary ensemble and threshold realization, not a historical winning solution.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_text_pair_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_text_pair_publication_gates.json':sha(gates_path)})
