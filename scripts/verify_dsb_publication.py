"""Read back served DSB versions and execute the catalog graph against source."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_dsb_serialized_training as comparison


async def verify(root, source_root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_dsb_full_source_execution.json').read_text())
    fqdn = 'cdg.competition.solution.kaggle.dsb2017_1st.execution'
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        record = rows[0]
        assert record['content_hash'] == expected['execution_graph_sha256']
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='dsb-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        assert len(bindings) == len(graph.nodes) == 8
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='dsb-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert {r['name'] for r in outputs} == {'metrics', 'case_probability', 'proposal_probabilities'}
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='641e41bc1adb24d96850eb4974f5a39e807bbadbabb7f9dee2a8dc33222b3c9d', dependency_role='cdg', optional=False)]
    ...
    with patch.object(comparison, 'build_dsb_raw_training_graph', return_value=graph):
        result = await comparison.validate(root, source_root, include_inference=True)
    assert result == expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=8,
        served_cdg_count=served, catalog_graph_source_comparison='passed', cases=result['cases'],
...
if __name__ == '__main__':
    ...
    report = asyncio.run(verify(root, args.source_root))
    (root / 'docs/reviews/competition_dsb_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
