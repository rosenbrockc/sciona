"""Reject weakened Physical operator evidence without changing files or catalog."""
import hashlib
import json
from pathlib import Path
from unittest.mock import patch
import review_physical_operator_execution as review
...
def main():
    ...
    def failure(suffix,name,mutate):
        target=f'competition_physical_operator_{suffix}.json'
    ...
    failure('runtime_tests','calibration_leakage',lambda d:d['checks'].__setitem__('validation_training_isolation',False))
    failure('runtime_tests','missing_oof_validation',lambda d:d['checks'].__setitem__('projection_kkt',False))
    failure('runtime_tests','query_population_leakage',lambda d:d['checks'].__setitem__('query_training_isolation',False))
    ...
    paths=['scripts/review_physical_operator_execution.py','scripts/validate_physical_operator_review_gates.py']
    report=dict(format='physical_operator-review-negative-gates.v1',result='passed',
    ...
    (review.ROOT/'docs/reviews/competition_physical_operator_review_gates.json').write_text(json.dumps(report,indent=2)+'\n')
