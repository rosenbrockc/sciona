#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected eigenstate_orthogonality realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.eigenstate_orthogonality_execution import build_eigenstate_orthogonality_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_eigenstate_orthogonality_execution import validate

REFERENCE='mit-8321-hermitian-eigenstate-orthogonality'
URL='https://ocw.mit.edu/courses/8-321-quantum-theory-i-fall-2017/36c32531fa12a2d99687cb3f0ac58502_MIT8_321F17_lec2.pdf'
LIMITATIONS=[
    'Automated Tier 3 only; no human-reviewed Tier 1 or Tier 2 claim.',
    'Finite Hermitian operator in an orthonormal basis; nonzero eigenstates and real eigenvalues verified exactly.',
    'Undecidable premises rejected; no tolerance-based approximate-eigenvector acceptance.',
    'Degenerate eigenvalues permit nonzero overlap; no orthogonalization or eigensolver claim.',
    'Caller establishes consistent common operator units and dimensionless vector components.',
    'Malformed source ASTs reconstructed; no literal source parity or infinite-dimensional domain claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/eigenstate_orthogonality_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/eigenstate_orthogonality_test_review.json').read_text())
    if tests['test_results']!=dict(tests=41,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    if report['graph_digest']!=digest or report['full_runner_cases']!=6 or not report['exact_scalar_sum_oracle'] or not report['nonzero_degenerate_overlap_preserved'] or not report['cache_ast_strings_preserved']:raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['eigenstate_orthogonality_execution.json','eigenstate_orthogonality_test_review.json','eigenstate_orthogonality_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('eigenstate_orthogonality-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'eigenstate_orthogonality-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.eigenstate_orthogonality_corrected_execution'
        ...
        description='Verify exact Hermitian eigenstates and compute complex overlap, matrix element and eigenvalue-gap product, retaining degenerate nonzero overlap. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'eigenstates'},dict(version_id=version_id,node_id='eigenstates',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='eigenstate_orthogonality-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['overlap_srepr','matrix_element_srepr','gap_overlap_srepr']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; ordered Hermitian eigenstate identity reconstructed from incomplete symbolic fields. Original draft remains unchanged.'))})
        title='MIT 8.321 Quantum Theory I: Hermitian Eigenstates'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Hermitian eigenstate identity and distinct-eigenvalue orthogonality; the implementation preserves degenerate overlap.'})
            ...
            evidence_id=uuid5(selected,'eigenstate_orthogonality-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='eigenstate_orthogonality-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'eigenstate-orthogonality-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Finite Hermitian matrix in orthonormal basis with nonzero exact eigenstates. Common caller-established operator units. Equal eigenvalues accepted without zero-overlap claim. Undecidable conditions rejected; no infinite-dimensional operator scope.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=5,top_level_output_arity=3,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
