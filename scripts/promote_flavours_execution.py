"""Atomically publish the source-compared Flavours realization at automated Tier 3.
...
from scripts.review_flavours_execution import SOURCE_VERSION, SOURCE_HASH, STAGE_MAPPING
ROOT = Path(__file__).resolve().parents[1]
LIBRARY_VERSIONS = dict(line.split('==', 1) for line in
    (ROOT / 'requirements/flavours-reference.txt').read_text().splitlines()
    if line and not line.startswith('#'))
RUNNER = 'flavours-execution-community.v1'
EXPECTED_GATES = {
    'semantic_review:proposed_tier', 'semantic_review:verdict',
    'semantic_review:review_source', 'graph_validation:serialized_graph_sha256',
    'evidence_hash', 'installed_dependency',
}
from scripts.review_flavours_execution import LIMITATIONS as CORE_LIMITATIONS
LIMITATIONS = CORE_LIMITATIONS + json.loads(
    (ROOT / 'docs/reviews/competition_flavours_reference_scope_review.json').read_text())['conditions']
...
def review(root):
    from scripts.review_flavours_execution import final_review
    from importlib.metadata import version
    from sciona.atoms.ml import flavours_execution as provider
    expected = final_review()
    reviews = root / 'docs/reviews'
    semantic = json.loads((reviews / 'competition_flavours_semantic_review.json').read_text())
    if semantic != expected or semantic['conditions'] != LIMITATIONS:
        raise ValueError('Final semantic review differs from current evidence or conditions')
    if (semantic['proposed_tier'] != 3 or semantic['review_source'] != 'automated'
            or semantic['verdict'] != 'acceptable_with_limits' or not semantic['ready_for_transaction']):
        raise ValueError('Automated Tier 3 conditional review required')
    if len(LIBRARY_VERSIONS) != 94 or any(version(name) != pin for name,pin in LIBRARY_VERSIONS.items()):
        raise ValueError('Reviewed dependency runtime differs')
    check_hashes(root, semantic['evidence_sha256'])
    graph = build_graph()
    digest,nodes,edges = encode_execution_graph(graph)
    full = json.loads((reviews / 'competition_flavours_graph_validation.json').read_text())
    if full['graph_execution']['serialized_graph_sha256'] != digest or (len(nodes),len(edges)) != (1,0):
        raise ValueError('Trained graph identity differs')
    check_hashes(root, full['source_sha256'])
    ...
    for node in graph.nodes:
        ...
        spec = matches[0]
        fn = getattr(provider, spec.source_symbol)
        if Path(inspect.getfile(inspect.unwrap(fn))).resolve() != spec.file_path.resolve() or sha(spec.file_path) != full['graph_execution']['provider_sha256']:
            raise ValueError('Provider source comparison stale')
        specs[node.node_id] = spec
        interfaces[node.node_id] = provider_ports(node, fn)
        signatures[node.node_id] = str(inspect.signature(fn))
        witness = getattr(provider,'witness_'+spec.source_symbol)
        if list(inspect.signature(witness).parameters) != [p.name for p in node.inputs]:
            raise ValueError('Witness ports differ')
        values = {}
    ...
    if len(boundary['input']) != 6 or [p.name for p in boundary['output']] != ['result']:
        raise ValueError('Unexpected execution boundary')
    evidence = {str(p.relative_to(root)):sha(p) for p in [
        reviews/'competition_flavours_semantic_review.json',
        reviews/'competition_flavours_graph_validation.json',
        root/'requirements/flavours-reference.txt']}
    evidence.update(semantic['evidence_sha256'])
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_flavours_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates.get('format')!='flavours-publication-gates.v1'
            or gates['result']!='passed' or len(gates['rejected_gates'])!=len(EXPECTED_GATES)
            or set(gates['rejected_gates'])!=EXPECTED_GATES
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_flavours_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'docs/reviews/competition_flavours_publication_gates.json':sha(gates_path)})
    created = 0
    rollup = dict(overall_verdict='acceptable_with_limits', structural_status='pass', runtime_status='pass',
        semantic_status='conditional_pass', developer_semantics_status='conditional_pass', review_status='approved',
        review_semantic_verdict='conditional_pass', review_developer_semantics_verdict='conditional_pass', trust_readiness='ready',
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if {n.node_id for n in original.nodes} != set(STAGE_MAPPING):
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community corrected Flavours CPU workflow: physical features,repeated mass regression,five classifier branches,fifty neural fits,nonlinear blend and source diagnostics. Explicit runtime choices; no historical accuracy claim.',
        ...
        references=[]
        for reference,url,title in [('flavours-winner-go-polar-bears','https://github.com/aguschin/flavours-of-physics/tree/e642aafa4b2ddf8baa572cbdf2926450935ed005','Flavours first-place Go Polar Bears source')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Winning Go Polar Bears method provenance. Independent CPU reconstruction with explicit modern runtime and cross-fit choices; exact winner runtime and score equivalence unproven.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_flavours_graph_validation.json').read_text())['source_sha256'],
        ...
        for table, field, selected, expected in [('artifact_cdg_bindings', 'version_id', version_id, 1),
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=%s,top_level_output_arity=%s,topo_hash=%s WHERE artifact_id=%s", (len(boundary['input']), len(boundary['output']), topo, graph_id))
        ...
        rechecked_evidence=dict(rechecked[-1],**{'docs/reviews/competition_flavours_publication_gates.json':sha(gates_path)})
