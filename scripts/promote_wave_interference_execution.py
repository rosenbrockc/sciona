#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected wave_interference realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.wave_interference_execution import build_wave_interference_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_wave_interference_execution import validate

REFERENCE='openstax-two-wave-interference'
...
LIMITATIONS=[
    ...
    'Two scalar amplitudes in a common mode/polarization and common normalized intensity scale.',
    'Finite nonnegative fixed amplitudes, not both zero; finite relative phase in radians.',
    'Fixed-phase coherent intensity is distinct from constructive phase alignment.',
    'Incoherent mean assumes fixed amplitudes and zero averaged cosine; uniform relative phase is sufficient.',
    'Ratio two requires equal nonzero amplitudes and constructive phase; destructive cancellation may give zero ratio.',
    'Local1200digit positive intensity formula; float64 phase is literal, including rounded pi and very large phases.',
    'Exact zero/subnormals accepted; nonzero output underflow or overflow rejects the entire call.',
    'Independent rounding does not preserve exact sums/ratios; no universal correct-rounding claim.',
    'Conjugation, cross terms, cosine substitution, graph bindings and ensemble averaging reconstructed; no literal AST parity.',
    'No arbitrary polarization, propagation, stochastic-amplitude, partial-coherence or absolute SI calibration model.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/wave_interference_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/wave_interference_test_review.json').read_text())
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['wave_interference_execution.json','wave_interference_test_review.json','wave_interference_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('wave_interference-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'wave_interference-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.wave_interference_corrected_execution'
        ...
        description='Compute normalized two-wave fixed-phase intensity, incoherent mean, constructive and destructive extrema, interference cross term and ratio. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'interference'},dict(version_id=version_id,node_id='interference',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='wave_interference-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['intensity','incoherent_mean','constructive','destructive','interference','ratio']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; conjugation and cross-term corrections, explicit phase substitution, repaired source bindings and fixed-amplitude ensemble interpretation. Factor-two branch requires equal nonzero amplitudes and constructive phase. Original draft unchanged.'))})
        title='OpenStax University Physics Volume3: Youngs Double-Slit Interference'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Coherent phase relationships, random incoherent phase relationships, and constructive/destructive interference of amplitudes.'})
            ...
            evidence_id=uuid5(selected,'wave_interference-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='wave_interference-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'normalized-fixed-amplitude-interference-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Two normalized scalar waves with common mode/polarization, finite fixed nonnegative amplitudes not both zero, and finite literal relative phase. Incoherent mean requires vanishing ensemble cosine. Factor two is restricted to equal amplitudes and constructive phase; all outputs independently rounded.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
