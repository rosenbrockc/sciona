"""Atomically publish the source-compared Otto realization at automated Tier 3.
...
LIBRARY_VERSIONS = {'certifi': '2026.2.25', 'charset-normalizer': '3.4.5', 'h2o': '3.46.0.8', 'idna': '3.11', 'joblib': '1.5.3', 'numpy': '2.4.2', 'requests': '2.32.5', 'scikit-learn': '1.8.0', 'scipy': '1.17.1', 'six': '1.17.0', 'tabulate': '0.10.0', 'threadpoolctl': '3.6.0', 'urllib3': '2.6.3', 'xgboost': '3.4.1'}
RUNNER = 'otto-execution-community.v1'
from scripts.review_otto_execution import LIMITATIONS
...
def review(root):
    from scripts.review_otto_execution import audit
    import sciona.atoms.ml.otto_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_otto_semantic_review.json').read_text())
    ...
    full=json.loads((reviews/'competition_otto_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_otto_review_gates.json').read_text())
    ...
    for name in ('competition_otto_semantic_review.json','competition_otto_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_otto_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_otto_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_otto_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if {n.node_id for n in original.nodes} != {'numerical_engineering','interaction_features','stratification','base_modeling','level_2_meta_modeling','final_ensemble'}:
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community independent Otto CPU stack: 33 first-level entries, seven supplemental blocks, four-fold meta selection, complete 250/600/250 bags and the documented final blend. Synthetic qualification; no historical winning accuracy claim.',
        ...
        references=[]
        for reference,url,title in [('otto-winner-method-14335','https://www.kaggle.com/competitions/otto-group-product-classification-challenge/writeups/gilberto-titericz-stanislav-semenov-1st-place-winn','Otto winner complete stacking method')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Primary provenance for all enumerated first-level entries, supplemental features, shared folds, full meta bags and the geometric/arithmetic blend. Independent CPU implementation; historical details and competitive accuracy remain unqualified.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_otto_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_otto_publication_gates.json':sha(gates_path)})
