"""Atomically publish the source-compared Santander realization at automated Tier 3.
...
RUNNER = 'santander-execution-community.v1'
from scripts.review_santander_execution import LIMITATIONS
...
def review(root):
    from scripts.review_santander_execution import audit
    import sciona.atoms.ml.santander_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_santander_semantic_review.json').read_text())
    ...
    graph=build_santander_graph()
    digest,nodes,edges=encode_execution_graph(graph)
    full=json.loads((reviews/'competition_santander_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_santander_review_gates.json').read_text())
    ...
    for name in ('competition_santander_semantic_review.json','competition_santander_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_santander_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_santander_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_santander_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if {n.node_id for n in original.nodes} != {'fake_data_identification','real_data_filtering','count_encoding','value_substitution','shuffle_augmentation','lightgbm_training'}:
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community independent Santander CPU ensemble: supervised uniqueness features, shared-attention neural model, augmented LightGBM, full hard pseudo-label retraining and 2.1:1 rank blend. Synthetic qualification; no independent validation or historical accuracy claim.',
        ...
        references=[]
        for reference,url,title in [('santander-winner-method-89003','https://www.kaggle.com/competitions/santander-customer-transaction-prediction/writeups/wizardry-1-solution','Santander winner uniqueness and pseudo-label ensemble')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Primary provenance for supervised uniqueness features, neural/tree augmentation, hard pseudo-label retraining and final rank blend. Independent CPU implementation; historical details and competitive accuracy remain unqualified.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_santander_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_santander_publication_gates.json':sha(gates_path)})
