#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected circular_orbit_speed realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.circular_orbit_speed_execution import build_circular_orbit_speed_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_circular_orbit_speed_execution import validate

REFERENCE='nasa-earth-circular-speed-approximation'
...
LIMITATIONS=[
    ...
    'Positive radius and circuit period; circular-path average speed, instantaneous only for uniform motion.',
    'SI input units supplied by caller; no calendar conversion implicit.',
    'Educational Earth radius and365day period are approximations, not an ephemeris or gravitational model.',
    'Both outputs positive finite representable float64; no broadcasting; pi precision evidence is not universal rounding proof.',
    'Missing powers and units reconstructed; exact conversion retained until final source-example rounding; no literal AST parity.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/circular_orbit_speed_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/circular_orbit_speed_test_review.json').read_text())
    if tests['test_results']!=dict(tests=34,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['circular_orbit_speed_execution.json','circular_orbit_speed_test_review.json','circular_orbit_speed_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('circular_orbit_speed-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'circular_orbit_speed-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.circular_orbit_speed_corrected_execution'
        ...
        description='Compute circular-path circumference and average speed from radius and period, including reviewed source example with exact unit conversion. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'orbit'},dict(version_id=version_id,node_id='orbit',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='circular_orbit_speed-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['circumference','average_speed']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; missing scientific notation and units reconstructed with exact365day conversion and approximate final rounding. Original draft unchanged.'))})
        title='NASA Basics of Space Flight: Reference Systems'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Describes non-circular Earth orbit and varying speed; supports limitation of the reviewed educational circular approximation.'})
