#!/usr/bin/env python3
"""Create non-latest exact source-ID alternatives without parsing display labels."""
import argparse
...
def main():
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        ...
        for row in rows:
            ...
            normalized,display_labels=normalize_source_identities(fresh['upstream_symbolic']['sympy_srepr'],definitions)
            source_symbols={name:name for name in normalized.variables}
            if normalized.srepr_str==row['sympy_srepr']:continue
            digest=_digest({'kind':'pdg-source-identity-normalization.v1','previous_content_hash':row['previous_content_hash'],'source_ast':fresh['upstream_symbolic']['sympy_srepr'],'normalized_srepr':normalized.srepr_str,'dimensional_hash':normalized.dimensional_hash,'candidate_payload_sha256':fresh['candidate_payload_sha256']})
            version_id=uuid5(UUID(str(row['artifact_id'])),'pdg-source-identity-normalization.v1:'+digest);expression_id=uuid5(version_id,'primary')
            ...
            comparison={'runner_version':'pdg-source-identity-comparison.v1',
                'correspondence':'exact_source_identity_ast','source_ast_sha256':_digest(fresh['upstream_symbolic']['sympy_srepr']),
                'dimension_status':'checker_passed','symbol_file_sha256':fresh['symbol_file_sha256'],
                'scope':'Exact source-ID AST, not display-label or original-LaTeX correspondence.'}
            provenance={'runner_version':'pdg-source-identity-normalization.v1','selected_representation':'pinned upstream SymPy AST preserving source scalar identities','scope':'alternative source representation, not a claim that the original LaTeX parse was corrected or that physical behavior is validated','original_latex_agreement':'not_established','previous_expression_id':str(row['expression_id']),'previous_version_id':str(row['version_id']),'previous_content_hash':row['previous_content_hash'],'previous_evidence_sha256':_digest(row['evidence_json']),'source_symbol_mapping':source_symbols,'display_labels':display_labels,'implementation_hashes':{p:hashlib.sha256((root/p).read_bytes()).hexdigest() for p in ['sciona/physics_ingest/source_identity_normalization.py','sciona/ghost/symbolic_normalization.py','scripts/materialize_source_identity_normalizations.py','tests/physics_ingest/test_source_identity_normalization.py']}}
            dimensional={'runner_version':'pdg-source-identity-normalization.v1','status':'passed','dimensional_hash':normalized.dimensional_hash,'source_comparison_sha256':_digest(comparison),'scope':'source-backed SI dimensional consistency, not physical validity'}
            expression={'expression_id':expression_id,'artifact_id':row['artifact_id'],'version_id':version_id,'candidate_id':row['candidate_id'],'expression_kind':row['expression_kind'],'expression_role':row['expression_role'],'sympy_srepr':normalized.srepr_str,'canonical_expr_hash':normalized.expression_hash,'topology_hash':normalized.topology_hash,'dimensional_hash':normalized.dimensional_hash,'raw_formula':normalized.srepr_str,'raw_formula_format':'sympy','source_expression_id':row['source_expression_id'],'parse_status':'normalized','parse_confidence':0.95,'review_status':'unreviewed','validation_status':'unknown','evidence_json':Jsonb({'source_normalization':provenance,'parse_roundtrip':{'status':'passed'},'source_identity_comparison':comparison,'dimensional_analysis':dimensional})}
            ...
            ensure_row(db,'artifact_audit_evidence',{'evidence_id':audit_id},{'evidence_id':audit_id,'artifact_id':row['artifact_id'],'version_id':version_id,'audit_type':'asset_integrity_check','passed':True,'status':'completed','source_kind':'automated','runner_version':'pdg-source-identity-normalization.v1','details':Jsonb({'scope':'source-ID AST, roundtrip and source-backed dimensions only; display labels unparsed; original correspondence and physical validity not established; bindings/latest unchanged','content_hash':digest,'provenance':provenance})})
