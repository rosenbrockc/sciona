"""Atomically publish the source-compared Resource Route realization at automated Tier 3.
...
LIBRARY_VERSIONS = {'pydantic': '2.12.5'}
RUNNER = 'resource_route-execution-community.v1'
from scripts.review_resource_route_execution import LIMITATIONS
...
def review(root):
    from scripts.review_resource_route_execution import audit
    import sciona.atoms.ml.resource_route_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_resource_route_semantic_review.json').read_text())
    ...
    full=json.loads((reviews/'competition_resource_route_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(5,4):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_resource_route_review_gates.json').read_text())
    ...
    for name in ('competition_resource_route_semantic_review.json','competition_resource_route_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_resource_route_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_resource_route_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_resource_route_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community five-stage finite resource-constrained route planning with candidate filtering, exact expanded-state search, independent path validation and explicit search-limit status. Synthetic evidence; no historical winning agent or arbitrary simulator claim.',
        ...
        for reference,url,title in [
            ('resource-route-python-heapq','https://docs.python.org/3.13/library/heapq.html','Python priority-queue operations used by independent route search')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Standard-library priority queue documentation; route-search correctness supported by explicit source scope and synthetic exhaustive comparisons, not historical competition performance.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_resource_route_graph_execution.json').read_text())['code_sha256'],
        ...
        for table, field, selected, expected in [('artifact_cdg_bindings', 'version_id', version_id, 5),
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=5,top_level_input_arity=%s,top_level_output_arity=%s,topo_hash=%s WHERE artifact_id=%s", (len(boundary['input']), len(boundary['output']), topo, graph_id))
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_resource_route_publication_gates.json':sha(gates_path)})
