"""Atomically publish the source-compared Future Sales realization at automated Tier 3.
...
LIBRARY_VERSIONS = {'lightgbm':'4.7.0','optuna':'4.8.0'}
RUNNER = 'future_sales-execution-community.v1'
from scripts.review_future_sales_execution import LIMITATIONS
...
def review(root):
    from scripts.review_future_sales_execution import audit
    import sciona.atoms.ml.future_sales_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_future_sales_semantic_review.json').read_text())
    ...
    graph=build_future_sales_graph()
    digest,nodes,edges=encode_execution_graph(graph)
    full=json.loads((reviews/'competition_future_sales_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_future_sales_review_gates.json').read_text())
    ...
    for name in ('competition_future_sales_semantic_review.json','competition_future_sales_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_future_sales_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_future_sales_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_future_sales_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community generic period-demand pipeline with aggregate-then-clip targets, causal lag/rolling/group means, LightGBM TPE search, early stopping, selected-round refit and one-period forecast in an isolated worker. Synthetic evidence; no historical winning recipe or fixed-origin multihorizon claim',
        ...
        references=[]
        for reference,url,title in [
            ('future-sales-host-aggregation','https://www.kaggle.com/competitions/competitive-data-science-final-project/discussion/42488','Competition host target aggregation and clipping clarification'),
            ('future-sales-lightgbm-early-stopping','https://lightgbm.readthedocs.io/en/stable/pythonapi/lightgbm.early_stopping.html','LightGBM early-stopping documentation'),
            ('future-sales-optuna-tpe','https://optuna.readthedocs.io/en/v4.8.0/reference/samplers/generated/optuna.samplers.TPESampler.html','Optuna 4.8.0 TPE documentation')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Host target clarification and library documentation supporting the explicit generic forecasting pipeline; retained notices and synthetic evidence, not a historical winning solution.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_future_sales_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_future_sales_publication_gates.json':sha(gates_path)})
