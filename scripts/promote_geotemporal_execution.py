"""Atomically publish the source-compared Geotemporal realization at automated Tier 3.
...
LIBRARY_VERSIONS = {'numpy': '2.4.2', 'scikit-learn': '1.8.0', 'threadpoolctl': '3.6.0', 'joblib': '1.5.3', 'scipy': '1.17.1'}
RUNNER = 'geotemporal-execution-community.v1'
from scripts.review_geotemporal_execution import LIMITATIONS
...
def review(root):
    from scripts.review_geotemporal_execution import audit
    import sciona.atoms.ml.geotemporal_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_geotemporal_semantic_review.json').read_text())
    ...
    full=json.loads((reviews/'competition_geotemporal_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_geotemporal_review_gates.json').read_text())
    ...
    for name in ('competition_geotemporal_semantic_review.json','competition_geotemporal_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_geotemporal_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_geotemporal_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_geotemporal_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community causal planar regression pipeline with available context joins, spatial/lag features, expanding forward validation, ExtraTrees full refit and nonnegative same-time geographic smoothing. Synthetic evidence; no unseen-location or historical accuracy claim.',
        ...
        references=[]
        for reference,url,title in [('geotemporal-extratrees','https://scikit-learn.org/1.8/modules/generated/sklearn.ensemble.ExtraTreesRegressor.html','Scikit-learn ExtraTrees regression documentation')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Library provenance for ExtraTrees regression. Independently tested causal joins, fold-local target history, forward validation and geographic smoothing; synthetic evidence only.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_geotemporal_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_geotemporal_publication_gates.json':sha(gates_path)})
