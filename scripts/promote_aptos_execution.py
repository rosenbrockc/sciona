"""Atomically publish the source-compared APTOS realization at automated Tier 3.
...
from scripts.review_aptos_execution import SOURCE_VERSION, SOURCE_HASH, STAGE_MAPPING
ROOT = Path(__file__).resolve().parents[1]
LIBRARY_VERSIONS = dict(line.split('==', 1) for line in
    (ROOT / 'requirements/aptos-reference.txt').read_text().splitlines()
    if line and not line.startswith('#'))
RUNNER = 'aptos-execution-community.v1'
EXPECTED_GATES = {
    'semantic_review:proposed_tier', 'semantic_review:verdict',
    'semantic_review:review_source', 'graph_pipeline_validation:serialized_graph_sha256',
    'evidence_hash', 'installed_dependency',
}
from scripts.review_aptos_execution import LIMITATIONS as CORE_LIMITATIONS
LIMITATIONS = CORE_LIMITATIONS + json.loads(
    (ROOT / 'docs/reviews/competition_aptos_reference_scope_review.json').read_text())['conditions']
...
def review(root):
    from scripts.review_aptos_execution import final_review
    from importlib.metadata import version
    from sciona.atoms.ml import aptos_execution as provider
    expected = final_review()
    reviews = root / 'docs/reviews'
    semantic = json.loads((reviews / 'competition_aptos_semantic_review.json').read_text())
    ...
    if len(LIBRARY_VERSIONS) != 70 or any(version(name) != pin for name,pin in LIBRARY_VERSIONS.items()):
    ...
    full = json.loads((reviews / 'competition_aptos_graph_pipeline_validation.json').read_text())
    if full['graph_execution']['serialized_graph_sha256'] != digest or (len(nodes),len(edges)) != (2,1):
    ...
    if len(boundary['input']) != 15 or [p.name for p in boundary['output']] != ['result']:
        raise ValueError('Unexpected execution boundary')
    evidence = {str(p.relative_to(root)):sha(p) for p in [
        reviews/'competition_aptos_semantic_review.json',
        reviews/'competition_aptos_graph_pipeline_validation.json',
        root/'requirements/aptos-reference.txt']}
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_aptos_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates.get('format')!='aptos-publication-gates.v1'
            ...
            or gates['validator_sha256']!=sha(root/'scripts/validate_aptos_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'docs/reviews/competition_aptos_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community corrected APTOS CPU workflow: eight pretrained Inception/SE-ResNeXt regressors with trainable GeM, Smooth-L1, source-separated supplemental target refinement and ten additional epochs. Explicit reference choices; no historical accuracy claim.',
        ...
        references=[]
        for reference,url,title in [('aptos-winner-eight-model','https://www.kaggle.com/competitions/aptos2019-blindness-detection/writeups/guanshuo-xu-1st-place-solution-summary','APTOS first-place eight-model two-stage solution')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Winning eight-model two-stage solution provenance. Corrected CPU reconstruction with source-separated supplemental label roles and explicitly reviewed optimizer, augmentation, teacher and threshold choices; exact winner runtime, artifact identity and score equivalence unproven.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_aptos_graph_pipeline_validation.json').read_text())['sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'docs/reviews/competition_aptos_publication_gates.json':sha(gates_path)})
