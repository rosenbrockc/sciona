...
SOURCE_SHA = '68bb01686d6bcd10e876517623da1aed9cc9a8b49ac440748561ab8d3930e090'


def load_reference(reference_dir):
    path = reference_dir / 'Feng_models_KNNmorefeature.py'
...
def main():
    ...
    for seed in [9211, 9212, 9213, 9214, 9215]:
        for windows in [10, 12, 31]:
            ...
            prediction = rng.normal(size=(4, 12, 384)) + np.array([0, 3, 0, 3])[:, None, None]+11
            train[0, 0, 0] = np.nan
            ...
            actual = feng_expanded_knn_segment_probabilities(train, labels, prediction)
    ...
    report = dict(source_commit='00f937cc7710977dc812d9fc675864e2b8288658', source_sha256=SOURCE_SHA,
        provider_sha256=hashlib.sha256(Path(inspect.getsourcefile(feng_expanded_knn_segment_probabilities)).read_bytes()).hexdigest(),
        validation_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        test_sha256=hashlib.sha256((root/'tests/test_feng_expanded_knn.py').read_bytes()).hexdigest(),
