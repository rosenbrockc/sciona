...
def main():
    ...
    paths = sorted(str(p) for p in Path('sciona').glob('aptos_*.py')) + [str(pins_path), 'scripts/validate_aptos_pipeline.py']
    ...
    with TemporaryDirectory(prefix='aptos-validation-') as work:
        print('START complete APTOS eight-model two-stage reference', flush=True)
    ...
    report = {'passed': True, 'approved': False, 'catalog_mutations': 0, 'synthetic_only': True,
        ...
        'sha256': before}
    Path('docs/reviews/competition_aptos_pipeline_validation.json').write_text(json.dumps(report, indent=2) + '\n')
    print('PASS complete APTOS eight-model two-stage reference', flush=True)
