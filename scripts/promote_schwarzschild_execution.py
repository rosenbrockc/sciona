#!/usr/bin/env python3
"""Atomically intake and approve the reviewed radius realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.schwarzschild_execution import build_schwarzschild_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_schwarzschild_execution import validate

REFERENCE='openstax-university-physics-v1-13-7'
URL='https://openstax.org/books/university-physics-volume-1/pages/13-7-einsteins-theory-of-gravity'
LIMITATIONS=[
    'Automated Tier 3 only; no Tier 1 certification or Tier 2 usage claim.',
    'Length-scale formula; horizon interpretation requires caller-established spherical nonrotating uncharged Schwarzschild regime with asymptotically flat vacuum exterior.',
    'No black-hole classification, collapse simulation, rotating/charged horizon model or general-relativistic field-equation solution.',
    'Explicit positive SI mass and constants; positive subnormal outputs have reduced relative precision.',
    'Forward conditional source proof only; Newtonian light-motion assumptions are not certified.',
]


async def promote(root,symbol_file,rule_file,apply=False):
    report=await validate(root,symbol_file,rule_file)
    retained=json.loads((root/'docs/reviews/schwarzschild_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/schwarzschild_test_review.json').read_text())
    if tests['test_results']!=dict(tests=33,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    graph=build_schwarzschild_execution();digest,nodes,edges=encode_execution_graph(graph)
    if report['graph_digest']!=digest or report['full_runner_cases']!=5 or report['synthetic_numeric_cases']!=208 or report['maximum_ulp_error']>3:raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['schwarzschild_execution.json','schwarzschild_test_review.json','schwarzschild_semantic_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('schwarzschild-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'schwarzschild-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.schwarzschild_execution'
        ...
        description='Compute the Schwarzschild length scale 2GM/c^2 from explicit positive SI mass and constants. Automated Tier 3; physical interpretation subject to declared regime.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'radius'},dict(version_id=version_id,node_id='radius',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='schwarzschild-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['radius_metres']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory source proof provenance, not an invoked numerical dependency.'))})
        title='OpenStax University Physics Volume 1: Einstein’s Theory of Gravity'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Equation 13.12 and explicit limits of the Newtonian argument; automated Tier 3 review.'})
            ...
            evidence_id=uuid5(selected,'schwarzschild-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='schwarzschild-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,source_proof=report['source_proof'],limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'schwarzschild-positive-regime.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Positive finite SI mass and scalar constants; finite positive output. Horizon interpretation requires spherical nonrotating uncharged Schwarzschild regime with asymptotically flat vacuum exterior.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
        for selected in [spec.version_id,version_id]:
            if db.execute('SELECT count(*) AS n FROM artifact_io_specs WHERE version_id=%s',(selected,)).fetchone()['n']!=4:
                raise ValueError('Unexpected canonical ports')
        if db.execute('SELECT count(*) AS n FROM atom_io_specs WHERE version_id=%s',(spec.version_id,)).fetchone()['n']!=4:
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=3,top_level_output_arity=1,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
