#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected curl_curl realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.curl_curl_execution import build_curl_curl_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_curl_curl_execution import validate

REFERENCE='mit-18013a-curl-curl-vector-identity'
...
LIMITATIONS=[
    ...
    'C2 three-component fields on a common open region in fixed right-handed orthonormal Cartesian coordinates.',
    'General identity retains gradient of divergence; no Maxwell or divergence-free assumption.',
    'Original index contractions, vector parentheses and malformed ASTs explicitly reconstructed; no literal source parity claim.',
    'Caller establishes field units, smoothness and branch/pole domain; complex representations allowed.',
    'Protected AST strings and formal derivatives for composed functions; no curvilinear metric, distributional, boundary-value or throughput claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/curl_curl_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/curl_curl_test_review.json').read_text())
    ...
    if report['graph_digest']!=digest or report['full_runner_cases']!=6 or report['symbolic_derivative_checks']!=6 or not report['formal_derivatives_preserved'] or not report['nonzero_divergence_preserved'] or not report['cache_ast_strings_preserved'] or report['sympy_version']!='1.14.0':raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['curl_curl_execution.json','curl_curl_test_review.json','curl_curl_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('curl_curl-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'curl_curl-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.curl_curl_corrected_execution'
        ...
        description='Construct symbolic Cartesian curl-curl, gradient of divergence and component Laplacian for general C2 vector fields. Corrected Levi-Civita contraction verified exhaustively. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'curl_identity'},dict(version_id=version_id,node_id='curl_identity',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='curl_curl-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['curl_curl_srepr','gradient_divergence_srepr','laplacian_srepr']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; invalid index contraction and vector parentheses reconstructed as the general C2 Cartesian curl-curl identity. Original draft remains unchanged.'))})
        title='MIT 18.013A: Vector Identity Used in the Wave Equation'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Uses the Cartesian curl-curl vector identity; the reviewed realization independently verifies its Levi-Civita contraction and retains the general divergence term.'})
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=2,top_level_output_arity=3,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
