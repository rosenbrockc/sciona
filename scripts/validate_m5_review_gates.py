"""Reject weakened M5 evidence without changing evidence files or catalog."""
import hashlib
import json
from pathlib import Path
from unittest.mock import patch
import review_m5_execution as review
...
def main():
    ...
    def failure(suffix,name,mutate):
        target=f'competition_m5_{suffix}.json'
    ...
    failure('execution_source_review','wrong_blend',lambda d:d['stage_mapping'].__setitem__('magic_multiplier','apply constant multiplier'))
    failure('lag_comparison','missing_lag_comparison',lambda d:d['checks'].__setitem__('source_function_comparisons',0))
    failure('component_tests','missing_tests',lambda d:d['checks'].__setitem__('tests_passed',0))
    failure('full_inventory','reduced_training_limits',lambda d:d['checks'].__setitem__('configured_rounds_per_model',1))
    ...
    paths=['scripts/review_m5_execution.py','scripts/validate_m5_review_gates.py']
    report=dict(format='m5-review-negative-gates.v1',result='passed',catalog_mutations=0,
    ...
    (review.ROOT/'docs/reviews/competition_m5_review_gates.json').write_text(json.dumps(report,indent=2)+'\n')
