"""Validate full hyperbolic collection and exponential certificate through serialized full runner."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.atoms.physics import hyperbolic_identities as provider
from sciona.physics_ingest.hyperbolic_identities_execution import SOURCE_VERSION,build_hyperbolic_identities_execution
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg
from scripts.validate_hyperbolic_identities_source import validate as validate_source
...
async def validate(root,symbol_file,rule_file,expression_file):
    ...
    names=['functions_json','identities_json','certificate_json']
    with tempfile.TemporaryDirectory(prefix='sciona-hyperbolic-identities-') as directory:
        prior=runner.RUNS_DIR;runner.RUNS_DIR=Path(directory)
        try:
            for i,angle in enumerate(cases):
                run='case-'+str(i)
                result=await runner.CDGExecutionSession(None,'synthetic-hyperbolic-identities',run).execute({'argument_srepr':sp.srepr(angle)},cdg=restored)
                if result['status']!='completed':raise ValueError('Full runner failed')
                outputs=[]
                for name in names:
                    record=json.loads((Path(directory)/run/'identities'/('out_'+name+'.json')).read_text())
                    if record['type']!='str':raise ValueError('String transport lost')
                    outputs.append(record['value'])
                from tests.physics_ingest.test_hyperbolic_identities import check_outputs
                check_outputs(angle, outputs)
                functions=json.loads(outputs[0])
                if i>=4 and any(not provider._checked_parse(v).has(u) for v in functions.values()):
                    raise ValueError('General real argument scope lost')
                cache=runner.load_cached_outputs(Path(directory)/run,'identities')
                if any(cache['out_'+n]!=s for n,s in zip(names,outputs)):raise ValueError('Cache changed output')
        finally:runner.RUNS_DIR=prior
    files=['sciona/physics_ingest/hyperbolic_identities_execution.py','scripts/validate_hyperbolic_identities_execution.py',
           'tests/physics_ingest/test_hyperbolic_identities.py','tests/physics_ingest/test_hyperbolic_identities_proof.py',
