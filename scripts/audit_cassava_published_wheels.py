...
def main():
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        packages = dict(pool.map(inspect, audit['packages'].items()))
    unresolved = [name for name, details in packages.items() if 'selected_wheel' not in details]
    report = {'approved': False, 'all_pins_have_compatible_unyanked_wheels': not unresolved,
        ...
        'sha256': {p: hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in [
            str(audit_path), 'scripts/audit_cassava_published_wheels.py']}}
    Path('docs/reviews/competition_cassava_published_wheel_audit.json').write_text(json.dumps(report, indent=2) + '\n')
