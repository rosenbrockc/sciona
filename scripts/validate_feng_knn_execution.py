...
from validate_feng_knn import load_reference as knn_reference, SOURCE_SHA as KNN_SHA
...
async def validate(directory):
    ...
    for seed in [9321, 9322, 9323]:
        ...
        expected = np.array([v for _, v in knn['predict'](None, knn['train'](None, None), None)])
        ...
        with TemporaryDirectory(prefix='sciona-feng-knn-') as temporary:
            ...
            try:
                ...
                actual = np.load(Path(temporary)/'check/classifier/out_feng_knn_segment_probabilities.npy')
    ...
    modules = ['sciona.feng_knn_execution', 'sciona.visualizer.runner', 'sciona.services.execution_graph_codec',
    ...
    return dict(synthetic_only=True, full_runner_cases=len(cases), cases=cases, maximum_absolute_error=0.,
        ...
        source_hashes={'Feng_preprocessors_filtering.py': FILTER_SHA, 'Feng_preprocessors_fft.py': FFT_SHA, 'Feng_models_KNN.py': KNN_SHA},
        ...
        validation_script_hashes={name: hashlib.sha256((Path(__file__).parent/name).read_bytes()).hexdigest() for name in ['validate_feng_knn_execution.py', 'validate_feng_filter.py', 'validate_feng_fft.py', 'validate_feng_knn.py']},
        library_versions={name: importlib.metadata.version(name) for name in ['numpy', 'scipy', 'pandas', 'scikit-learn']},
        limitations='One source model for one caller-selected population. Source float32 loading and prediction-batch scaling preserved. Python 2 FFT shape division adapted. Current numerical libraries, synthetic signals only; no historical-engine, clinical or competition-performance claim. Full ensemble remains incomplete.')
