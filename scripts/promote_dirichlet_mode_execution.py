#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected dirichlet_mode realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.dirichlet_mode_execution import build_dirichlet_mode_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_dirichlet_mode_execution import validate

REFERENCE='openstax-normalized-dirichlet-mode'
...
LIMITATIONS=[
    ...
    'Finite positive interval width, positive integer mode index and finite real constant phase.',
    'Fixed real x in [0,width]; parameters independent of x and consistent symbol assumptions.',
    'General symbolic parameters retained; parsed floats become exact binary-value rationals.',
    'Unit L2 normalized wave, opposite sign, wavenumber, two spatial derivatives and checkable certificate.',
    'Complex conjugate normalization fixes amplitude magnitude; source real plus/minus signs are phase choices.',
    'Zero and negative eigenvalue branches cannot satisfy these normalized Dirichlet conditions.',
    'Source ASTs, definite integration, endpoint evaluation and swapped sign bindings reconstructed; no literal parity.',
    'Interior differential equation with endpoint values; no exterior derivative matching or distributional extension.',
    'No finite-well, quantum-energy conversion, arbitrary boundary or expansion-completeness claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/dirichlet_mode_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/dirichlet_mode_test_review.json').read_text())
    ...
    graph=build_dirichlet_mode_execution();digest,nodes,edges=encode_execution_graph(graph)
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['dirichlet_mode_execution.json','dirichlet_mode_test_review.json','dirichlet_mode_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('dirichlet_mode-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'dirichlet_mode-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.dirichlet_mode_corrected_execution'
        ...
        description='Construct normalized Dirichlet interval modes with both global-sign representatives, wavenumber, spatial derivatives and boundary/normalization certificate. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'mode'},dict(version_id=version_id,node_id='mode',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='dirichlet_mode-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['wave_srepr','opposite_wave_srepr','wavenumber_srepr','derivative_srepr','second_derivative_srepr','certificate_json']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; derivatives, definite interval normalization, endpoint evaluation and paired sign substitutions restored. Positive integer spectrum and complex phase domain explicit. Original draft unchanged.'))})
        title='OpenStax University Physics Volume3: The Quantum Particle in a Box'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Finite interval Dirichlet boundaries, positive integer mode indices and normalized sine spatial modes.'})
            ...
            evidence_id=uuid5(selected,'dirichlet_mode-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='dirichlet_mode-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'normalized-dirichlet-interval-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Positive finite interval width, positive integer mode index, finite real phase independent of fixed real x. Unit L2 norm and zero endpoint values on [0,width], differential equation in the interior. No external extension or quantum-energy conversion.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
        for selected in [spec.version_id,version_id]:
            if db.execute('SELECT count(*) AS n FROM artifact_io_specs WHERE version_id=%s',(selected,)).fetchone()['n']!=9:
                raise ValueError('Unexpected canonical ports')
        if db.execute('SELECT count(*) AS n FROM atom_io_specs WHERE version_id=%s',(spec.version_id,)).fetchone()['n']!=9:
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=3,top_level_output_arity=6,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
