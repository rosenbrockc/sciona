...
def main():
    ...
    with TemporaryDirectory(prefix='sciona-resample-audit-') as temporary:
        ...
        for target in [128, 256]:
            ...
            for count in [4001, 236801]:
                ...
                expected = andriy_documented_resample(x.T, 400, target).T
    ...
    report = dict(documentation='https://www.mathworks.com/help/signal/ref/resample.html',
        ...
        provider_sha256=hashlib.sha256(Path(inspect.getsourcefile(andriy_documented_resample)).read_bytes()).hexdigest(),
        test_sha256=hashlib.sha256((Path(__file__).resolve().parents[1]/'tests/test_andriy_resampling.py').read_bytes()).hexdigest(),
        ...
        conclusions=['Octave default resample is not an interchangeable MATLAB-default reference.',
                     ...
                     'Provider cross-language execution validated with explicit coefficients; no full graph approval implied.'])
