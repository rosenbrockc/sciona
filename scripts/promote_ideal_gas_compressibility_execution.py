#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected ideal_gas_compressibility realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.ideal_gas_compressibility_execution import build_ideal_gas_compressibility_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_ideal_gas_compressibility_execution import validate

REFERENCE='openstax-ideal-gas-compressibility-community'
URL='https://openstax.org/books/college-physics-2e/pages/13-3-the-ideal-gas-law'
LIMITATIONS=[
    'Automated Tier 3 only; no human-reviewed Tier 1 or Tier 2 usage claim.',
    'Local isothermal volume compressibility kappa_T=1/P, not isentropic compressibility or bulk modulus.',
    'Caller establishes ideal-gas PV=nRT at fixed positive temperature and amount in moles; positive absolute pressure in pascals.',
    'No real-gas, phase-transition, finite-change, low-temperature applicability or throughput claim.',
    'Source derivative placeholders reconstructed; dimensionless n corrected to molar amount; dimensionless kappa corrected to inverse pressure.',
    'Nonempty finite positive real arrays including scalars, float64 conversion, shape preserved; exact reciprocal rounded once.',
    'Subnormal outputs supported; reciprocal overflow rejected.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/ideal_gas_compressibility_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/ideal_gas_compressibility_test_review.json').read_text())
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['ideal_gas_compressibility_execution.json','ideal_gas_compressibility_test_review.json','ideal_gas_compressibility_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('ideal_gas_compressibility-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'ideal_gas_compressibility-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.ideal_gas_compressibility_corrected_execution'
        ...
        description='Evaluate ideal-gas isothermal volume compressibility from absolute pressure. Reviewed six-step derivative proof, molar amount and inverse-pressure dimensions. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'compressibility'},dict(version_id=version_id,node_id='compressibility',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='ideal_gas_compressibility-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['isothermal_compressibility']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; fixed-temperature derivative, amount and inverse-pressure dimensions explicitly reconstructed. Original draft unchanged.'))})
        title='OpenStax College Physics 2e: The Ideal Gas Law'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Ideal gas law PV=nRT with absolute temperature and amount in moles; reviewed isothermal compressibility follows by differentiating volume with respect to pressure at fixed temperature and amount.'})
            ...
            evidence_id=uuid5(selected,'ideal_gas_compressibility-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='ideal_gas_compressibility-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'ideal-gas-compressibility-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Ideal gas at fixed positive temperature and amount in moles. Nonempty finite positive absolute pressure in pascals; output local isothermal volume compressibility in inverse pascals with matching shape. Reciprocal must be representable.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
        for selected in [spec.version_id,version_id]:
            if db.execute('SELECT count(*) AS n FROM artifact_io_specs WHERE version_id=%s',(selected,)).fetchone()['n']!=2:
                raise ValueError('Unexpected canonical ports')
        if db.execute('SELECT count(*) AS n FROM atom_io_specs WHERE version_id=%s',(spec.version_id,)).fetchone()['n']!=2:
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=1,top_level_output_arity=1,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
