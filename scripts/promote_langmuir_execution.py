#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected langmuir realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.langmuir_execution import build_langmuir_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_langmuir_execution import validate

REFERENCE='iupac-single-adsorptive-isotherm'
URL='https://media.iupac.org/reports/2001/colloid_2001/manual_of_s_and_t/node21.html'
LIMITATIONS=[
    'Automated Tier3; no human-reviewed Tier1 designation.',
    'Fixed-temperature single non-dissociative adsorbate, identical independent monolayer sites, equilibrium.',
    'Positive adsorption/desorption coefficients and site density; nonnegative partial pressure; no broadcasting.',
    'No lateral interactions, multilayers, competitive adsorption, transient kinetics or material-fit claim.',
    'Zero-pressure endpoint independently verified without reciprocal source steps; K=ka/kd has inverse-pressure units.',
    'Independent exact-rational output rounding preserves vacancy near saturation; nonzero underflow/overflow rejected.',
    'Equilibrium rate is common adsorption/desorption event rate, not net accumulation; source missing divisor corrected.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/langmuir_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/langmuir_test_review.json').read_text())
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['langmuir_execution.json','langmuir_test_review.json','langmuir_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('langmuir-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'langmuir-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.langmuir_corrected_execution'
        ...
        description='Compute Langmuir coverage, occupied and vacant site densities and common equilibrium event rate from kinetic coefficients. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'equilibrium'},dict(version_id=version_id,node_id='equilibrium',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='langmuir-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['coverage','occupied_sites','vacant_sites','equilibrium_rate']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; missing desorption divisor corrected and zero-pressure boundary independently validated. Original draft unchanged.'))})
        title='IUPAC: Adsorption at the solid/gas interface'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Single-adsorptive isotherms relate adsorbed amount to equilibrium pressure at constant temperature; model-specific Langmuir kinetics separately checked in retained proof.'})
