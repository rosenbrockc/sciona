#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected integration_parts realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from scripts.validate_integration_parts_execution import validate

REFERENCE='openstax-calculus-v2-integration-by-parts'
...
LIMITATIONS=[
    ...
    'Symbolic synthesis transformation, not numerical integration or guaranteed closed-form antiderivative evaluation.',
    'Scalar commutative u and v must be C1 on one common connected open real interval; the caller establishes domain, poles and branches.',
    'Real independent variable with consistent symbol assumptions; scalar constant must have no free dependence on it.',
    'Safe AST grammar only; nonfinite expressions, noncommutative inputs and executable Python syntax rejected.',
    'Residual integral and formal derivatives retained in output ASTs; differentiation checked before serialization.',
    'Source differential placeholders reconstructed using a real parameter and explicit integration constant; no literal source-AST parity claim.',
    'Requires reviewed SymPy 1.14.0 and parser implementation; no quadrature, convergence discovery or throughput claim.',
]
...
async def promote(root,symbol_file,rule_file,apply=False):
    ...
    tests=json.loads((root/'docs/reviews/integration_parts_test_review.json').read_text())
    if tests['test_results']!=dict(tests=40,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    if report['graph_digest']!=digest or report['full_runner_cases']!=6 or report['symbolic_derivative_checks']!=6 or not report['residual_integrals_preserved'] or not report['cache_ast_strings_preserved'] or report['sympy_version']!='1.14.0':raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['integration_parts_execution.json','integration_parts_test_review.json','integration_parts_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('integration_parts-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'integration_parts-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.integration_parts_corrected_execution'
        ...
        description='Symbolically rewrite an indefinite integral by parts, retaining its residual integral and independent constant. Safe AST strings, derivative-verified output and explicit C1 domain. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'parts'},dict(version_id=version_id,node_id='parts',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='integration_parts-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['integrand_srepr','antiderivative_srepr']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; incomplete differentials explicitly reconstructed through a C1 real parameter and integration constant. Original draft remains unchanged.'))})
        title='OpenStax Calculus Volume 2: Integration by Parts'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Integration-by-parts transformation derived from the product rule; residual integral and arbitrary constant retained.'})
            ...
            evidence_id=uuid5(selected,'integration_parts-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='integration_parts-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'integration-parts-c1-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Scalar commutative C1 functions of a real parameter on a common connected open interval, with independent integration constant and finite expressions in the reviewed AST grammar. Caller establishes differentiability and branch/pole exclusions. Symbolic residual integral, not quadrature.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=4,top_level_output_arity=2,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
