...
def validate(root,source,library,notebook):
    ...
    training_model=original((32,32),True,True,load_weights=False)
    ...
    reference=original((32,32),False,False,load_weights=False).eval()
    ...
    nodes=[definitions[name] for name in ['my_collate_fn','get_pred_mask']]
    constants=[]
    ...
    try:
        for h,w,res,inp,pad,batch,bias,tta in [(35,37,32,32,8,4,1.,4),(17,19,1024,320,256,12,1.,3),(35,37,32,32,8,4,0.,3)]:
            with torch.no_grad():
            ...
            # Reuse only verified library/raster dependency bindings. Execute
            # final notebook dataset and transform definitions themselves.
            namespace=dict(ds.__class__.__init__.__globals__)
            namespace['INPUT_PATH']='synthetic'
            selected=constants+[definitions[name] for name in ['get_transforms_test','HuBMAPDataset']]
            exec(compile(ast.Module(body=selected,type_ignores=[]),'<final-notebook-input>','exec'),namespace)
            import pandas as pd
            ds=namespace['HuBMAPDataset'](0,pd.DataFrame(dict(id=['synthetic'])))
            ...
            try:
                actual=predict_slide([candidate],image,python_rng=pr,torch_generator=tg,resolution=res,
                                     input_resolution=inp,pad_size=pad,batch_size=batch,tta=tta,threshold=.5)
                random.seed(317)
                ...
                ns=dict(np=np,cv2=cv2,torch=torch,DataLoader=loader,HuBMAPDataset=lambda *args:ds,
                        ...
                        config=dict(test_batch_size=batch,tta=tta,mask_threshold=.5))
                ...
                cases.append(dict(resolution=res,input_resolution=inp,tiles=len(ds),batch_size=batch,
                                  classifier_logit=bias,tta=tta,model_calls=len(logits[0]),exact_mask_logits_rng=True))
            finally:
    ...
    paths=['sciona/hubmap_raw_inference.py','sciona/hubmap_inference.py','sciona/hubmap_inference_tiles.py',
           ...
           'scripts/validate_hubmap_final_inference.py','scripts/validate_hubmap_final_network.py','scripts/validate_hubmap_network.py',
           ...
           'docs/reviews/competition_hubmap_augmentation_pins.json','docs/reviews/competition_hubmap_final_notebook_pins.json']
    return dict(approved=False,synthetic_only=True,exact_training_checkpoint_to_inference_state=True,cases=cases,
                ...
                limitations=['Decoded synthetic RGB arrays and synthetic full-network checkpoint; no learned accuracy claim.',
                             ...
                             'Full inference-to-retraining lifecycle and full publication gates remain.'])
...
if __name__=='__main__':
    ...
    (root/'docs/reviews/competition_hubmap_final_inference.json').write_text(json.dumps(report,indent=2)+'\n')
