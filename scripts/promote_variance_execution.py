#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected variance realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from scripts.validate_variance_execution import validate

REFERENCE='openstax-statistics-expected-value-variance'
URL='https://openstax.org/books/statistics/pages/4-2-mean-or-expected-value-and-standard-deviation'
LIMITATIONS=[
    'Automated Tier 3 only; no human-review Tier 1 or Tier 2 usage claim.',
    'Normalized finite real distribution with nonnegative weights and positive total mass; dimensionless source variable x.',
    'Finite real identical nonempty (...,N) arrays, N>0, no broadcasting; inputs convert to float64 before algebra.',
    'Population variance, not unbiased sample variance, standard deviation, signed measure or continuum quadrature accuracy claim.',
    'Source intermediate -2 E[x^2] explicitly corrected to -2 E[x]^2; source AST placeholders are not approved.',
    'Exact rational normalized mean and centered second moment before independent float64 rounding; variance uses exact mean.',
    'Exact zeros/subnormals supported; nonzero underflow to zero and overflow rejected. No throughput claim.',
    'Generic runner statistics may overflow for extreme magnitudes; complete numerical outputs are independently checked.',
]
...
async def promote(root,symbol_file,rule_file,apply=False):
    ...
    tests=json.loads((root/'docs/reviews/variance_test_review.json').read_text())
    ...
    if report['graph_digest']!=digest or report['full_runner_cases']!=6 or report['synthetic_distributions']!=11 or report['maximum_ulp_error']!=0:raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['variance_execution.json','variance_test_review.json','variance_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('variance-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'variance-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.variance_corrected_execution'
        ...
        description='Compute normalized finite-distribution mean and population variance using exact converted-input arithmetic. Corrected source expectation identity. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'variance'},dict(version_id=version_id,node_id='variance',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='variance-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['mean','population_variance']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; faulty moment intermediate explicitly reconstructed under normalized linear expectation. Original draft remains unchanged.'))})
        title='OpenStax Statistics: Mean or Expected Value and Standard Deviation'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Normalized expectation and population variance of a discrete random variable.'})
            ...
            evidence_id=uuid5(selected,'variance-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='variance-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'variance-finite-distribution-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Finite real dimensionless values and nonnegative weights of identical (...,N) shape, with positive total weight per batch. Normalized population moments with no signed weights or sample-bias correction. Representable finite outputs; nonzero underflow to zero rejected.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
