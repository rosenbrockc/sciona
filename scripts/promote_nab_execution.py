"""Atomically publish the source-compared NAB realization at automated Tier 3.
...
COMMITS = {'benchmark': 'ea702d75cc2258d9d7dd35ca8e5e2539d71f3140'}
RUNNER = 'nab-execution-community.v1'
from scripts.review_nab_execution import LIMITATIONS
...
def review(root):
    from scripts.review_nab_execution import audit
    import sciona.atoms.ml.nab_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_nab_semantic_review.json').read_text())
    ...
    for name in ('competition_nab_semantic_review.json','competition_nab_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_nab_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_nab_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_nab_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community generic causal window isolation-forest pipeline with explicit fitting history, prior-score rolling three-sigma threshold and corrected NAB scoring/normalization. Synthetic causality and source-scoring evidence; no historical winning detector or leaderboard parity claim',
        ...
        references=[]
        reference='nab-benchmark-'+COMMITS['benchmark'][:12]
        url='https://github.com/numenta/NAB/tree/'+COMMITS['benchmark']
        title='Pinned Numenta Anomaly Benchmark scoring software (MIT)'
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Pinned NAB benchmark scoring software with MIT notice; independent generic causal forest pipeline and corrected scoring, without historical winning-detector equivalence.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_nab_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_nab_publication_gates.json':sha(gates_path)})
