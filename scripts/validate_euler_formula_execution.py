...
from sciona.atoms.physics import euler_formula as provider
from sciona.physics_ingest.euler_formula_execution import SOURCE_VERSION,build_euler_formula_execution
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg
from scripts.validate_euler_formula_source import validate as validate_source
...
async def validate(root,symbol_file,rule_file,expression_file):
    ...
    names=['exponential_srepr','trigonometric_srepr','certificate_json']
    with tempfile.TemporaryDirectory(prefix='sciona-euler-formula-') as directory:
        ...
        try:
            for i,angle in enumerate(cases):
                ...
                if sp.simplify(lhs-sp.exp(sp.I*angle))!=0 or sp.simplify(rhs-sp.cos(angle)-sp.I*sp.sin(angle))!=0:
                ...
                if c['initial_value']!=1 or c['ode_residual']!='Integer(0)' or c['integrating_factor_derivative']!='Integer(0)' or c['uses_complex_log']:
                    raise ValueError('ODE certificate differs')
                # Independent re/im ODE system and normalization, without assuming
                # exp(ix)=cosx+i sinx in the generic derivative argument.
                x=sp.Symbol('x',real=True)
                if sp.diff(sp.cos(x),x)!=-sp.sin(x) or sp.diff(sp.sin(x),x)!=sp.cos(x):raise ValueError('Real system failed')
                if sp.cos(0)!=1 or sp.sin(0)!=0:raise ValueError('Initial conditions failed')
    ...
    files=['sciona/physics_ingest/euler_formula_execution.py','scripts/validate_euler_formula_execution.py',
           'tests/physics_ingest/test_euler_formula.py','tests/physics_ingest/test_euler_formula_proof.py',
