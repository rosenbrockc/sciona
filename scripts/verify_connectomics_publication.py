"""Read back served connectomics versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_connectomics_graph_execution as comparison


def verify(root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_connectomics_graph_execution.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('e9f83a4e-4cb1-5297-8c04-8fd65c6e7d33',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='connectomics-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='connectomics-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='c37cf5cd41c4c7f7f19af3f2f48ab3a69d94bd451822a830088abe2c632774ed', dependency_role='cdg', optional=False)]
    ...
    evidence_path=root/'docs/reviews/competition_connectomics_graph_execution.json'
    ...
    directory=os.environ.get('SCIONA_CONNECTOMICS_SOURCE_DIR','/private/tmp/sciona_connectomics_source')
    ns=source_namespace(Path(directory));ns['print']=lambda *a,**kw:None
    directivity=comparison.reference_directivity(x)
    cases=[]
    for mode in ('simple','tuned'):
        base=ns['make_'+mode+'_inference'](x)
        for directed in (False,True):
            payload=dict(version=1,signals=x.tolist(),mode=mode,directivity=directed)
            captured={}
            def capture(directory,node,name,value):
                if node=='execute' and name=='out_result':captured['result']=value
            with tempfile.TemporaryDirectory(prefix='sciona_connectomics_served_synthetic_') as temp:
                with patch.dict(os.environ,{'SCIONA_CONNECTOMICS_SOURCE_DIR':directory}), patch.object(comparison.runner,'RUNS_DIR',Path(temp)), patch.object(comparison.runner,'save_intermediate_value',side_effect=capture):
                    status=asyncio.run(comparison.runner.CDGExecutionSession(None,'synthetic-connectomics-served','case').execute({'payload':payload},cdg=graph))
            assert status['status']=='completed'
            result=json.loads(json.dumps(captured['result'],allow_nan=False))
            target=.997*base+.003*directivity if directed else base
            np.testing.assert_array_equal(result['scores'],target)
            assert result['pca_fits']==(240 if mode=='simple' else 480)
            cases.append(dict(mode=mode,directivity=directed,pca_fits=result['pca_fits'],exact_source_match=True))
    assert evidence_path.read_bytes()==before
    from scripts.promote_connectomics_execution import review
    review(root)
    result=expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=2,
        served_cdg_count=served, catalog_graph_runtime_comparison='passed', serialized_graph_cases=4, cases=cases,
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_connectomics_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
