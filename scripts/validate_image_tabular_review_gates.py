"""Reject weakened Image tabular evidence without changing files or catalog."""
import hashlib
import json
from pathlib import Path
from unittest.mock import patch
import review_image_tabular_execution as review
...
def main():
    ...
    def failure(suffix,name,mutate):
        target=f'competition_image_tabular_{suffix}.json'
    ...
    failure('runtime_tests','calibration_leakage',lambda d:d['checks'].__setitem__('heldout_feature_label_isolation',False))
    failure('runtime_tests','missing_oof_coverage',lambda d:d['checks'].__setitem__('complete_oof_coverage',False))
    failure('runtime_tests','query_population_leakage',lambda d:d['checks'].__setitem__('calibration_labels_do_not_change_scores',False))
    ...
    paths=['scripts/review_image_tabular_execution.py','scripts/validate_image_tabular_review_gates.py']
    report=dict(format='image_tabular-review-negative-gates.v1',result='passed',
    ...
    (review.ROOT/'docs/reviews/competition_image_tabular_review_gates.json').write_text(json.dumps(report,indent=2)+'\n')
