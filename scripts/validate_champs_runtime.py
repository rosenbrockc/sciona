...
def main():
    ...
    for name in catalog["names"]:
        ...
        variant = folder.split('/')[-1]
        assert runtime.model_config(variant) == config
        torch_state = torch.random.get_rng_state().clone()
        numpy_state = np.random.get_state()
        model = runtime.create_model(variant,seed=1729)
        ...
        del model, prediction, loss, grads, _, abs_error, type_error, expected, log_loss
        gc.collect()
    assert len(results) == 13
    args.output.write_text(json.dumps({"source_commit": pins["commit"], "results": results,
        "raw_preprocessing": packing,
        "runtime_sha256": hashlib.sha256((repo / "sciona/champs_source_runtime.py").read_bytes()).hexdigest(),
