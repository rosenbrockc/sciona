#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected ideal_gas_expansion realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.ideal_gas_expansion_execution import build_ideal_gas_expansion_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_ideal_gas_expansion_execution import validate

REFERENCE='openstax-ideal-gas-expansion-community'
...
LIMITATIONS=[
    ...
    'Volumetric isobaric expansion coefficient alpha=1/T, not linear expansion.',
    'Caller establishes ideal-gas PV=nRT with fixed positive pressure and amount in moles, positive absolute temperature in kelvin.',
    'No real-gas, phase-transition, low-temperature applicability, finite-change or throughput claim.',
    'Original derivative ASTs reconstructed; missing intermediate denominator restored; dimensionless source n corrected to molar amount dimension.',
    'Nonempty finite positive real arrays including scalars, float64 conversion, shape preserved; one correctly rounded reciprocal.',
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/ideal_gas_expansion_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/ideal_gas_expansion_test_review.json').read_text())
    ...
    graph=build_ideal_gas_expansion_execution();digest,nodes,edges=encode_execution_graph(graph)
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['ideal_gas_expansion_execution.json','ideal_gas_expansion_test_review.json','ideal_gas_expansion_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('ideal_gas_expansion-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'ideal_gas_expansion-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.ideal_gas_expansion_corrected_execution'
        ...
        description='Evaluate the ideal-gas volumetric isobaric expansion coefficient from absolute temperature. Reviewed five-step derivation, molar amount dimensions and restored intermediate denominator. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'expansion'},dict(version_id=version_id,node_id='expansion',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='ideal_gas_expansion-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['volumetric_expansion']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; derivative semantics, amount dimensions and missing denominator explicitly reconstructed. Original draft unchanged.'))})
