"""Read back served Flavours versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_flavours_graph as comparison
from scripts import flavours_graph_execution as graph_execution


def verify(root):
    graph_execution.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_flavours_graph_validation.json').read_text())
    semantic = json.loads((root / 'docs/reviews/competition_flavours_semantic_review.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('6256d5b1-d742-53b1-9ff4-c2c7265ce981',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='flavours-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        def check_review(identity, legacy=False):
            ...
            for table, key in references:
                ...
                assert rows == [dict(ref_id='flavours-winner-go-polar-bears',
                    url='https://github.com/aguschin/flavours-of-physics/tree/e642aafa4b2ddf8baa572cbdf2926450935ed005',
                    verified=True)]
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='flavours-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='1f7003883cd8067a1fcaa170601336d99e2911e2024a0590eb41425757dc0539', dependency_role='cdg', optional=False)]
    ...
    evidence_path = root / 'docs/reviews/competition_flavours_graph_validation.json'
    ...
    assert report['passed'] and report['graph_execution']['actual_runner_nodes'] == 1
    assert report['graph_execution']['serialized_graph_sha256'] == record['content_hash']
    assert report['model_fits'] == 115 and report['tree_boosting_rounds'] == 111458 and report['neural_optimizer_updates'] == 600000
    assert report['neural_checkpoint_replays_exact']
    assert evidence_path.read_bytes() == before
    from scripts.promote_flavours_execution import review
    review(root)
    return dict(read_only=True, synthetic_only=True, trust_tier=3,
        provider_versions_verified=1, served_cdg_count=served,
        catalog_graph_runtime_execution='passed', full_training_and_inference_budgets=True,
        model_fits=115, tree_boosting_rounds=111458, neural_optimizer_updates=600000, graph_sha256=record['content_hash'],
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_flavours_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
