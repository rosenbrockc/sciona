"""Atomically publish the source-compared Connectomics realization at automated Tier 3.
...
COMMITS = {'winning_solver': 'b3abb178b9a9d558fdd5a7de49ae50425576a011', 'historical_pca': '8d04380d474723467b5a717328efd0c9fc5bd898'}
RUNNER = 'connectomics-execution-community.v1'
from scripts.review_connectomics_execution import LIMITATIONS
...
def review(root):
    from scripts.review_connectomics_execution import audit
    import sciona.atoms.ml.connectomics_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_connectomics_semantic_review.json').read_text())
    ...
    graph=build_connectomics_graph()
    digest,nodes,edges=encode_execution_graph(graph)
    full=json.loads((reviews/'competition_connectomics_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_connectomics_review_gates.json').read_text())
    ...
    for name in ('competition_connectomics_semantic_review.json','competition_connectomics_review_gates.json'):
...
def promote(root, apply=False):
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community Connectomics corrected complete simple/tuned threshold-grid precision ensemble with historical PCA and optional temporal precedence. Automated synthetic evidence; no historical accuracy or original buggy-output claim.',
        ...
        references=[]
        reference='connectomics-winning-solver-'+COMMITS['winning_solver'][:12]
        url='https://github.com/asutera/kaggle-connectomics/tree/'+COMMITS['winning_solver']
        title='Pinned Connectomics first-place source (per-file BSD 3 clause notices)'
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Pinned primary Connectomics execution software with explicit threshold/filter corrections and retained BSD notices; historical PCA pinned separately in evidence.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_connectomics_graph_execution.json').read_text())['code_sha256'],
