...
def audit():
    ...
    provider_path = directory / 'src/sciona/atoms/ml/aptos_execution.py'
    ...
    if symbolic['result'] != {'kind': 'Aptos.ExecutionResult'}:
    ...
    return {'approved': False, 'catalog_mutations': 0, 'passed': True,
            ...
            'sha256': {name: sha(ROOT / name) for name in [
                'scripts/build_aptos_execution_graph.py',
                'scripts/audit_aptos_provider_contract.py']}}


if __name__ == '__main__':
    report = audit()
    (ROOT / 'docs/reviews/competition_aptos_provider_structure.json').write_text(
