#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected hermitian realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.hermitian_execution import build_hermitian_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_hermitian_execution import validate

REFERENCE='mit-805-2013-adjoint-hermitian-expectation'
URL='https://ocw.mit.edu/courses/8-05-quantum-physics-ii-fall-2013/4de6d044fa9d7e5b8998c5f8ca984a42_MIT8_05F13_Chap_04.pdf'
LIMITATIONS=[
    'Automated Tier 3 only; no Tier 1 human certification or Tier 2 usage claim.',
    'Finite-dimensional pure states and exactly Hermitian operators in one orthonormal basis; no infinite-dimensional domain or measurement-sampling claim.',
    'Finite numeric (...,N,N) operator and (...,N) state, identical nonempty batch shape; nonzero state in every member.',
    'Inputs convert to complex128 before exact Hermitian validation and rational algebra; no broadcasting or silent symmetrization.',
    'Explicit state-norm division extends the normalized source expectation to arbitrary nonzero finite state scale.',
    'Incomplete source symbolic sides are reconstructed from six reviewed source equations and four verified adjoint steps; no literal source-AST parity.',
    'Exact converted-input quadratic form and positive norm before single float64 rounding; nonzero underflow to zero and overflow rejected.',
    'Generic runner summary statistics may discard imaginary parts or overflow for extreme inputs; full complex arrays and validated numerical outputs are preserved.',
]
...
async def promote(root,symbol_file,rule_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/hermitian_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/hermitian_test_review.json').read_text())
    if tests['test_results']!=dict(tests=38,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    graph=build_hermitian_execution();digest,nodes,edges=encode_execution_graph(graph)
    if report['graph_digest']!=digest or report['full_runner_cases']!=5 or report['synthetic_states']!=12 or report['maximum_ulp_error']!=0:raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['hermitian_execution.json','hermitian_test_review.json','hermitian_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('hermitian-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'hermitian-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.hermitian_corrected_execution'
        ...
        description='Compute normalized finite-dimensional Hermitian expectation from an operator and nonzero pure state. Reconstructed adjoint proof and exact converted-input rational arithmetic. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'expectation'},dict(version_id=version_id,node_id='expectation',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='hermitian-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['expectation']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; incomplete symbolic sides reconstructed from reviewed adjoint equations. Original draft not invoked or rewritten.'))})
        title='MIT Quantum Physics II: Dirac bra-ket notation, adjoints and Hermitian operators'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Adjoint algebra and real expectation of Hermitian operators; finite-dimensional reconstruction with explicit state normalization.'})
            ...
            evidence_id=uuid5(selected,'hermitian-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='hermitian-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'hermitian-finite-matrix-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Finite-dimensional exactly Hermitian operator and nonzero pure state in a common orthonormal basis. Matching batch and basis shapes. Finite complex128 converted inputs and finite float64 normalized expectation; no nonzero underflow to zero.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
        for selected in [spec.version_id,version_id]:
            if db.execute('SELECT count(*) AS n FROM artifact_io_specs WHERE version_id=%s',(selected,)).fetchone()['n']!=3:
                raise ValueError('Unexpected canonical ports')
        if db.execute('SELECT count(*) AS n FROM atom_io_specs WHERE version_id=%s',(spec.version_id,)).fetchone()['n']!=3:
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=2,top_level_output_arity=1,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
