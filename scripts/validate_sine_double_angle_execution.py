#!/usr/bin/env python3
"""Execute the sine double-angle identity through the serialized CDG runner."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.atoms.physics import sine_double_angle as provider
from sciona.physics_ingest.sine_double_angle_execution import SOURCE_VERSION,build_sine_double_angle_execution
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg
from scripts.validate_sine_double_angle_source import validate as validate_source
from tests.physics_ingest.test_sine_double_angle import reference
...
async def validate(root,symbol_file,rule_file,expression_file):
    ...
    for value in [np.finfo(float).max,-np.finfo(float).max,np.nextafter(0.,1.),np.pi/2,0.]:
        cases.append(np.array(value))
    with tempfile.TemporaryDirectory(prefix='sciona-sine-double-angle-runner-') as directory:
        prior=runner.RUNS_DIR;runner.RUNS_DIR=Path(directory)
        try:
            for i,args in enumerate(cases):
                run='case-'+str(i)
                result=await runner.CDGExecutionSession(None,'synthetic-sine-double-angle',run).execute(
                    dict(angle=args),cdg=restored)
                if result['status']!='completed':raise ValueError('Full runner failed')
                actual=np.load(Path(directory)/run/'double_angle/out_double_angle_sine.npy')
                np.testing.assert_array_equal(actual,reference(args))
        finally:runner.RUNS_DIR=prior
    files=['sciona/physics_ingest/sine_double_angle_execution.py','scripts/validate_sine_double_angle_execution.py',
           'tests/physics_ingest/test_sine_double_angle.py','sciona/services/execution_graph_codec.py','sciona/visualizer/runner.py','../sciona-atoms-physics/pyproject.toml']
