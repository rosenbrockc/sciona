#!/usr/bin/env python3
"""Execute ideal-gas volumetric expansion through the serialized CDG runner."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.atoms.physics import ideal_gas_expansion as provider
from sciona.physics_ingest.ideal_gas_expansion_execution import SOURCE_VERSION,build_ideal_gas_expansion_execution
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg
from scripts.validate_ideal_gas_expansion_source import validate as validate_source
from tests.physics_ingest.test_ideal_gas_expansion import reference
...
async def validate(root,symbol_file,rule_file,expression_file):
    ...
    with tempfile.TemporaryDirectory(prefix='sciona-ideal-gas-expansion-runner-') as directory:
        prior=runner.RUNS_DIR;runner.RUNS_DIR=Path(directory)
        try:
            for i,args in enumerate(cases):
                run='case-'+str(i)
                result=await runner.CDGExecutionSession(None,'synthetic-ideal-gas-expansion',run).execute(
                    dict(temperature=args),cdg=restored)
                if result['status']!='completed':raise ValueError('Full runner failed')
                actual=np.load(Path(directory)/run/'expansion/out_volumetric_expansion.npy')
                np.testing.assert_array_equal(actual,reference(args))
        finally:runner.RUNS_DIR=prior
    files=['sciona/physics_ingest/ideal_gas_expansion_execution.py','scripts/validate_ideal_gas_expansion_execution.py',
           'tests/physics_ingest/test_ideal_gas_expansion.py','sciona/services/execution_graph_codec.py','sciona/visualizer/runner.py']
