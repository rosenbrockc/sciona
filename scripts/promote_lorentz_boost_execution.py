#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected lorentz_boost realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.lorentz_boost_execution import build_lorentz_boost_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_lorentz_boost_execution import validate

REFERENCE='openstax-standard-lorentz-boost'
...
LIMITATIONS=[
    ...
    'Standard inertial x-axis boost with common origin and aligned axes; positive c and signed |v|<c.',
    'Positive gamma; exact identity at v=0, signed event times and coordinates; transverse coordinates unchanged.',
    'Exact rational coordinate subtraction before local450digit square root/products; independent float64 output rounding.',
    'Exact zeros/subnormals accepted; nonzero underflow or overflow rejects the entire call.',
    'No exact invariant or inverse guarantee after independent output rounding; no universal correct-rounding claim.',
    'No accelerated, curved-spacetime, arbitrary-direction or superluminal-frame claim.',
    'Source denominator, expansion, aliases, missing ASTs and branch domains reconstructed; no literal source AST parity.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/lorentz_boost_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/lorentz_boost_test_review.json').read_text())
    if tests['test_results']!=dict(tests=37,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['lorentz_boost_execution.json','lorentz_boost_test_review.json','lorentz_boost_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('lorentz_boost-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'lorentz_boost-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.lorentz_boost_corrected_execution'
        ...
        description='Compute positive Lorentz factor and transformed event time and spatial coordinates for a standard inertial x-axis boost. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'boost'},dict(version_id=version_id,node_id='boost',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='lorentz_boost-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['gamma','time_prime','x_prime','y_prime','z_prime']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; corrected Lorentz denominator/expansion, full coefficient conditions and positive branch; v0 identity separately verified. Original draft unchanged.'))})
        title='OpenStax University Physics Volume3: The Lorentz Transformation'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Standard inertial Lorentz transformation, positive Lorentz factor and inverse under velocity reversal.'})
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=6,top_level_output_arity=5,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
