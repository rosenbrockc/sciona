#!/usr/bin/env python3
"""Compare wavelet feature with unchanged pinned helpers executed in Octave."""
import argparse
import hashlib
import inspect
import json
...
def main():
    ...
    hashes = json.loads((root/'docs/reviews/andriy_wavelet_source_hashes.json').read_text())
    cases = []
    with TemporaryDirectory(prefix='sciona-andriy-wavelet-') as temporary:
        ...
        (directory/'check.m').write_text("load('input.mat'); result=zeros(rows(windows),1); for k=1:rows(windows); result(k,1)=wavelet_coeff(windows(k,:)',256); end; reference_version=version; save('-mat7-binary','output.mat','result','reference_version');\n")
        for seed in [10211,10212,10213]:
            for samples in [128,129,257,7680]:
                ...
                actual=andriy_wavelet_features(x)
    ...
    report=dict(source_revision='00f937cc7710977dc812d9fc675864e2b8288658',source_hashes=hashes,
        provider_sha256=hashlib.sha256(Path(inspect.getsourcefile(andriy_wavelet_features)).read_bytes()).hexdigest(),
        validation_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        test_sha256=hashlib.sha256((root/'tests/test_andriy_wavelet_features.py').read_bytes()).hexdigest(),
        ...
        limitations=['Unmodified numerical source helpers in current Octave; no original MATLAB runtime claim.',
                     'One helper output only; full feature ordering, masking and model execution remain separate.',
                     'Standard db4 coefficients replace source polynomial root construction; numerical agreement is tolerance-based, not bitwise.'])
