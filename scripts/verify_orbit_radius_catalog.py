#!/usr/bin/env python3
"""Independent read-only verification of the approved orbit_radius realization."""
import json
...
def verify(root):
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        for row in targets:
            ...
            if len(ports) != 5 or {(p['direction'],p['name'],p['ordinal'],p['dim_signature']) for p in ports} != expected_ports:
            ...
            if row['artifact_kind'] == 'atom':
                ...
                if binding != [dict(node_id='orbit',bound_artifact_fqdn=atom_fqdn,bound_version_content_hash=atom['content_hash'])]:
...
if __name__ == '__main__':
    ...
    result = verify(root)
    (root/'docs/reviews/orbit_radius_catalog_verification.json').write_text(json.dumps(result, indent=2, default=str)+'\n')
