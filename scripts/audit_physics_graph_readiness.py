...
def audit(root, rule_file):
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        for graph in graphs:
            ...
            signatures = [json.loads(n['type_signature']) if isinstance(n['type_signature'],str) else n['type_signature'] for n in nodes]
            ...
            for binding in bindings:
                ...
                exact = [c for c in dimensioned if c['evidence_json'].get('pdg_source_comparison',{}).get('source_comparison',{}).get('correspondence') == 'exact_ast_match']
    ...
    return dict(read_only=True,approval_applied=False,draft_graphs=len(reports),
        ...
        limitations=['Stored evidence only; source bytes, implementation hashes and execution must be freshly verified before approval.',
