#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected trig_exponential realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.trig_exponential_execution import build_trig_exponential_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_trig_exponential_execution import validate

REFERENCE='mit-18013a-trig-exponential-forms'
URL='https://ocw.mit.edu/ans7870/18/18.013a/textbook/chapter02/exercise10.html'
LIMITATIONS=[
    ...
    'Euler prerequisite from product rule, initial value and FTC; linear inverse yields both trigonometric representations.',
    'Both inverse branches retained; positive/negative Euler premise independently checked without complex logarithm.',
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/trig_exponential_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/trig_exponential_test_review.json').read_text())
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['trig_exponential_execution.json','trig_exponential_test_review.json','trig_exponential_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('trig_exponential-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'trig_exponential-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.trig_exponential_corrected_execution'
        ...
        description='Construct both sine and cosine exponential representations with checked Euler premise and linear-system certificate. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'inverse_formula'},dict(version_id=version_id,node_id='inverse_formula',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='trig_exponential-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['cosine_exponential_srepr','sine_exponential_srepr','certificate_json']))))
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Sine and cosine obtained from positive and negative Euler identities by sum and difference.'})
