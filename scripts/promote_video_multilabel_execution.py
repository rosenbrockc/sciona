"""Atomically publish the source-compared Video Multilabel realization at automated Tier 3.
...
LIBRARY_VERSIONS = {'scikit-learn': '1.8.0', 'threadpoolctl': '3.6.0', 'joblib': '1.5.3', 'scipy': '1.17.1', 'numpy': '2.4.2', 'torch': '2.11.0', 'fsspec': '2026.2.0', 'jinja2': '3.1.6', 'markupsafe': '3.0.3', 'networkx': '3.6.1', 'sympy': '1.14.0', 'mpmath': '1.3.0', 'setuptools': '81.0.0', 'typing-extensions': '4.15.0', 'filelock': '3.25.2'}
RUNNER = 'video_multilabel-execution-community.v1'
from scripts.review_video_multilabel_execution import LIMITATIONS
...
def review(root):
    from scripts.review_video_multilabel_execution import audit
    import sciona.atoms.ml.video_multilabel_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_video_multilabel_semantic_review.json').read_text())
    ...
    full=json.loads((reviews/'competition_video_multilabel_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_video_multilabel_review_gates.json').read_text())
    ...
    for name in ('competition_video_multilabel_semantic_review.json','competition_video_multilabel_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_video_multilabel_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_video_multilabel_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_video_multilabel_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community precomputed-feature video multilabel pipeline with batch padding, masked attention, context gating, sparse logistic score fusion and separate per-label F1 thresholds. Synthetic evidence; no raw-video extraction or historical accuracy claim.',
        ...
        references=[]
        for reference,url,title in [('video-multilabel-dictvectorizer','https://scikit-learn.org/1.8/modules/generated/sklearn.feature_extraction.DictVectorizer.html','Scikit-learn sparse dictionary vectorizer documentation')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Library provenance for training-only sparse dictionary encoding. Independent attention/context-gated neural and sparse-linear fusion with synthetic evidence; no historical benchmark reproduction.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_video_multilabel_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_video_multilabel_publication_gates.json':sha(gates_path)})
