"""Read back served image_tabular versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_image_tabular_graph_execution as comparison


def verify(root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_image_tabular_graph_execution.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('4d7b7da1-6568-56b0-aa5e-7e2e7a275556',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='image_tabular-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='image_tabular-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='41907e416953f825b93f7b409d3fb36f16fce58303147e7540566f90bdcc622f', dependency_role='cdg', optional=False)]
    ...
    evidence_path=root/'docs/reviews/competition_image_tabular_graph_execution.json'
    ...
    with tempfile.TemporaryDirectory(prefix='sciona_image_tabular_served_synthetic_') as temp:
        with patch.object(comparison.runner,'RUNS_DIR',Path(temp)), patch.object(comparison.runner,'save_intermediate_value',side_effect=capture):
            status=asyncio.run(comparison.runner.CDGExecutionSession(None,'synthetic-image_tabular-served','case').execute({'payload':payload},cdg=graph))
    ...
    assert result['folds']==3 and result['oof_rows']==12 and result['visual_features']==31 and result['epochs']==40
    ...
    assert result['final_training_loss']<result['initial_training_loss']*.5
    assert evidence_path.read_bytes()==before
    from scripts.promote_image_tabular_execution import review
    review(root)
    result=expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=2,
        served_cdg_count=served, catalog_graph_runtime_execution='passed', serialized_graph_cases=1, training_rows=12,calibration_rows=4,query_rows=2,folds=3,oof_rows=12,epochs=40,
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_image_tabular_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
