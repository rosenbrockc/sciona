"""Reject weakened Temporal sparse evidence without changing files or catalog."""
import hashlib
import json
from pathlib import Path
from unittest.mock import patch
import review_temporal_sparse_execution as review
...
def main():
    ...
    def failure(suffix,name,mutate):
        target=f'competition_temporal_sparse_{suffix}.json'
    ...
    failure('runtime_tests','source_changes_accepted',lambda d:d['checks'].__setitem__('source_mutation_rejected',False))
    failure('runtime_tests','unbounded_output',lambda d:d['checks'].__setitem__('bounded_prediction_output',False))
    failure('runtime_tests','validation_label_leakage',lambda d:d['checks'].__setitem__('validation_labels_isolated',False))
    failure('graph_execution','missing_training',lambda d:d['checks'].__setitem__('updates_per_model',0))
    ...
    paths=['scripts/review_temporal_sparse_execution.py','scripts/validate_temporal_sparse_review_gates.py']
    report=dict(format='temporal_sparse-review-negative-gates.v1',result='passed',
    ...
    (review.ROOT/'docs/reviews/competition_temporal_sparse_review_gates.json').write_text(json.dumps(report,indent=2)+'\n')
