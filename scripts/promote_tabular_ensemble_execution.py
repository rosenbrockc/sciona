"""Atomically publish the source-compared Tabular Ensemble realization at automated Tier 3.
...
LIBRARY_VERSIONS = {'scikit-learn': '1.8.0', 'numpy': '2.4.2', 'scipy': '1.17.1'}
RUNNER = 'tabular_ensemble-execution-community.v1'
from scripts.review_tabular_ensemble_execution import LIMITATIONS
...
def review(root):
    from scripts.review_tabular_ensemble_execution import audit
    import sciona.atoms.ml.tabular_ensemble_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_tabular_ensemble_semantic_review.json').read_text())
    ...
    full=json.loads((reviews/'competition_tabular_ensemble_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_tabular_ensemble_review_gates.json').read_text())
    ...
    for name in ('competition_tabular_ensemble_semantic_review.json','competition_tabular_ensemble_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_tabular_ensemble_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_tabular_ensemble_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_tabular_ensemble_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community generic binary mixed-table pipeline with fold-local cleaning and encoding, logistic/ExtraTrees base models, OOF logistic stacking and separate sigmoid calibration. Synthetic evidence; no historical winning solution or empirical calibration guarantee.',
        ...
        for reference,url,title in [
            ('tabular-ensemble-sklearn-trees','https://scikit-learn.org/1.8/modules/generated/sklearn.ensemble.ExtraTreesClassifier.html','Scikit-learn ExtraTrees classifier documentation'),
            ('tabular-ensemble-sklearn-logistic','https://scikit-learn.org/1.8/modules/generated/sklearn.linear_model.LogisticRegression.html','Scikit-learn logistic regression documentation')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Library documentation supporting explicit tree and logistic estimators; independently tested binary OOF stacking and held-out sigmoid realization, not a historical winning solution.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_tabular_ensemble_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_tabular_ensemble_publication_gates.json':sha(gates_path)})
