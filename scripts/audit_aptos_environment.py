...
DECLARED_ROOTS = ['torch', 'torchvision', 'pretrainedmodels', 'numpy', 'pillow', 'safetensors']
IMPORT_REPORT = Path('docs/reviews/competition_aptos_import_origins.json')
...
def main():
    ...
    report = {'approved': False, 'dependency_constraints_satisfied': not failures,
        ...
        'sha256': {'scripts/audit_aptos_environment.py': hashlib.sha256(Path(__file__).read_bytes()).hexdigest(), str(IMPORT_REPORT): hashlib.sha256(IMPORT_REPORT.read_bytes()).hexdigest()}}
    Path('docs/reviews/competition_aptos_environment_audit.json').write_text(json.dumps(report, indent=2) + '\n')
