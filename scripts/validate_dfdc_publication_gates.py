"""DFDC publication must reject review/evidence/dependency drift before writes."""
import hashlib
import json
...
from scripts import promote_dfdc_execution as publication
...
def main():
    ...
    cases=[('semantic_review','proposed_tier',1),('semantic_review','verdict','pending'),
           ('semantic_review','review_source','human'),('graph','serialized_graph_sha256','invalid')]
    for suffix,field,value in cases:
        target=ROOT/f'docs/reviews/competition_dfdc_{suffix}.json'
    ...
    with patch.object(publication,'sha',side_effect=lambda p:'invalid' if p.name=='competition_dfdc_codec.json' else real_sha(p)):
        try:publication.review(ROOT)
    ...
    report={'format':'dfdc-publication-gates.v1','result':'passed','rejected_gates':rejected,
            'publisher_sha256':real_sha(ROOT/'scripts/promote_dfdc_execution.py'),
            'validator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
            'limits':'Six injected incompatible inputs rejected by read-only review; no catalog mutations.'}
    (ROOT/'docs/reviews/competition_dfdc_publication_gates.json').write_text(json.dumps(report,indent=2)+'\n')
