#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected wave_relations realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.wave_relations_execution import build_wave_relations_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_wave_relations_execution import validate

REFERENCE='openstax-periodic-wave-parameters'
...
LIMITATIONS=[
    ...
    'Single positive-frequency periodic mode, positive phase-speed magnitude; cycles/s and m/s inputs.',
    'Returns period, wavelength, angular frequency and angular wavenumber magnitude; no group velocity, signed wavevector, spatial cycles, broadband or dispersion inference.',
    'Corrected source frequency feed, exact pi interpretation and dependency-ordered six-step proof; original unchanged.',
    'Finite positive identical nonempty shapes including scalars, float64 conversion; every output must be positive finite representable, subnormals accepted.',
    'Rational outputs exact before rounding; angular outputs evaluated at 450 decimal digits in isolated context. No universal correct-rounding or throughput claim.',
    'Requires mpmath 1.3.0 precision extra; no output derived from already-rounded output.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/wave_relations_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/wave_relations_test_review.json').read_text())
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['wave_relations_execution.json','wave_relations_test_review.json','wave_relations_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('wave_relations-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'wave_relations-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.wave_relations_corrected_execution'
        ...
        description='Compute all four periodic-wave parameters from ordinary frequency and phase-speed magnitude, with explicit angular conventions and reviewed six-step source proof. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'parameters'},dict(version_id=version_id,node_id='parameters',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='wave_relations-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['period','wavelength','angular_frequency','angular_wavenumber']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; exact frequency identity, pi semantics and dependency order reconstructed. Original draft unchanged.'))})
        title='OpenStax University Physics Volume 1: Mathematics of Waves'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Defines angular wavenumber and angular frequency from wavelength and period, and their phase-speed relationship.'})
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=2,top_level_output_arity=4,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
