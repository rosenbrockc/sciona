#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected spring_mass realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.spring_mass_execution import build_spring_mass_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_spring_mass_execution import validate

REFERENCE='openstax-spring-mass-motion'
...
LIMITATIONS=[
    ...
    'Constant positive mass and stiffness; linear spring is net restoring force about equilibrium.',
    'Real signed initial displacement and time; zero initial velocity, no damping or driving.',
    'Natural frequency positive by convention; both signed source roots yield identical cosine motion.',
    'All four outputs independently rounded; exact zeros and subnormals allowed, nonzero underflow and overflow rejected.',
    'Local1200digit arithmetic; large-phase inputs treated as exact float64, not a measurement-precision claim.',
    'Corrected missing square root and derivative ASTs; no literal source parity claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/spring_mass_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/spring_mass_test_review.json').read_text())
    ...
    if report['graph_digest']!=digest or report['full_runner_cases']!=7 or report['synthetic_states']!=30 or report['maximum_ulp_error']!=0:raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['spring_mass_execution.json','spring_mass_test_review.json','spring_mass_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('spring_mass-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'spring_mass-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.spring_mass_corrected_execution'
        ...
        description='Compute natural frequency and full released-from-rest spring displacement, velocity and acceleration. Corrected source square root; automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'motion'},dict(version_id=version_id,node_id='motion',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='spring_mass-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['angular_frequency','displacement','velocity','acceleration']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; missing square root and derivative expressions corrected, both frequency branches retained. Original draft unchanged.'))})
        title='OpenStax University Physics Volume1: Simple Harmonic Motion'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Linear spring motion has natural angular frequency sqrt(k/m); the cosine solution is released from rest.'})
