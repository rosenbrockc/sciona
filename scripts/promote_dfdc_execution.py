"""Atomically publish the source-compared DFDC realization at automated Tier 3.
...
SOURCE_VERSION = '95e3414b-c0ed-5816-b944-4466d697d9fc'
SOURCE_HASH = '66f965f75d83ba4ec2daf15afcfa7a657589938dc98c00722b9ccea7eec92679'
COMMIT = '89c6290490bac96b29193a4061b3db9dd3933e36'
RUNNER = 'dfdc-execution-community.v1'
LIMITATIONS = ['Tier3 automated Community reconstruction only; no Tier1 human certification or Tier2 usage qualification.', 'Original intake stays draft. Source correction selects MTCNN and B7, no RetinaFace/Se-ResNeXt or inference landmark alignment.', 'Complete decoded arrays replace media codecs; caller supplies positional parts/pairs, explicit states and native68-point predictor. No historical membership lists or weights bundled.', 'Source frame duplicate behavior, one-face/error fallback,127-face default cap and per-checkpoint confidence heuristic are preserved.', 'Full paired crop/landmark/difference-mask preparation, augmentation, population balancing, smoothed class-balanced BCE, SGD, mutating scheduler and numbered snapshots are included.', 'Source submission requests7 states from5 runs, including suffix40; source default40-epoch run produces0–39. Default combination is rejected. Caller must explicitly supply an executable plan; no automatic extension/rename or historical checkpoint authentication.', 'Graph evidence runs actual MTCNN, native synthetic dlib and5 full B7 models with2epochs/1batch1 update each and7-state prediction. Shortened plan is explicit, not full default-scale training or historical accuracy.', 'Historical stochastic-depth function restored with exact synthetic CPU encoder outputs/gradients/RNG; historical Torch/CUDA/Apex/distributed runtime equivalence is not established.', 'Current facenet/dlib/albumentations/skimage adaptations remain explicit; numeric conversion fixes current MTCNN object-landmark rounding. No positive pretrained detector-quality claim.', 'DFDC execution restores caller RNG after provider discovery. First-time discovery of unrelated packages changes NumPy state; startup isolation is not claimed.', 'Dependencies pinned to tested installed versions; clean independent installation not tested. Explicit initialization supplies pretrained states without automatic downloads.', 'Runtime input arrays, states and native predictor paths remain private; validation uses synthetic fixtures and publishes aggregate evidence only.']
...
def validate_component_evidence(root, full, semantic):
    from scripts.review_dfdc_execution import EVIDENCE
    import importlib.metadata
    reviews=root/'docs/reviews'
    names=set(EVIDENCE)|{'competition_dfdc_execution_dependencies.json'}
    if set(semantic['evidence_sha256'])!=names or semantic['limitations']!=LIMITATIONS:
        raise ValueError('Semantic evidence inventory or limitations differ')
    required={str(p.relative_to(root)) for p in (root/'sciona').glob('dfdc_*.py')}
    if not required<=set(full['sha256']):
        raise ValueError('Incomplete implementation evidence coverage')
    check_hashes(root,full['sha256'])
    docs={}
    for name in names:
        document=json.loads((reviews/name).read_text());docs[name]=document
        if document.get('sha256'):check_hashes(root,document['sha256'])
    def doc(suffix):return docs['competition_dfdc_'+suffix+'.json']
    for suffix in ('graph','runtime_full','stochastic_depth','selection','codec_b7','ensemble_b7'):
        if doc(suffix)['result']!='passed':raise ValueError('Required execution evidence failed')
    checks=doc('stochastic_depth')['checks']
    if checks['parameter_gradient_mismatches']!=0 or not checks['rng_continuation_exact']:
        raise ValueError('Historical stochastic-depth evidence missing')
    if not all(doc('selection')['checks'][k] for k in ('inconsistent_source_default_rejected','explicit_extension_flagged')):
        raise ValueError('Checkpoint adaptation evidence missing')
    if not doc('runtime_full')['checks']['nonfallback_prediction']:
        raise ValueError('Full runtime evidence is only fallback')
    pins=doc('source_pins')
    if (pins['commit'],pins['intake_version'],pins['intake_content_hash'])!=(COMMIT,SOURCE_VERSION,SOURCE_HASH):
        raise ValueError('Source identity differs')
    if sha(root/'docs/licenses/DFDC-MIT.txt')!=next(f['sha256'] for f in pins['files'] if f['path']=='LICENSE'):
        raise ValueError('Source license drift')
    check_hashes(root,semantic['auxiliary_sha256'])
    dependency=doc('execution_dependencies')
    for name,version in dependency['dependencies'].items():
        if importlib.metadata.version(name)!=version:raise ValueError('Installed dependency drift: '+name)
    if sha(root.parent/'sciona-atoms-dl/pyproject.toml')!=dependency['provider_package_sha256']:
        raise ValueError('Shared provider metadata drift')
    evidence={name:sha(reviews/name) for name in names}
    evidence['competition_dfdc_semantic_review.json']=sha(reviews/'competition_dfdc_semantic_review.json')
    return evidence


def review(root):
    import sciona.atoms.dl.dfdc_execution
    graph=build_dfdc_graph()
    digest,nodes,edges=encode_execution_graph(graph)
    reviews=root/'docs/reviews'
    full=json.loads((reviews/'competition_dfdc_graph.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    expected=dict(provider_witness_contracts=2,serialized_full_runtime_graphs=1,actual_B7_training_runs=5,training_updates=10,ensemble_states=7,runner_rejections=1)
    if full['checks']!=expected:raise ValueError('Graph coverage differs')
    semantic=json.loads((reviews/'competition_dfdc_semantic_review.json').read_text())
...
def promote(root, apply=False):
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community DFDC execution: MTCNN paired preparation, full B7 training, explicit run/checkpoint selection and confidence-based ensemble inference. Source default checkpoint40 mismatch requires an explicit executable plan. Synthetic evidence; no historical accuracy claim.',
        ...
        reference = 'dfdc-dfdc-' + COMMIT[:12]
        url = 'https://github.com/selimsef/dfdc_deepfake_challenge/tree/' + COMMIT
        title = 'Pinned DFDC source implementation (MIT)'
        ...
        for identity, selected, node_id in targets:
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_dfdc_graph.json').read_text())['sha256'],
