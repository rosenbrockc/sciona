"""Atomically publish the source-compared Cornell realization at automated Tier 3.
...
COMMITS = {'winning_solver': '4ad1aa4ed99bc097289c7593c55bc09234e0fc59'}
RUNNER = 'cornell-execution-community.v1'
from scripts.review_cornell_execution import LIMITATIONS
...
def review(root):
    from scripts.review_cornell_execution import audit
    import sciona.atoms.ml.cornell_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_cornell_semantic_review.json').read_text())
    ...
    graph=build_cornell_graph()
    digest,nodes,edges=encode_execution_graph(graph)
    full=json.loads((reviews/'competition_cornell_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_cornell_review_gates.json').read_text())
    ...
    for name in ('competition_cornell_semantic_review.json','competition_cornell_review_gates.json'):
...
def promote(root, apply=False):
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community Cornell full thirteen-member DenseNet121 SED lifecycle with seventeen training phases, source augmentation and validation, complete checkpoint reload, ten-copy inference and independent fixed voting. Explicit schedules and initialization; automated synthetic evidence, no historical accuracy claim.',
        ...
        references=[]
        reference='cornell-winning-solver-'+COMMITS['winning_solver'][:12]
        url='https://github.com/ryanwongsa/kaggle-birdsong-recognition/tree/'+COMMITS['winning_solver']
        title='Pinned Cornell first-place training source (MIT; retained dependency notices)'
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Pinned primary Cornell training software and retained MIT notices; independent inference with explicit corrections and complete source-bound synthetic lifecycle evidence.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_cornell_graph_execution.json').read_text())['code_sha256'],
