"""Read back served future_sales versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_future_sales_graph_execution as comparison


def verify(root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_future_sales_graph_execution.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('e6803752-90ef-5cab-8b30-5ebeec716c15',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='future_sales-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='future_sales-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='ffde31b5d874a8d6fafa0103988b0b9a8e0f13a0169f39ff6dce4e30369423e2', dependency_role='cdg', optional=False)]
    ...
    evidence_path=root/'docs/reviews/competition_future_sales_graph_execution.json'
    ...
    with tempfile.TemporaryDirectory(prefix='sciona_future_sales_served_synthetic_') as temp:
        with patch.object(comparison.runner,'RUNS_DIR',Path(temp)), patch.object(comparison.runner,'save_intermediate_value',side_effect=capture):
            status=asyncio.run(comparison.runner.CDGExecutionSession(None,'synthetic-future_sales-served','case').execute({'payload':payload},cdg=graph))
    assert status['status']=='completed'
    result=json.loads(json.dumps(captured['result'],allow_nan=False))
    assert [result[k] for k in ('training_rows','validation_rows','refit_rows','forecast_rows')]==[40,16,56,8]
    assert len(result['trials'])==3
    assert len(result['predictions'])==8 and all(0<=v<=20 for v in result['predictions'])
    assert evidence_path.read_bytes()==before
    from scripts.promote_future_sales_execution import review
    review(root)
    result=expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=2,
        served_cdg_count=served, catalog_graph_runtime_execution='passed', serialized_graph_cases=1, training_rows=40,validation_rows=16,refit_rows=56,forecast_rows=8,trials=3,
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_future_sales_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
