"""Atomically publish the source-compared Image Tabular realization at automated Tier 3.
...
LIBRARY_VERSIONS = {'numpy': '2.4.2', 'scikit-learn': '1.8.0', 'threadpoolctl': '3.6.0', 'joblib': '1.5.3', 'scipy': '1.17.1', 'torch': '2.11.0', 'fsspec': '2026.2.0', 'jinja2': '3.1.6', 'markupsafe': '3.0.3', 'networkx': '3.6.1', 'sympy': '1.14.0', 'mpmath': '1.3.0', 'setuptools': '81.0.0', 'typing-extensions': '4.15.0', 'filelock': '3.25.2'}
RUNNER = 'image_tabular-execution-community.v1'
from scripts.review_image_tabular_execution import LIMITATIONS
...
def review(root):
    from scripts.review_image_tabular_execution import audit
    import sciona.atoms.ml.image_tabular_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_image_tabular_semantic_review.json').read_text())
    ...
    full=json.loads((reviews/'competition_image_tabular_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_image_tabular_review_gates.json').read_text())
    ...
    for name in ('competition_image_tabular_semantic_review.json','competition_image_tabular_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_image_tabular_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_image_tabular_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_image_tabular_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        references=[]
        for reference,url,title in [('image-tabular-imputation','https://scikit-learn.org/1.8/modules/generated/sklearn.impute.SimpleImputer.html','Scikit-learn median imputation and empty-feature handling')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Library provenance for numeric metadata imputation. Independently tested RGB descriptors, fold-local preparation, dropout MLP ensemble and calibration; synthetic evidence only.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_image_tabular_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_image_tabular_publication_gates.json':sha(gates_path)})
