"""Atomically publish the source-compared ARC realization at automated Tier 3.
...
COMMITS = {'winning_solver': '9407072659de1270358c2ba34c527785214dd68b'}
RUNNER = 'arc-execution-community.v1'
LIMITATIONS = ['Automated Tier 3 Community reconstruction. No Tier 1 human certification or Tier 2 usage qualification; original intake remains draft.', 'Original intake repository URL is superseded by the primary winner announcement linking top-quarks/ARC-solution. Source methods correct Python/A* and mandatory all-training-match claims.', 'Full pinned C++ solver with two recorded portability patches and explicit signed-char compilation. Reviewed macOS ARM64 binary only; another platform/toolchain requires a new reviewed build identity.', 'Same-toolchain release builds were byte-identical; 30 paired synthetic ASan/UBSan comparisons passed. These checks do not prove correctness for every input or historical GCC equivalence.', 'Four adaptive phases, original concurrency and estimates preserved. Monotonic clock and 10ms RSS sampling differ from original 100ms monitor. Timed-out children are explicitly killed/reaped; missing baselines/outputs are exposed, not fabricated.', 'Source nine-hour/95-percent time and memory defaults cap caller budgets; synthetic evidence used shorter explicit budgets. No nine-hour competition benchmark or historical accuracy claim.', 'RSS is sampled process memory, not an instantaneous OS cap or parent-memory accounting. Zero observed baseline RSS yields a derived limit failure rather than silently increasing the budget.', 'Strict rectangular integer grids, maximum 30x30 and explicit adapter caps of 20 training pairs/20 test inputs. Test labels rejected; each test is isolated. Predictions and runtime grids are private caller data.', 'No implicit downloads or caller executable paths. A local reviewed build bundle is required; matcher supplies broader runner dependencies and clean-environment installation is unverified.']
...
def review(root):
    from scripts.review_arc_execution import audit
    import sciona.atoms.ml.arc_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_arc_semantic_review.json').read_text())
    ...
    full=json.loads((reviews/'competition_arc_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_arc_review_gates.json').read_text())
    ...
    for name in ('competition_arc_semantic_review.json','competition_arc_review_gates.json'):
...
def promote(root, apply=False):
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community ARC winning C++ solver: full color normalization, DSL/piece search, candidate scoring and adaptive four-phase scheduling. Reviewed macOS arm64 binary required; explicit budgets and missing predictions. Synthetic evidence; no historical competition accuracy claim.',
        ...
        references=[]
        reference='arc-winning-solver-'+COMMITS['winning_solver'][:12]
        url='https://github.com/top-quarks/ARC-solution/tree/'+COMMITS['winning_solver']
        title='Pinned ARC first-place C++ solver (MIT)'
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Primary winning-author announcement links top-quarks/ARC-solution, correcting the intake repository URL. Pinned MIT software with source-complete automated synthetic execution review.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_arc_graph_execution.json').read_text())['code_sha256'],
