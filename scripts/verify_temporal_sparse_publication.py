"""Read back served temporal_sparse versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_temporal_sparse_graph_execution as comparison


def verify(root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_temporal_sparse_graph_execution.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('ad8e992f-1e5d-530c-9475-c9caf4a08bad',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='temporal_sparse-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='temporal_sparse-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='5ee3659042611115b7a83dbf165f7b44355a0f09ff346514ce4dd2a15e66340b', dependency_role='cdg', optional=False)]
    ...
    evidence_path=root/'docs/reviews/competition_temporal_sparse_graph_execution.json'
    before=evidence_path.read_bytes()
    captured={}
    def capture(directory,node,name,value):
        if node=='execute' and name=='out_result':captured['result']=value
    with tempfile.TemporaryDirectory(prefix='sciona_temporal_sparse_served_synthetic_') as temp:
        payload=comparison.payload(Path(temp))
        with patch.object(comparison.runner,'RUNS_DIR',Path(temp)), patch.object(comparison.runner,'save_intermediate_value',side_effect=capture):
            status=asyncio.run(comparison.runner.CDGExecutionSession(None,'synthetic-temporal_sparse-served','case').execute({'payload':payload},cdg=graph))
    assert status['status']=='completed'
    result=json.loads(json.dumps(captured['result'],allow_nan=False))
    import numpy as np
    assert (result['training_rows'],result['validation']['rows'],result['query_rows'])==(30,10,2)
    assert result['models']==2 and result['updates_per_model']==5 and result['feature_count']==24
    scores=np.asarray(result['predictions'])
    assert scores.shape==(2,) and np.isfinite(scores).all() and ((scores>=0)&(scores<=20)).all()
    assert np.isfinite(result['validation']['rmsle']) and result['validation']['rmsle']>=0
    assert evidence_path.read_bytes()==before
    from scripts.promote_temporal_sparse_execution import review
    review(root)
    result=expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=2,
        served_cdg_count=served, catalog_graph_runtime_execution='passed', serialized_graph_cases=1, training_rows=30,validation_rows=10,query_rows=2,models=2,updates_per_model=5,feature_count=24,
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_temporal_sparse_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
