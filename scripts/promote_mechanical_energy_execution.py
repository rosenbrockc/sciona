#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected mechanical_energy realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.mechanical_energy_execution import build_mechanical_energy_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_mechanical_energy_execution import validate

REFERENCE='openstax-mechanical-energy-conservation'
...
LIMITATIONS=[
    ...
    'One-dimensional Newtonian motion with positive constant mass and constant signed net force.',
    'Time-independent potential U=-F*x with zero at x=0; no other work or dissipation.',
    'Finite initial position and velocity, nonnegative duration; zero force, zero duration and reversal supported.',
    'Source interval-power equations use average velocity; equality to endpoint mean requires constant acceleration.',
    'Exact rational arithmetic after float64 conversion, then independent output rounding.',
    'Exact zero/subnormals accepted; nonzero output underflow or overflow rejects the entire call.',
    'Rounded energies need not sum exactly; work uses signed displacement, not path length.',
    'Four missing source equations recovered, malformed energy identifier corrected; no literal source AST parity.',
    'No variable-force, dissipative, relativistic or time-dependent-potential claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/mechanical_energy_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/mechanical_energy_test_review.json').read_text())
    if tests['test_results']!=dict(tests=28,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    graph=build_mechanical_energy_execution();digest,nodes,edges=encode_execution_graph(graph)
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['mechanical_energy_execution.json','mechanical_energy_test_review.json','mechanical_energy_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('mechanical_energy-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'mechanical_energy-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.mechanical_energy_corrected_execution'
        ...
        description='Compute constant-force position, velocity, kinetic and potential energy, conserved total energy and signed work. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'energy'},dict(version_id=version_id,node_id='energy',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='mechanical_energy-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['final_position','final_velocity','kinetic_energy','potential_energy','total_energy','work']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; recovered kinetic energy and Newton law equations, corrected initial-energy identifier and explicit interval-average semantics. Constant-force trajectories prove zero-duration extension. Original draft unchanged.'))})
        title='OpenStax University Physics Volume1: Conservation of Energy'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Mechanical energy conservation when no nonconservative work is present; constant force is a scoped special case.'})
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=5,top_level_output_arity=6,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
