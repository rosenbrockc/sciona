#!/usr/bin/env python3
"""Execute circular orbital radius and speed through the serialized CDG runner."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.atoms.physics import orbit_radius as provider
from sciona.physics_ingest.orbit_radius_execution import SOURCE_VERSION,build_orbit_radius_execution
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg
from scripts.validate_orbit_radius_source import validate as validate_source
from tests.physics_ingest.test_orbit_radius import reference
...
async def validate(root,symbol_file,rule_file,expression_file):
    ...
    cases=[tuple(np.exp(rng.uniform(-300,300,size=(2,3,4))) for _ in range(3))]
    cases += [tuple(np.array(v) for v in row) for row in [(1.,8.,2*np.pi),
        (1e308,1e308,1e-300),(1e-308,1e-308,1e308),(1.,1.,1.),(np.nextafter(0.,1.),1.,1.)]]
    with tempfile.TemporaryDirectory(prefix='sciona-orbit-radius-runner-') as directory:
        prior=runner.RUNS_DIR;runner.RUNS_DIR=Path(directory)
        try:
            for i,args in enumerate(cases):
                run='case-'+str(i)
                result=await runner.CDGExecutionSession(None,'synthetic-orbit-radius',run).execute(
                    dict(gravitational_constant=args[0],central_mass=args[1],period=args[2]),cdg=restored)
                ...
                for name,expected in zip(['radius','orbital_speed'],expected_arrays):
                    actual=np.load(Path(directory)/run/('orbit/out_'+name+'.npy'))
                    np.testing.assert_array_equal(actual,expected)
        finally:runner.RUNS_DIR=prior
    files=['sciona/physics_ingest/orbit_radius_execution.py','scripts/validate_orbit_radius_execution.py',
           'tests/physics_ingest/test_orbit_radius.py','tests/physics_ingest/test_orbit_radius_proof.py','sciona/services/execution_graph_codec.py','sciona/visualizer/runner.py']
