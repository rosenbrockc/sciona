"""Record the installed CPU VSB dependency closure and retain notices."""
import hashlib
...
def main():
    ...
    for name in versions:
        ...
        for file in sorted(dist.files or [],key=str):
            ...
            data=path.read_bytes();digest=hashlib.sha256(data).hexdigest();target='VSB-notice-'+digest[:16]+'.txt'
    ...
    requirements=ROOT/'requirements/vsb-execution.txt'
    ...
    (ROOT/'docs/reviews/competition_vsb_environment.json').write_text(json.dumps(report,indent=2)+'\n')
