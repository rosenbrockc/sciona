...
PREFIX='sciona.atoms.electrical.'
SERIES=PREFIX+'series_resistance.series_resistance'
REFERENCE='openstax-university-physics-v2-10-2'
...
def main():
    ...
    specs={p.fqdn:p for p in _parse_registered_atoms(repo=ProviderRepo('sciona-atoms',provider),artifact_root=provider/'src/sciona/atoms') if p.fqdn==SERIES or p.fqdn.startswith(PREFIX+'residuals.')}
    if len(specs)!=5:raise ValueError('expected five tested providers')
    reviews=[]
    for path in ['series_resistance_pending.json','residuals_community_pending.json']:
    ...
    with psycopg.connect(os.environ['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        ...
        reports=db.execute("SELECT * FROM artifact_audit_evidence WHERE runner_version IN ('series-cdg-execution.v1','residual-cdg-execution.v1') AND passed FOR SHARE").fetchall()
        for fqdn,spec in specs.items():
            ...
            ref={**reference,'ref_key':REFERENCE,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Supports the documented ohmic/series relation. Implementation behavior is covered by the version-bound automated review; no human certification is claimed.'}
