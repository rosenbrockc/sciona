#!/usr/bin/env python3
"""Store and bind the exact Andriy execution graph without approving it."""
import argparse
import hashlib
import json
...
def intake(root, reference_dir, library, apply=False):
    prerequisite = audit(root, reference_dir, library)
    providers = {record['runtime_fqdn']: record for record in inventory(root)['providers']}
    graph = build_andriy_execution_graph()
    digest, nodes, edges = encode_execution_graph(graph)
    artifact_id = uuid5(SOURCE_ID, 'andriy-execution.v1')
    version_id = uuid5(artifact_id, digest)
    fqdn = 'cdg.competition.solution.kaggle.barachant_seizure_1st.andriy_execution'
    evidence_hashes = {name: hashlib.sha256((root/'docs/reviews'/name).read_bytes()).hexdigest()
        for name in ['andriy_population_parity.json', 'andriy_execution_parity.json', 'andriy_raw_small_sample_failure.json']}
    created = 0
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('andriy-execution-draft-intake.v1'))")
        ...
        created += ensure_row(db, 'artifacts', {'artifact_id': artifact_id}, dict(
            ...
            description='Three Andriy branches across three ordered populations from raw clips. Draft runtime reconstruction with documented current-engine adaptations; full raw execution and promotion review pending.'))
        ...
        for node in graph.nodes:
            ...
            provider = db.execute("SELECT a.artifact_id,v.content_hash,e.details FROM artifacts a JOIN artifact_versions v USING(artifact_id) JOIN artifact_audit_evidence e USING(version_id) WHERE a.fqdn=%s AND v.version_id=%s AND v.is_latest AND e.runner_version='andriy-provider-intake.v1' AND e.passed FOR SHARE OF a,v,e", (record['catalog_fqdn'], record['version_id'])).fetchone()
            if not provider or provider['content_hash'] != record['content_hash'] or provider['details']['source_sha256'] != record['provider_sha256']:
                raise ValueError('Exact provider intake missing or changed')
            if provider['details']['interfaces'] != record['interfaces']:
            ...
            row = dict(version_id=version_id, node_id=node.node_id, bound_artifact_fqdn=record['catalog_fqdn'],
                bound_version_content_hash=record['content_hash'], binding_confidence=1., binding_source='andriy-provider-binding.v1',
        ...
        evidence_id = uuid5(version_id, 'andriy-execution-draft-intake.v1')
        created += ensure_row(db, 'artifact_audit_evidence', {'evidence_id': evidence_id}, dict(
            ...
            passed=True, status='completed', source_kind='automated', runner_version='andriy-execution-draft-intake.v1',
    ...
    return dict(applied=apply, rows_created=created, nodes=len(nodes), edges=len(edges), bindings=len(graph.nodes),
        catalog_roundtrip='passed', publication='draft', execution_prerequisites_complete=prerequisite['component_execution_prerequisites_complete'])
...
if __name__ == '__main__':
    ...
    print(json.dumps(intake(Path(__file__).resolve().parents[1], args.reference_dir, args.library, args.apply), indent=2))
