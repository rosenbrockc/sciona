"""Read back served DFDC versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_dfdc_graph as comparison


def verify(root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_dfdc_graph.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('95e3414b-c0ed-5816-b944-4466d697d9fc',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='dfdc-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='dfdc-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='66f965f75d83ba4ec2daf15afcfa7a657589938dc98c00722b9ccea7eec92679', dependency_role='cdg', optional=False)]
    ...
    evidence_path=root/'docs/reviews/competition_dfdc_runtime_full.json'
    before=evidence_path.read_bytes()
    with patch.object(comparison.full,'execute',side_effect=lambda payload: comparison.run_graph(graph,payload)):
        comparison.full.main()
    ...
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=2,
        served_cdg_count=served, catalog_graph_runtime_comparison='passed', serialized_graph_cases=result['checks']['serialized_full_runtime_graphs'],
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_dfdc_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
