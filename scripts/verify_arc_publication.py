"""Read back served arc versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_arc_graph_execution as comparison


def verify(root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_arc_graph_execution.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('4a5eaed3-fbc0-564f-b12b-120a76304e05',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='arc-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='arc-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='6be75e601582ede9f8122169f2ff4a763759a628a0631acae25b34a3fda826bf', dependency_role='cdg', optional=False)]
    ...
    evidence_path=root/'docs/reviews/competition_arc_graph_execution.json'
    ...
    with tempfile.TemporaryDirectory(prefix='sciona_arc_served_synthetic_') as temp:
        with patch.dict(os.environ,{'SCIONA_ARC_BUILD_DIR':os.environ.get('SCIONA_ARC_BUILD_DIR','/private/tmp/sciona_arc_reviewed_build_v1')}), patch.object(comparison.runner,'RUNS_DIR',Path(temp)), patch.object(comparison.runner,'save_intermediate_value',side_effect=capture):
            status=asyncio.run(comparison.runner.CDGExecutionSession(None,'synthetic-arc-served','case').execute({'payload':payload},cdg=graph))
    ...
    assert len(result['attempts'])==8 and all(a['status']=='success' for a in result['attempts'])
    assert all(t['input'] in [c['grid'] for c in predictions] for t,predictions in zip(payload['task']['test'],result['predictions'],strict=True))
    assert evidence_path.read_bytes()==before
    from scripts.promote_arc_execution import review
    review(root)
    result=expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=2,
        served_cdg_count=served, catalog_graph_runtime_comparison='passed', serialized_graph_cases=1, full_source_search_runs=8,
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_arc_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
