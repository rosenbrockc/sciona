#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected momentum realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.momentum_execution import build_momentum_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_momentum_execution import validate
...
LIMITATIONS=[
    ...
    'Explicit real-3D reconstruction of incomplete source dot-product syntax; no literal source-AST parity.',
    'Finite real arrays of identical nonempty shape (...,3), common orthonormal frame, SI momentum units.',
    'Momentum conservation and zero other initial momentum are premises; no energy or full scattering-event validity claim.',
    'Exact converted-input rational differences and squared norm rounded independently to float64; squared norm is not recomputed from rounded recoil.',
    'Nonzero values rounding to zero and nonfinite outputs rejected; exact zeros and subnormals supported. No throughput claim.',
]
...
async def promote(root,symbol_file,rule_file,apply=False):
    ...
    tests=json.loads((root/'docs/reviews/momentum_test_review.json').read_text())
    if tests['test_results']!=dict(tests=51,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    graph=build_momentum_execution();digest,nodes,edges=encode_execution_graph(graph)
    if report['graph_digest']!=digest or report['full_runner_cases']!=5 or report['synthetic_vectors']!=84 or report['maximum_ulp_error']!=0:raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['momentum_execution.json','momentum_test_review.json','momentum_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('momentum-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'momentum-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.momentum_corrected_execution'
        ...
        description='Compute real 3D recoil momentum and its squared Euclidean norm from two momentum vectors. Explicitly reconstructed source proof; exact rational arithmetic before float64 rounding. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'recoil'},dict(version_id=version_id,node_id='recoil',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='momentum-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['recoil_momentum','squared_momentum']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; reconstructed vector semantics and explicit domain are separately reviewed. Not an invoked dependency or source-parity claim.'))})
        title='OpenStax University Physics Volume 3: The Compton Effect'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Vector momentum conservation with initially stationary electron; this realization does not enforce energy conservation.'})
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=2,top_level_output_arity=2,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
