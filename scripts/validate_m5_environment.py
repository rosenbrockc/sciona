"""Record the installed CPU M5 dependency closure and retain notices."""
import hashlib
...
def main():
    ...
    for name in versions:
        ...
        for file in sorted(dist.files or [],key=str):
            ...
            data=path.read_bytes();digest=hashlib.sha256(data).hexdigest();target='M5-notice-'+digest[:16]+'.txt'
    ...
    requirements=ROOT/'requirements/m5-execution.txt'
    ...
    (ROOT/'docs/reviews/competition_m5_environment.json').write_text(json.dumps(report,indent=2)+'\n')
