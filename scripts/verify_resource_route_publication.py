"""Read back served resource_route versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_resource_route_graph as comparison


def verify(root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_resource_route_graph_execution.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('65947cba-17e3-5a60-ac2b-7a3545885a39',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='resource_route-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        assert len(bindings) == len(graph.nodes) == 5
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='resource_route-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='d07d9ca3d6cf5dc01c2b487384d68cc218797da1504dc97483cbeb69b0f0ee1f', dependency_role='cdg', optional=False)]
    ...
    evidence_path=root/'docs/reviews/competition_resource_route_graph_execution.json'
    before=evidence_path.read_bytes()
    cases=[]
    payload=comparison.payload();cases.append((payload,'optimal',3))
    payload=comparison.payload();payload['blocked_edges']=['detour'];cases.append((payload,'infeasible',None))
    payload=comparison.payload();payload['max_expansions']=1;cases.append((payload,'search_limit',None))
    payload=comparison.payload();payload['goal']='s';cases.append((payload,'optimal',0))
    for index,(payload,wanted,cost) in enumerate(cases):
        captured={};executed=set()
        def capture(directory,node,name,value):
            executed.add(node)
            if node=='select' and name=='out_result':captured['result']=value
        with tempfile.TemporaryDirectory(prefix='sciona_resource_route_served_synthetic_') as temp:
            with patch.object(comparison.runner,'RUNS_DIR',Path(temp)), patch.object(comparison.runner,'save_intermediate_value',side_effect=capture):
                status=asyncio.run(comparison.runner.CDGExecutionSession(None,'synthetic-resource-route-served',str(index)).execute({'payload':payload},cdg=graph))
        assert status['status']=='completed'
        result=json.loads(json.dumps(captured['result'],allow_nan=False))
        assert result['status']==wanted and result['cost']==cost
        assert executed=={'encode','generate','search','validate','select'}
        if cost==3:assert result['edge_ids']==['detour','join','finish'] and result['resources']==[3]
        else:assert result['edge_ids']==[]
    assert evidence_path.read_bytes()==before
    from scripts.promote_resource_route_execution import review
    review(root)
    result=expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=5,
        served_cdg_count=served, catalog_graph_runtime_execution='passed', serialized_graph_cases=4, executed_nodes_per_case=5,
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_resource_route_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
