"""Raw synthetic slides through group partition and original full training comparison."""
import argparse
...
from sciona.hubmap_raw_training import run_raw_training, prepare_fold
...
def validate(root,source,library):
    ...
    training,validation=prepare_fold(slides,groups,['synthetic-valid'],tile_size=32)
    ...
    actual=run_raw_training(candidate,optimizers[0],schedulers[0],
        slides,groups,['synthetic-valid'],tile_size=32,
    ...
    return dict(approved=False,synthetic_only=True,epochs=[r['epoch'] for r in expected],
        ...
        limitations=['Reference starts at separately source-validated preparation/partition boundary; candidate executes raw preparation within training.',
...
if __name__=='__main__':
    ...
    (root/'docs/reviews/competition_hubmap_raw_training.json').write_text(json.dumps(report,indent=2)+'\n')
