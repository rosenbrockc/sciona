#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected infall_speed realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.infall_speed_execution import build_infall_speed_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_infall_speed_execution import validate

REFERENCE='openstax-infall-zero-energy'
...
LIMITATIONS=[
    ...
    'Positive G, test mass, fixed central mass and center-based radius; negligible test-mass backreaction.',
    'Newtonian exterior spherical or point-source field, inward radial zero-energy motion; caller establishes clearance.',
    'Rest at infinity is an asymptotic boundary, not a finite-time launch event.',
    'Speed positive, inward radial velocity negative, work on particle positive, potential negative with zero at infinity.',
    'No finite-mass two-body, drag, relativity, finite travel-time or collision-free escape guarantee.',
    'Local450digit arithmetic; subnormals accepted, nonzero output underflow and overflow reject.',
    'Corrected source force sign, malformed ASTs, work aliases and alternative roots; no literal source parity.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    tests=json.loads((root/'docs/reviews/infall_speed_test_review.json').read_text())
    if tests['test_results']!=dict(tests=44,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['infall_speed_execution.json','infall_speed_test_review.json','infall_speed_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('infall_speed-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'infall_speed-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.infall_speed_corrected_execution'
        ...
        description='Compute radial infall speed, inward velocity, gravitational work and potential from rest at infinity in a fixed central field. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'infall'},dict(version_id=version_id,node_id='infall',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='infall_speed-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['speed','inward_radial_velocity','gravitational_work','potential_energy']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; force sign, integral ASTs, infinity boundary and signed velocity branches reconstructed. Original draft unchanged.'))})
