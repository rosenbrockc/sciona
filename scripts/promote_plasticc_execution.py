"""Atomically publish the source-compared Avocado realization at automated Tier 3.
...
SOURCE_VERSION = '57909e28-f500-51c8-91da-8e104377d2fd'
SOURCE_HASH = '3290c60e5dc631a1a69f70a9fed6a9d232727c4f8c7dc1fd97dfc778c8be7a57'
COMMIT = 'cd7809db4d7eb92860b178d8f5a53ab16f03ad91'
RUNNER = 'plasticc-execution-community.v1'
LIMITATIONS = ['Original intake tsfresh/cesium, MLP and Platt labels are unsupported by inspected implementation; separate derived graph uses actual Avocado source.', 'Complete normal path: supplied empirical reference, redshift/time/wavelength augmentation, GP/noise/detection, custom230raw/41selected features, grouped stratified folds, flat/redshift weights, LightGBM and source unknown-class formula.', 'Astropy/george/scipy used directly. Independent covariance/gradient, bin weights/loss and postprocessing oracles supplement source AST comparisons.', 'LightGBM current callbacks replace removed fit kwargs; source and native API actual fitting agree, no historical binary replay claim.', 'Numerical source defaults pinned by Git blob/SHA256; IO configuration omitted. Explicit instance augmentation RNG and fold/model seeds declared.', 'Required classes, fold counts, finite input, positive errors, nonzero signal, disjoint populations and output probability mass checked; sparse source sampling failures propagate without numerical substitution.', 'Source NaNs in undefined features retained for LightGBM missing-value handling; final nonfinite probabilities reject. Source failed GP returns attempted parameters while using guessed predictor, preserved.', 'Optional bias simulation is an experimental branch, not required by default train script. File codecs and historical accuracy excluded explicitly.', 'Tier3 Community requires no human certification. Tier2 community use and Tier1 human expert review are not established.', 'Synthetic configured workloads, no competition performance claim', 'CPU/current numerical dependency versions', 'No bundled observations, empirical catalogs or pretrained models']
...
def review(root):
    import sciona.atoms.dl.plasticc_execution
    graph=build_plasticc_graph()
    digest,nodes,edges=encode_execution_graph(graph)
    reviews=root/'docs/reviews'
    full=json.loads((reviews/'competition_plasticc_graph.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(3,2):
        raise ValueError('Serialized graph identity differs')
    expected=dict(provider_contracts=3,serialized_graph_cases=1,parameter_rejections=9,runner_rejections=1)
    if full['checks']!=expected:raise ValueError('Graph coverage differs')
    semantic=json.loads((reviews/'competition_plasticc_semantic_review.json').read_text())
    ...
    suffixes=['observations','features','augmentation','weighting','training','predictions','population','runtime','graph']
    evidence={}
    required={ 'observations': 'independent_covariance', 'features':'real_gp_integration',
        'augmentation':'real_gp','weighting':'independent_redshift','training':'native_api_oracles',
        'predictions':'independent_rows','population':'trained_folds','runtime':'serialized_complete_runs',
        'graph':'serialized_graph_cases'}
    for suffix in suffixes:
        name='competition_plasticc_'+suffix+'.json'
        document=json.loads((reviews/name).read_text())
        if (suffix!='graph' and document.get('status')!='passed') or document['checks'].get(required[suffix],0)<1:
            raise ValueError('Missing execution coverage')
        check_hashes(root,document.get('hashes',document.get('implementation_sha256')))
        evidence[name]=sha(reviews/name)
    pins=json.loads((reviews/'competition_plasticc_source_pins.json').read_text())
    if pins['commit']!=COMMIT or sha(root/'docs/licenses/Avocado-MIT.txt')!=pins['files']['LICENSE.md']:
        raise ValueError('Source pin/license differs')
    from sciona.plasticc_defaults import DEFAULTS
    defaults=json.loads((reviews/'competition_plasticc_defaults.json').read_text())
    if defaults['source_commit']!=COMMIT or defaults['numerical_defaults']!=DEFAULTS:
        raise ValueError('Source defaults differ')
    for suffix in ['source_pins','defaults','semantic_review']:
        name='competition_plasticc_'+suffix+'.json';evidence[name]=sha(reviews/name)
...
def promote(root, apply=False):
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community Avocado derived runtime: explicit observations and empirical reference, astronomical augmentation, GP/custom features, grouped weighted LightGBM training and separate-object probabilities. Synthetic evidence; no competition accuracy claim.',
        ...
        reference = 'avocado-plasticc-' + COMMIT[:12]
        url = 'https://github.com/kboone/avocado/tree/' + COMMIT
        title = 'Pinned Avocado source implementation (MIT)'
        ...
        for identity, selected, node_id in targets:
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_plasticc_graph.json').read_text())['implementation_sha256'],
