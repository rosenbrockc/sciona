#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected parallel_resistance realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.parallel_resistance_execution import build_parallel_resistance_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_parallel_resistance_execution import validate
...
LIMITATIONS=[
    ...
    'Two positive finite ideal linear resistors across common nodes; signed common voltage with consistent current orientation.',
    'Zero voltage accepted by constitutive model; no0/0 inference.',
    'All outputs independently rounded after exact rational arithmetic; every nonzero output must remain finite representable.',
    'Matching nonempty float64-compatible arrays; no broadcasting; caller establishes topology and SI units.',
    'No ideal short/open circuit, negative resistance, reactive impedance or nonlinear-device claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    tests=json.loads((root/'docs/reviews/parallel_resistance_test_review.json').read_text())
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['parallel_resistance_execution.json','parallel_resistance_test_review.json','parallel_resistance_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('parallel_resistance-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'parallel_resistance-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.parallel_resistance_corrected_execution'
        ...
        description='Compute equivalent resistance, two branch currents and total current for ideal parallel resistors with signed common voltage, including zero. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'network'},dict(version_id=version_id,node_id='network',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='parallel_resistance-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['equivalent_resistance','current_1','current_2','total_current']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; recovered Ohms-law source and explicit nonzero-voltage division branch with zero-voltage constitutive extension. Original draft unchanged.'))})
        title='OpenStax University Physics Volume2: Resistors in Series and Parallel'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Parallel resistors share voltage; branch currents sum and equivalent conductances add.'})
