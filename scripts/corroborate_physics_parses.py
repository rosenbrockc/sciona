#!/usr/bin/env python3
"""Reconcile parse confidence only after recomputing pinned source correspondence."""
import argparse
import json
import os
...
from sciona.physics_ingest.parse_corroboration import corroborate_pending_parses
...
def main():
    ...
    with psycopg.connect(url, row_factory=dict_row) as db:
        result = corroborate_pending_parses(db, args.symbol_file.read_bytes())
        if not args.apply:
            db.rollback()
