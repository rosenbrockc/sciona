"""Read back served champs versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_champs_graph_execution as comparison


def verify(root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_champs_graph_execution.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('af2888ee-499c-5db7-8576-f4096e1f8804',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='champs-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='champs-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='d1891c5f46afe2b677bce08e67522a016bd798bb654d43f4060822f957eb30a9', dependency_role='cdg', optional=False)]
    ...
    evidence_path=root/'docs/reviews/competition_champs_graph_execution.json'
    before=evidence_path.read_bytes()
    payload=comparison.synthetic_payload()
    import torch
    torch.set_num_threads(2)
    captured={}
    def capture(directory,node,name,value):
        if node=='execute' and name=='out_result':captured['result']=value
    with tempfile.TemporaryDirectory(prefix='sciona_champs_served_synthetic_') as temp:
        with patch.dict(os.environ,{'SCIONA_CHAMPS_SOURCE_DIR':os.environ.get('SCIONA_CHAMPS_SOURCE_DIR','/private/tmp/sciona_champs_source')}), patch.object(comparison.runner,'RUNS_DIR',Path(temp)), patch.object(comparison.runner,'save_intermediate_value',side_effect=capture):
            status=asyncio.run(comparison.runner.CDGExecutionSession(None,'synthetic-champs-served','case').execute({'payload':payload},cdg=graph))
    assert status['status']=='completed'
    result=json.loads(json.dumps(captured['result'],allow_nan=False))
    assert result['models_completed']==list(comparison.MODEL_ORDER)
    assert len(result['predictions'])==12 and len(result['training'])==13
    assert all(r['selected_epoch']==0 for r in result['training'].values())
    assert evidence_path.read_bytes()==before
    from scripts.promote_champs_execution import review
    review(root)
    result=expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=2,
        served_cdg_count=served, catalog_graph_runtime_comparison='passed', serialized_graph_cases=1, full_model_lifecycles=13,
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_champs_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
