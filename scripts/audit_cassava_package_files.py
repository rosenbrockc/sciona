...
def main():
    ...
    report = {'approved': False, 'all_recorded_hashes_match': all(not item['failures'] for item in results.values()),
        ...
        'sha256': {p: hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in [
            str(audit_path), 'scripts/audit_cassava_package_files.py']}}
    Path('docs/reviews/competition_cassava_package_file_audit.json').write_text(json.dumps(report, indent=2) + '\n')
