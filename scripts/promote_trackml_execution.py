"""Atomically publish the source-compared TrackML realization at automated Tier 3.
...
COMMIT = 'f1a6e63969167159ef72c4ba32897e5aef7c3888'
RUNNER = 'trackml-execution-community.v1'
LIMITATIONS = [
    'Automated Tier 3 computational review; no human-reviewed Tier 1 designation.',
    'Explicit runtime module/hit/cell tables, settings and optional layer grids required; no data or learned maps embedded.',
    'Full iterative source seeding, fitting, following, pairing, scoring, commitment, optional odd-hit postprocessing and fill retained.',
    'Every detector layer must retain enough hits for configured kNN in every round; sparse/depleted populations explicitly reject.',
    'Source fixed detector discovery assumptions, contiguous hit IDs and source numeric storage limits apply.',
    'Synthetic source comparisons include cells/maps; no learned calibration, competition accuracy or physical usefulness claim.',
    'Compatibility changes cover modern pandas, isolated defaults, vector pre-move and empty/no-seed completion.',
    'Ground-truth scoring, file codecs and diagnostic exports excluded from in-memory execution.',
    'Original 11-stage intake remains draft; derived graph has decode/full iterative tracking/encode boundaries.',
...
def review(root):
    import sciona.atoms.physics.trackml_execution
    graph=build_trackml_graph()
    digest,nodes,edges=encode_execution_graph(graph)
    reviews=root/'docs/reviews'
    full=json.loads((reviews/'competition_trackml_lifecycle.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(3,2):
        raise ValueError('Serialized graph identity or lifecycle boundary differs')
    checks=full['checks']
    expected={'event_cases':18,'commit_rounds':21,'execution_cases':12,'payload_cases':12,
              'serialized_graph_cases':4,'sparse_rejections':3,'payload_rejections':5,
              'cell_cases':4,'map_cases':4,'raw_source_empty_failures':3,'raw_source_no_seed_failures':2}
    if any(checks.get(k)!=v for k,v in expected.items()):raise ValueError('Lifecycle coverage differs')
    if any(checks['assignments_by_mode'].get(k,0)<=0 for k in ['single','multi','sunset','odd','maps','cells','cells_maps']):
        raise ValueError('Nonempty assignment coverage required')
    suffixes=['circle','helix','nearest','detector','intersections','corrections','layer_integration',
              'neighbors','bayesian','candidates','extension','cells','seeding','runtime','lifecycle']
    evidence={}
    for suffix in suffixes:
        name='competition_trackml_'+suffix+'.json'
        document=json.loads((reviews/name).read_text())
        if not document.get('synthetic_only'):raise ValueError('Synthetic evidence required')
        check_hashes(root,document['implementation_sha256']);evidence[name]=sha(reviews/name)
    pins=json.loads((reviews/'competition_trackml_source_pins.json').read_text())
    if pins['commit']!=COMMIT or sha(root/'docs/licenses/TrackML-BSD-2-Clause.txt')!=pins['files']['LICENSE']:
        raise ValueError('Source pin/license differs')
    evidence['competition_trackml_source_pins.json']=sha(reviews/'competition_trackml_source_pins.json')
...
def promote(root, apply=False):
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='TrackML source-grounded iterative particle tracking from explicit runtime geometry, hits, optional cells and maps through seeding, following, pairing, ranking, commitment and complete hit assignment. Synthetic parity; complete-layer preconditions; no accuracy claim.',
        ...
        reference = 'edwinst-trackml-' + COMMIT[:12]
        url = 'https://github.com/edwinst/trackml_solution/tree/' + COMMIT
        title = 'Pinned TrackML source implementation (BSD-2-Clause)'
        ...
        for identity, selected, node_id in targets:
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_trackml_lifecycle.json').read_text())['implementation_sha256'],
