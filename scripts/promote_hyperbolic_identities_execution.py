#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected hyperbolic_identities realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.hyperbolic_identities_execution import build_hyperbolic_identities_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_hyperbolic_identities_execution import validate

REFERENCE='nist-dlmf-hyperbolic-definitions'
URL='https://dlmf.nist.gov/4.28'
LIMITATIONS=[
    'Automated Tier3; no human-reviewed Tier1 designation.',
    'Provably finite real commutative dimensionless symbolic argument; caller establishes units and assumptions.',
    'Six function forms and all18 reconstructed identities, with direct complex sine/cosine exponential definition checks.',
    'Mathematical I replaces named source symbol; self-products and step8-before-step7 dependency explicit.',
    'Real input ensures positive cosh denominator; no arbitrary complex quotient domain.',
    'Symbolic outputs and self-contained certificate; no numeric cancellation or literal source AST parity claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/hyperbolic_identities_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/hyperbolic_identities_test_review.json').read_text())
    ...
    graph=build_hyperbolic_identities_execution();digest,nodes,edges=encode_execution_graph(graph)
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['hyperbolic_identities_execution.json','hyperbolic_identities_test_review.json','hyperbolic_identities_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('hyperbolic_identities-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'hyperbolic_identities-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.hyperbolic_identities_corrected_execution'
        ...
        description='Construct six hyperbolic and imaginary-argument function forms, eighteen identities and a self-contained exponential-definition certificate. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'identities'},dict(version_id=version_id,node_id='identities',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='hyperbolic_identities-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['functions_json','identities_json','certificate_json']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; mathematical I, self-product multiplicity and dependency order reconstructed with all18identities retained. Original draft unchanged.'))})
        title='NIST DLMF4.28: Hyperbolic Definitions and Periodicity'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Exponential hyperbolic definitions and relations sin(I*x)=I*sinh(x), cos(I*x)=cosh(x).'})
            ...
            evidence_id=uuid5(selected,'hyperbolic_identities-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='hyperbolic_identities-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'hyperbolic-identities-real-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Finite real dimensionless commutative argument; six function forms and eighteen symbolic identities with direct complex exponential definitions and positive hyperbolic denominator. No general complex quotient or numerical cancellation guarantee.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=1,top_level_output_arity=3,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
