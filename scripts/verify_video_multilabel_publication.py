"""Read back served video_multilabel versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_video_multilabel_graph_execution as comparison


def verify(root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_video_multilabel_graph_execution.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('13b48adf-8fb5-5ea2-a26c-44344bb8907e',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='video_multilabel-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='video_multilabel-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='30d0444ec640598942c6221136e608d7b15449732ceb848939e1424321c496ad', dependency_role='cdg', optional=False)]
    ...
    evidence_path=root/'docs/reviews/competition_video_multilabel_graph_execution.json'
    ...
    with tempfile.TemporaryDirectory(prefix='sciona_video_multilabel_served_synthetic_') as temp:
        with patch.object(comparison.runner,'RUNS_DIR',Path(temp)), patch.object(comparison.runner,'save_intermediate_value',side_effect=capture):
            status=asyncio.run(comparison.runner.CDGExecutionSession(None,'synthetic-video_multilabel-served','case').execute({'payload':payload},cdg=graph))
    assert status['status']=='completed'
    result=json.loads(json.dumps(captured['result'],allow_nan=False))
    import numpy as np
    assert (result['training_rows'],result['calibration_rows'],result['query_rows'])==(8,4,2)
    assert result['epochs']==80 and result['label_count']==result['sparse_models']==2
    scores=np.asarray(result['scores'])
    assert scores.shape==(2,2) and np.isfinite(scores).all() and ((scores>=0)&(scores<=1)).all()
    assert result['labels']==(scores>=result['thresholds']).astype(int).tolist()
    assert result['final_training_loss']<result['initial_training_loss']*.25
    assert evidence_path.read_bytes()==before
    from scripts.promote_video_multilabel_execution import review
    review(root)
    result=expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=2,
        served_cdg_count=served, catalog_graph_runtime_execution='passed', serialized_graph_cases=1, training_rows=8,calibration_rows=4,query_rows=2,epochs=80,label_count=2,sparse_models=2,
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_video_multilabel_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
