"""Record the installed CPU Santander dependency closure and retain notices."""
import hashlib
...
def main():
    ...
    for name in versions:
        ...
        for file in sorted(dist.files or [],key=str):
            ...
            data=path.read_bytes();digest=hashlib.sha256(data).hexdigest();target='Santander-notice-'+digest[:16]+'.txt'
    ...
    requirements=ROOT/'requirements/santander-execution.txt'
    ...
    (ROOT/'docs/reviews/competition_santander_environment.json').write_text(json.dumps(report,indent=2)+'\n')
