"""Read back served adversarial versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_adversarial_graph_execution as comparison


def verify(root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_adversarial_graph_execution.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('cebbfc7d-e32e-5f52-9ec2-e1de0d955501',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='adversarial-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='adversarial-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='d6b977ae079c82dec43ffdf88f64fb9089c17e587c3ee4c36f5411aa0b28828b', dependency_role='cdg', optional=False)]
    ...
    # Execute the served graph through the full native/neural synthetic fixture.
    # The fixture writes its deterministic aggregate report; require unchanged bytes.
    evidence_path=root/'docs/reviews/competition_adversarial_graph_execution.json'
    before=evidence_path.read_bytes()
    original_run=comparison.run_graph
    with patch.object(comparison,'run_graph',side_effect=lambda ignored,payload: original_run(graph,payload)):
        comparison.main()
    assert evidence_path.read_bytes()==before
    from scripts.promote_adversarial_execution import review
    review(root)
    result=expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=2,
        served_cdg_count=served, catalog_graph_runtime_comparison='passed', serialized_graph_cases=1,
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_adversarial_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
