#!/usr/bin/env python3
"""Execute time-based projectile motion through the serialized CDG runner."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.atoms.physics import projectile_motion as provider
from sciona.physics_ingest.projectile_motion_execution import SOURCE_VERSION,build_projectile_motion_execution
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg
from scripts.validate_projectile_motion_source import validate as validate_source
from tests.physics_ingest.test_projectile_motion import reference
...
async def validate(root,symbol_file,rule_file,expression_file):
    ...
    cases=[(rng.normal(size=(2,3,4)),rng.normal(size=(2,3,4)),
    ...
    cases += [tuple(np.array(v,dtype=float) for v in row) for row in [
        (1,2,3,np.pi/2,1,4),(1,2,0,1e308,1,3),(1,2,3,-1e308,0,2),
        (-1e308,1e308,1e308,0,5e307,2),(0,0,np.nextafter(0.,1.),0,0,1)]]
    with tempfile.TemporaryDirectory(prefix='sciona-projectile-motion-runner-') as directory:
        prior=runner.RUNS_DIR;runner.RUNS_DIR=Path(directory)
        try:
            for i,args in enumerate(cases):
                run='case-'+str(i)
                result=await runner.CDGExecutionSession(None,'synthetic-projectile-motion',run).execute(
                    dict(initial_x=args[0],initial_y=args[1],initial_speed=args[2],launch_angle=args[3],gravity=args[4],elapsed_time=args[5]),cdg=restored)
                ...
                for name,expected in zip(['x','y','vx','vy'],expected_arrays):
                    actual=np.load(Path(directory)/run/('motion/out_'+name+'.npy'))
                    np.testing.assert_array_equal(actual,expected)
        finally:runner.RUNS_DIR=prior
    files=['sciona/physics_ingest/projectile_motion_execution.py','scripts/validate_projectile_motion_execution.py',
           'tests/physics_ingest/test_projectile_motion.py','tests/physics_ingest/test_projectile_motion_proof.py','sciona/services/execution_graph_codec.py','sciona/visualizer/runner.py']
