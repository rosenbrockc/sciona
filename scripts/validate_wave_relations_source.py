#!/usr/bin/env python3
"""Recover missing wave_relations equations from pinned public Cypher source."""
import argparse
import hashlib
import json
...
from sciona.physics_ingest.wave_relations_proof import SOURCE_VERSION, SOURCE_HASH, build_proof, verify_proof, verify_source_equations
from sciona.ghost.dimensions import DimensionalSignature

EXPECTED = {'0404050504': ('\\lambda', '\\frac{v}{f}'), '0934990943': ('k', '\\frac{2 \\pi}{v T}'), '1293923844': ('\\lambda', 'v T'), '2113211456': ('f', '1/T'), '2131616531': ('T f', '1'), '3121513111': ('k', '\\frac{2 \\pi}{\\lambda}'), '3131111133': ('T', '1 / f'), '3131211131': ('\\omega', '2 \\pi f'), '3132131132': ('\\omega', '\\frac{2\\pi}{T}'), '5900595848': ('k', '\\frac{\\omega}{v}')}
...
def validate(root, symbol_file, rule_file, expression_file):
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if [n['name'] for n in nodes] != ['substitute RHS of expr 1 into expr 2']*3+['multiply both sides by','divide both sides by','substitute RHS of expr 1 into expr 2']:
        ...
        if {b['bound_artifact_fqdn'].rsplit('.', 1)[1] for b in bindings} != set(EXPECTED):
            raise ValueError('Ten exact source identities required')
        expected_steps = [
            ('111634',{'3132131132','5900595848','0934990943'},[]),
            ('111634',{'3131111133','0404050504','1293923844'},[]),
            ('111634',{'1293923844','0934990943','3121513111'},[]),
            ('111182',{'3131111133','2131616531'},["Symbol('pdg0006235')"]),
            ('111975',{'2131616531','2113211456'},["Symbol('pdg0009491')"]),
            ('111634',{'2113211456','3131211131','3132131132'},[]),
        ]
        ...
        if set(snapshots) != {'0404050504','0934990943','1293923844','5900595848'} or not pins or any(p != pins[0] for p in pins):
    ...
    scalar_expectations={
        'pdg0006235':('f',DimensionalSignature()),
        'pdg0004201':('f',DimensionalSignature(T=-1)),
        'pdg0009491':('T',DimensionalSignature(T=1)),
        'pdg0001115':(chr(92)+'lambda',DimensionalSignature(L=1)),
        'pdg0001357':('v',DimensionalSignature(L=1,T=-1)),
        'pdg0005321':('k',DimensionalSignature(L=-1)),
        'pdg0002321':(chr(92)+'omega',DimensionalSignature(T=-1)),
        'pdg0003141':(chr(92)+'pi',DimensionalSignature()),
    }
    ...
    if str(mapping[sp.Symbol('pdg0003141')])!='pi':
        raise ValueError('Exact pi interpretation required')
    ...
    return dict(approved=False, source_version_id=SOURCE_VERSION, source_content_hash=SOURCE_HASH,
                stored_source_equations=4, recovered_missing_equations=6, source_records=records, stored_evidence=snapshots,
                ...
                source_ast_check=verify_source_equations({k:r['equation'] for k,r in records.items()}),
                corrections=['Multiplication feed pdg0006235 is dimensionless and unrelated; corrected to source frequency identity pdg0004201.',
                             'Exact source pi identity interpreted as mathematical pi.',
                             'Stored numbering is not dependency order; replay 4,5,6,1,2,3 with positive frequency, period and phase speed.'],
                implementation_sha256={p: hashlib.sha256((root/p).read_bytes()).hexdigest() for p in
                    ['scripts/validate_wave_relations_source.py','sciona/physics_ingest/wave_relations_proof.py','sciona/physics_ingest/euler_constant_semantics.py','sciona/physics_ingest/source_symbolic.py']})
