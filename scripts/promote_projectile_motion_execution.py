#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected projectile_motion realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.projectile_motion_execution import build_projectile_motion_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_projectile_motion_execution import validate

REFERENCE='openstax-time-based-projectile-motion'
...
LIMITATIONS=[
    ...
    'Fixed Cartesian basis, upward-positive y, ideal point particle, constant downward gravity, no drag.',
    'Finite signed initial positions and angle; nonnegative initial speed, gravity and elapsed time.',
    'Zero speed/time and zero-gravity inertial limit included; no horizontal-velocity division.',
    'Local2000digit intermediate calculations and literal converted angle, including very large values.',
    'Rounded pi/2 is not exact vertical and is not snapped; all outputs independently rounded.',
    'Exact zero/subnormals accepted; nonzero output underflow or overflow rejects the entire call.',
    'No universal correct-rounding, exact rounded-energy, trajectory collision or throughput claim.',
    'Source aliases, inverted trigonometric ASTs and integration constants reconstructed; no literal source parity.',
    'No terrain cutoff, path-length, variable-gravity, drag or landing-event model.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/projectile_motion_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/projectile_motion_test_review.json').read_text())
    if tests['test_results']!=dict(tests=29,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    graph=build_projectile_motion_execution();digest,nodes,edges=encode_execution_graph(graph)
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['projectile_motion_execution.json','projectile_motion_test_review.json','projectile_motion_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('projectile_motion-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'projectile_motion-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.projectile_motion_corrected_execution'
        ...
        description='Compute full signed projectile position and velocity at elapsed time for arbitrary launch direction under constant downward gravity. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'motion'},dict(version_id=version_id,node_id='motion',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='projectile_motion-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['x','y','vx','vy']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; time-dependent functions and component aliases corrected, trigonometric quotients restored, definite integration with initial conditions. Zero speed/gravity/time limits established independently. Original draft unchanged.'))})
        title='OpenStax University Physics Volume1: Projectile Motion'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Independent horizontal/vertical constant-acceleration motion and initial velocity components from speed and launch angle.'})
            ...
            evidence_id=uuid5(selected,'projectile_motion-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='projectile_motion-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'time-based-ideal-projectile-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Fixed Cartesian axes, upward-positive y, no drag, constant nonnegative gravity. Finite initial positions and literal launch angle; nonnegative speed and time. Full signed x,y,vx,vy with no horizontal division or terrain cutoff. Zero limits included; independently rounded outputs.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=6,top_level_output_arity=4,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
