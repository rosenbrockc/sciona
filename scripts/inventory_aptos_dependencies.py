...
def main():
    ...
    requirements = Path('requirements/aptos-reference.txt')
    lines = ['# Candidate numerical reference environment; publication qualification pending.',
             '# Python 3.13 / macOS arm64 dependency resolution observed by audit_aptos_environment.py.']
    lines += [f'{name}=={details["version"]}' for name, details in packages.items()]
    requirements.write_text('\n'.join(lines) + '\n')
    paths = [str(requirements), str(audit_path), 'scripts/inventory_aptos_dependencies.py']
    ...
    Path('docs/reviews/competition_aptos_dependency_notice_inventory.json').write_text(json.dumps(report, indent=2) + '\n')
