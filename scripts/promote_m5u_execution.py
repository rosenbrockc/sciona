"""Atomically publish the source-compared M5 realization at automated Tier 3.
...
LIBRARY_VERSIONS = {'joblib': '1.5.3', 'lightgbm': '4.7.0', 'narwhals': '2.26.0', 'numpy': '2.4.2', 'pandas': '2.3.3', 'python-dateutil': '2.9.0.post0', 'pytz': '2026.1.post1', 'scikit-learn': '1.8.0', 'scipy': '1.17.1', 'six': '1.17.0', 'threadpoolctl': '3.6.0', 'tzdata': '2025.3'}
RUNNER = 'm5u-execution-community.v1'
from scripts.review_m5u_execution import LIMITATIONS
...
def review(root):
    from scripts.review_m5u_execution import audit
    import sciona.atoms.ml.m5u_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_m5_uncertainty_semantic_review.json').read_text())
    ...
    if semantic['dependencies']!=LIBRARY_VERSIONS:raise ValueError('Publisher dependency declaration differs')
    ...
    full=json.loads((reviews/'competition_m5_uncertainty_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_m5_uncertainty_review_gates.json').read_text())
    if gates['result']!='passed' or gates['checks']['injected_failures_rejected']!=12:
        raise ValueError('Negative semantic gates missing')
    check_hashes(root,gates['sha256'])
    evidence=dict(semantic['evidence_sha256'])
    for name in ('competition_m5_uncertainty_semantic_review.json','competition_m5_uncertainty_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_m5_uncertainty_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_m5u_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_m5_uncertainty_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if {n.node_id for n in original.nodes} != {'base_forecasting','residual_calculation','probability_distribution_fitting','quantile_generation','hierarchical_scaling'}:
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community corrected uncertainty CPU pipeline: hierarchy preparation, 14 direct-quantile levels, nine quantiles, full repeated 28-day forecasts and native-unit restoration. Explicit retained-base-reference correction for source-undefined aggregate normalization; no historical accuracy claim.',
        ...
        references=[]
        for reference,url,title in [('m5u-winner-archive-18e4e77','https://github.com/Mcompetitions/M5-methods/tree/18e4e776cfa1ded3b387fabe008c6c2c44922007/Code%20of%20Winning%20Methods/U1','M5 organizers archive of winning methods: U1')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Primary U1 software provenance for direct quantile regression, source-weighted training and repeated forecasts. Independent reconstruction with explicit aggregate normalization correction; archive reuse grant and historical parity remain unqualified.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_m5_uncertainty_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_m5_uncertainty_publication_gates.json':sha(gates_path)})
