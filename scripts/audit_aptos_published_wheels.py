...
def main():
    audit_path = Path('docs/reviews/competition_aptos_environment_audit.json')
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        packages = dict(pool.map(inspect, [(name, row) for name, row in audit['packages'].items() if name != 'pretrainedmodels']))
    ...
    report = {'approved': False, 'all_wheel_pins_have_compatible_unyanked_wheels': not unresolved,
        ...
        'source_only_packages': {'pretrainedmodels': {'version': audit['packages']['pretrainedmodels']['version'], 'evidence': 'docs/reviews/competition_aptos_pretrainedmodels_source_origin.json'}},
        ...
        'sha256': {p: hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in [
            str(audit_path), 'scripts/audit_aptos_published_wheels.py']}}
    Path('docs/reviews/competition_aptos_published_wheel_audit.json').write_text(json.dumps(report, indent=2) + '\n')
