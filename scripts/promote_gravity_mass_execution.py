#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected gravity_mass realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.gravity_mass_execution import build_gravity_mass_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_gravity_mass_execution import validate

REFERENCE='openstax-gravitational-mass-inference'
URL='https://openstax.org/books/university-physics-volume-1/pages/13-2-gravitation-near-earths-surface'
LIMITATIONS=[
    'Automated Tier3 only; no human-reviewed Tier1 or Tier2 claim.',
    'Newtonian spherical source at exterior/surface radius, pure gravitational acceleration magnitude.',
    'No effective-gravity rotation correction, nonspherical inference or precision Earth ephemeris claim.',
    'Positive finite g,r,G in consistent SI units; matching nonempty arrays, no broadcasting.',
    'Exact rational arithmetic before output rounding; positive finite representable mass required.',
    'Source numerical endpoint disagrees with stated inputs; corrected example retained, malformed units/ASTs reconstructed.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/gravity_mass_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/gravity_mass_test_review.json').read_text())
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['gravity_mass_execution.json','gravity_mass_test_review.json','gravity_mass_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('gravity_mass-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'gravity_mass-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.gravity_mass_corrected_execution'
        ...
        description='Infer Newtonian spherical-source mass from gravitational acceleration, radius and G, with exact arithmetic and corrected source-example evaluation. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'mass_inference'},dict(version_id=version_id,node_id='mass_inference',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='gravity_mass-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['mass']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; recovered force laws, corrected mass renaming/units and numerical evaluation mismatch. Original draft unchanged.'))})
        title='OpenStax University Physics Volume1: Gravitation Near Earth Surface'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Gravitational acceleration g=GM/r² for a spherical source supports inferred mass at exterior/surface distance.'})
