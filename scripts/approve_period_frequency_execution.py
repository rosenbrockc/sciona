#!/usr/bin/env python3
"""Approve the exact tested period-to-frequency execution CDG at Community tier."""
import argparse
import hashlib
import inspect
import json
...
def main():
    ...
    with psycopg.connect(os.environ['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('period-frequency-community-approval.v1'))")
        intakes=db.execute("SELECT e.*,a.fqdn,a.status AS artifact_status,a.is_publishable,v.content_hash,v.trust_tier FROM artifact_audit_evidence e JOIN artifacts a USING(artifact_id) JOIN artifact_versions v USING(version_id) WHERE e.runner_version='period-frequency-intake.v1' AND e.details->>'kind'='execution' FOR UPDATE OF a,v FOR SHARE OF e").fetchall()
        if len(intakes)!=1:raise ValueError('one period execution intake required')
        intake=intakes[0]
        ...
        def evidence(identity,runner):
            ...
            return row
        executed=evidence(intake['details']['source_execution_evidence_id'],'period-frequency-execution.v1')
        if _digest(executed['details'])!=intake['details']['source_execution_evidence_sha256']:raise ValueError('execution intake mismatch')
        replay=evidence(executed['details']['source_replay_evidence_id'],'pdg-source-graph-replay.v2')
        if _digest(replay['details'])!=executed['details']['source_replay_sha256']:raise ValueError('replay evidence mismatch')
        if replay['version_id']!=executed['version_id']:raise ValueError('source version mismatch')
        if hashlib.sha256(Path(inspect.getfile(frequency_from_period)).read_bytes()).hexdigest()!=executed['details']['provider_source_sha256']:raise ValueError('provider source drift')
        ...
        if source['content_hash']!=replay['details']['content_hash']:raise ValueError('source graph version drift')
        assessments={}
        for key,digest in replay['details']['expression_evidence_sha256'].items():
            ...
            assessments[key]={'source_comparison_sha256':_digest(fresh),'review_source':'automated','scope':'Exact pinned source AST used in successful rational replay; positive-period runtime domain checked separately.'}
        graph=CDGExport.model_validate(executed['details']['execution_graph'])
        ...
        reference=db.execute("SELECT ref_id,ref_key,title,url,verified FROM artifact_references WHERE artifact_id=%s AND ref_key='openstax-university-physics-v1-15-1' FOR SHARE",(replay['artifact_id'],)).fetchone()
        if not reference or not reference['verified']:raise ValueError('scientific reference missing')
        ensure_row(db,'artifact_references',{'artifact_id':intake['artifact_id'],'ref_key':reference['ref_key']},{'artifact_id':intake['artifact_id'],**reference,'source':'llm_extracted','confidence':'high','relevance_note':'Supports positive period and ordinary frequency; exact implementation and domain checked by automated evidence.'})
        limitations=['Community tier: automated review, no human certification.', 'Converts supplied finite positive cycle duration in seconds to ordinary frequency in hertz.', 'No period estimation or angular-frequency claim; shape preserved, invalid domains and floating errors rejected.', 'Embedded draft metadata records the immutable validation snapshot; catalog publication state is authoritative.']
        ...
        approval={'publication_tier':3,'review_source':'automated','content_hash':intake['content_hash'],'source_dependency_review':'passed as mandatory proof provenance, not an invoked numerical dependency','source_expression_assessments':assessments,'source_replay_evidence_id':str(replay['evidence_id']),'source_replay_sha256':_digest(replay['details']),'execution_evidence_id':str(executed['evidence_id']),'execution_evidence_sha256':_digest(executed['details']),'provider_approval_evidence_id':str(provider_review['evidence_id']),'provider_approval_sha256':_digest(provider_review['details']),'limitations':limitations}
        audit_id=uuid5(intake['version_id'],'period-frequency-community-approval.v1')
        created=ensure_row(db,'artifact_audit_evidence',{'evidence_id':audit_id},{'evidence_id':audit_id,'artifact_id':intake['artifact_id'],'version_id':intake['version_id'],'audit_type':'semantic_audit','passed':True,'status':'completed','source_kind':'automated','runner_version':'period-frequency-community-approval.v1','details':Jsonb(approval)})
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=1,top_level_output_arity=1,topo_hash=%s,source_symbol='frequency_from_period',description=%s WHERE artifact_id=%s",(topo,'Convert a positive finite cycle duration in seconds to ordinary frequency in hertz. Preserves scalar or array shape; does not estimate period or calculate angular frequency.',intake['artifact_id']))
