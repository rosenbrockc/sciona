#!/usr/bin/env python3
"""Independent read-only verification of the approved dirichlet_mode realization."""
import json
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        for row in targets:
            ...
            if row['artifact_kind'] == 'atom':
                ...
                if binding != [dict(node_id='mode',bound_artifact_fqdn=atom_fqdn,bound_version_content_hash=atom['content_hash'])]:
...
if __name__ == '__main__':
    ...
    result = verify(root)
    (root/'docs/reviews/dirichlet_mode_catalog_verification.json').write_text(json.dumps(result, indent=2, default=str)+'\n')
