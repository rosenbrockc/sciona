#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected orbit_radius realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.orbit_radius_execution import build_orbit_radius_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_orbit_radius_execution import validate

REFERENCE='openstax-circular-satellite-orbits'
...
LIMITATIONS=[
    ...
    'Positive G, central mass and period; uniform circular Newtonian test-mass orbit in exterior spherical field.',
    'Satellite mass negligible; gravity is sole centripetal force; caller establishes exterior clearance.',
    'Radius is center-based, not altitude; no perturbation, oblateness or station-keeping model.',
    'Geostationary interpretation also requires equatorial prograde circular orbit and sidereal rotation period.',
    'Independent float64 rounding from450digit arithmetic; positive subnormals accepted, underflow0 and overflow rejected.',
    'Corrected source mass/speed aliases and mathematical pi; no literal AST parity claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/orbit_radius_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/orbit_radius_test_review.json').read_text())
    if tests['test_results']!=dict(tests=42,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    graph=build_orbit_radius_execution();digest,nodes,edges=encode_execution_graph(graph)
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['orbit_radius_execution.json','orbit_radius_test_review.json','orbit_radius_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('orbit_radius-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'orbit_radius-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.orbit_radius_corrected_execution'
        ...
        description='Compute center-based orbital radius and circular speed from explicit G, central mass and period under the test-mass model. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'orbit'},dict(version_id=version_id,node_id='orbit',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='orbit_radius-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['radius','orbital_speed']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; mass/speed aliases reconstructed and geostationary assumptions made explicit. Original draft unchanged.'))})
        title='OpenStax University Physics Volume1: Satellite Orbits and Energy'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Circular orbital speed and period follow from gravity supplying centripetal force and the orbital circumference.'})
            ...
            evidence_id=uuid5(selected,'orbit_radius-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='orbit_radius-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'orbit-radius-circular-test-mass-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Positive G, central mass and period in Newtonian circular exterior test-mass model. Radius is not altitude. Geostationary requires equatorial prograde circular motion and sidereal period; no geometry or clearance certification from period alone.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
