#!/usr/bin/env python3
"""Independent read-only verification of the approved sound_scale realization."""
import json
...
def verify(root):
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        for row in targets:
            ...
            proof = row['details']['corrected_proof']['proof']
            if proof['physical_universal_bound_proven'] is not False or proof['approximation_steps'] != [1, 5, 7]:
                raise ValueError('Conditional physical scope lost')
            ...
            if len(ports) != 12 or {(p['direction'],p['name'],p['ordinal'],p['dim_signature']) for p in ports} != expected_ports:
            ...
            if row['artifact_kind'] == 'atom':
                ...
                atom = next(t for t in targets if t['artifact_kind'] == 'atom')
                atom_fqdn = db.execute('SELECT fqdn FROM artifacts WHERE artifact_id=%s', (atom['artifact_id'],)).fetchone()['fqdn']
                if binding != [dict(node_id='scaling',bound_artifact_fqdn=atom_fqdn,bound_version_content_hash=atom['content_hash'])]:
...
if __name__ == '__main__':
    ...
    result = verify(root)
    (root/'docs/reviews/sound_scale_catalog_verification.json').write_text(json.dumps(result, indent=2, default=str)+'\n')
