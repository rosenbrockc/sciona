"""Atomically publish the source-compared first-wave dynamics realization at automated Tier 3.
...
COMMIT = SOURCE_HASH
RUNNER = 'first_wave_dynamics-execution-community.v1'
LIMITATIONS = ['Corrected generated-source derivation; original derivative parse and missing-premise gaps retained in source review, not certified.', 'Positive constant mass, one inertial Cartesian component and twice differentiable position; acceleration equals second time derivative.', 'Force is net force in the chosen component. SI units and coefficient consistency are caller premises, not inferred by expression grammar.', 'Actual symbolic differentiation, force calculation and acceleration solve; formal unspecified position retains a derivative, sampled outputs require evaluable expressions.', 'No numerical force integration, global smoothness certification or Tier1 human expert review.', 'Synthetic analytic/proof and serialized graph evidence; automatic Tier3 only, no Tier2 community-use evidence.']
...
def review(root):
    import sciona.atoms.physics.first_wave_dynamics
    graph=build_first_wave_dynamics_graph()
    digest,nodes,edges=encode_execution_graph(graph)
    reviews=root/'docs/reviews'
    full=json.loads((reviews/'physics_first_wave_dynamics_graph.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(1,0):
        raise ValueError('Serialized graph identity differs')
    expected=dict(provider_contracts=1,serialized_graph_cases=3,runner_rejections=1)
    if full['checks']!=expected:raise ValueError('Graph coverage differs')
    semantic=json.loads((reviews/'physics_first_wave_dynamics_semantic_review.json').read_text())
    if (semantic['review_source']!='automated' or semantic['proposed_tier']!=3
        or semantic['verdict']!='acceptable_with_limits' or semantic['source_hash']!=SOURCE_HASH):
    ...
    evidence={}
    for suffix in ['review','execution','graph']:
        name='physics_first_wave_dynamics_'+suffix+'.json'
    ...
    source_review=json.loads((reviews/'physics_first_wave_dynamics_review.json').read_text())
    if source_review['source_hash']!=SOURCE_HASH or source_review['source_version_id']!=SOURCE_VERSION:
        raise ValueError('Source proof identity differs')
    if not all(source_review['corrected_proof']['checks'].values()) or source_review['checks']['exact_symbolic_examples']!=18:
        raise ValueError('Corrected proof incomplete')
    execution=json.loads((reviews/'physics_first_wave_dynamics_execution.json').read_text())
    if execution['status']!='passed' or execution['checks']['pytest_cases']!=36:
        raise ValueError('Execution coverage differs')
    name='physics_first_wave_dynamics_semantic_review.json';evidence[name]=sha(reviews/name)
    directory=root.parent/'sciona-atoms-physics'
    parsed=_parse_registered_atoms(repo=ProviderRepo('sciona-atoms-physics',directory),artifact_root=directory/'src/sciona/atoms')
...
def promote(root, apply=False):
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if db.execute('SELECT count(*) AS n FROM artifact_cdg_nodes WHERE version_id=%s',(SOURCE_VERSION,)).fetchone()['n'] != 2:
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community corrected first-wave dynamics: explicit position derivative, net force and acceleration solve under positive constant mass and stated kinematic assumptions.',
        ...
        reference = 'first-wave-dynamics-openstax-v1'
        url = 'https://openstax.org/books/university-physics-volume-1/pages/5-3-newtons-second-law'
        title = 'OpenStax University Physics: Newton second law'
        ensure('references_registry', {'ref_id': reference}, dict(ref_id=reference, ref_type='book', title=title, url=url))
        ...
        for identity, selected, node_id in targets:
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                source_identity_hash=SOURCE_HASH, evidence_sha256=evidence, limitations=LIMITATIONS,
                implementation_sha256=json.loads((root / 'docs/reviews/physics_first_wave_dynamics_graph.json').read_text())['implementation_sha256'],
