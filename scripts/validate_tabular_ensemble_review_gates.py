"""Reject weakened Tabular ensemble evidence without changing files or catalog."""
import hashlib
import json
from pathlib import Path
from unittest.mock import patch
import review_tabular_ensemble_execution as review
...
def main():
    ...
    def failure(suffix,name,mutate):
        target=f'competition_tabular_ensemble_{suffix}.json'
    ...
    failure('runtime_tests','calibration_leakage',lambda d:d['checks'].__setitem__('calibration_does_not_refit_stack',False))
    failure('runtime_tests','missing_oof_validation',lambda d:d['checks'].__setitem__('complete_oof_coverage',False))
    ...
    paths=['scripts/review_tabular_ensemble_execution.py','scripts/validate_tabular_ensemble_review_gates.py']
    report=dict(format='tabular_ensemble-review-negative-gates.v1',result='passed',
    ...
    (review.ROOT/'docs/reviews/competition_tabular_ensemble_review_gates.json').write_text(json.dumps(report,indent=2)+'\n')
