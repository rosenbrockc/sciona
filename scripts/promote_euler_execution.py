#!/usr/bin/env python3
"""Atomically intake and approve the reviewed interpreted Euler proof realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.euler_execution import build_euler_execution,PRIMITIVE,SOURCE_VERSION
SOURCE_HASH='e58d9a93c9e8fe0bc16a46a5b710ee1b172fb1286ea91a8206c8cbd25a1f8d5d'
...
from scripts.validate_euler_execution import validate

REFERENCE='nist-dlmf-4-14-e3'
URL='https://dlmf.nist.gov/4.14.E3'
LIMITATIONS=[
    'Automated Tier 3 only; no Tier 1 certification or Tier 2 usage claim.',
    'Fixed exact symbolic Euler proof; no user inputs, generic theorem-proving or physical simulation claim.',
    'Explicit reviewed constant interpretation, not literal source-AST parity; imaginary-unit variable-label discrepancy preserved.',
    'Initial real-angle identity checked with SymPy complex expansion, not derived from first principles.',
    'JSON-safe certificate contains four internal symbolic transformation steps, evaluated by one numerical-runner node.',
]



async def promote(root,symbol_file,rule_file,apply=False):
    report=await validate(root,symbol_file,rule_file)
    retained=json.loads((root/'docs/reviews/euler_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/euler_test_review.json').read_text())
    if tests['test_results']!=dict(tests=10,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    if report['graph_digest']!=digest or report['full_runner_cases']!=2 or report['proof_steps_per_run']!=4 or report['exact_source_interpreted_step_parity'] is not True:raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['euler_execution.json','euler_test_review.json','euler_execution_review.md','euler_source_interpretation.json']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('euler-interpreted-community.v1'))")
        ...
        interpreted=report['source_interpretation']['graphs'][0]
        prior=db.execute('SELECT details FROM artifact_audit_evidence WHERE evidence_id=%s AND version_id=%s AND NOT passed FOR SHARE',(interpreted['prior_failed_evidence_id'],SOURCE_VERSION)).fetchone()
        if not prior or _digest(prior['details'])!=interpreted['prior_evidence_sha256']:raise ValueError('Prior source evidence differs')
        graph_id=uuid5(source['artifact_id'],'euler-interpreted-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.euler_interpreted_execution'
        ...
        description='Evaluate the fixed exact Euler identity at pi and return its symbolic proof certificate. Automated Tier 3 with explicit reviewed constant interpretation; no literal source-AST parity claim.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'proof'},dict(version_id=version_id,node_id='proof',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='euler-interpreted-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['certificate']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory source provenance under explicit constant interpretation; not an invoked dependency or literal source-parity claim.'))})
        title='NIST Digital Library of Mathematical Functions: Equation 4.14.3'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Exact complex exponential and trigonometric identity supporting the root Euler relation; automated interpreted-proof review.'})
            ...
            evidence_id=uuid5(selected,'euler-interpreted-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='euler-interpreted-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,source_interpretation=report['source_interpretation'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'euler-exact-semantics.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='No-input exact identity proof: I imaginary unit, pi exact and initial angle real and dimensionless. No arbitrary symbol coercion or physical model.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
        for selected in [spec.version_id,version_id]:
            if db.execute('SELECT count(*) AS n FROM artifact_io_specs WHERE version_id=%s',(selected,)).fetchone()['n']!=1:
                raise ValueError('Unexpected canonical ports')
        if db.execute('SELECT count(*) AS n FROM atom_io_specs WHERE version_id=%s',(spec.version_id,)).fetchone()['n']!=1:
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=0,top_level_output_arity=1,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
