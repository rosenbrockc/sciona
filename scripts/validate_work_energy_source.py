#!/usr/bin/env python3
"""Recover missing work_energy equations from pinned public Cypher source."""
import argparse
import hashlib
import json
...
from sciona.physics_ingest.work_energy_proof import SOURCE_VERSION, SOURCE_HASH, build_proof, verify_proof
from sciona.ghost.dimensions import DimensionalSignature

EXPECTED = {'1590774089': ('dW', 'F dx'), '3253234559': ('x', '\\frac{v_2^2 - v_1^2}{2 a}'), '3360172339': ('W', 'KE_2 - KE_1'), '3512166162': ('W', 'F x'), '4811121942': ('W', '\\frac{1}{2} m v_2^2 - \\frac{1}{2} m v_1^2'), '4928007622': ('KE_1', '\\frac{1}{2} m v_1^2'), '5345738321': ('F', 'm a'), '5542528160': ('\\int dW', 'F \\int_0^x dx'), '5611024898': ('d', '\\frac{1}{2 a} (v^2 - v_0^2)'), '7676652285': ('KE_2', '\\frac{1}{2} m v_2^2'), '8953094349': ('W', 'm a x'), '9413699705': ('W', 'm a \\frac{v_2^2 - v_1^2}{2 a}')}
...
def validate(root, symbol_file, rule_file, expression_file):
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if [n['name'] for n in nodes] != ['integrate','evaluate definite integral','substitute LHS of expr 1 into expr 2','change three variables in expr','substitute LHS of expr 1 into expr 2','simplify','substitute LHS of two expressions into expr']:
        ...
        if len(bindings) != 18 or any(b['status'] != 'active' for b in bindings):
            raise ValueError('Source bindings differ')
        if {b['bound_artifact_fqdn'].rsplit('.', 1)[1] for b in bindings} != set(EXPECTED):
            raise ValueError('Twelve exact source identities required')
        expected_steps = [
            ('111408',{'1590774089','5542528160'},[]),
            ('111662',{'5542528160','3512166162'},[]),
            ('111556',{'3512166162','5345738321','8953094349'},[]),
            ('111236',{'5611024898','3253234559'},["Symbol('pdg0004037')","Symbol('pdg0004037')","Symbol('pdg0001357')","Symbol('pdg0004770')","Symbol('pdg0001357')","Symbol('pdg0002473')"]),
            ('111556',{'3253234559','8953094349','9413699705'},[]),
            ('111457',{'9413699705','4811121942'},[]),
            ('111732',{'7676652285','4928007622','4811121942','3360172339'},[]),
        ]
        for node,(rule,identities,feeds) in zip(nodes,expected_steps):
            ...
            if signature['inference_rule_id']!=rule or actual!=identities or [f['sympy'] for f in signature['variable_bindings']['feeds']]!=feeds:
                raise ValueError('Per-step rule, bindings or original feeds differ')
        for b in bindings:
        ...
        if set(snapshots) != {'1590774089','3253234559','3360172339','3512166162','4811121942','4928007622','5542528160','7676652285','8953094349','9413699705'} or not pins or any(p != pins[0] for p in pins):
    ...
    energy=DimensionalSignature(M=1,L=2,T=-2);length=DimensionalSignature(L=1)
    velocity=DimensionalSignature(L=1,T=-1)
    scalar_expectations={
        'pdg0009398':('dW',energy),'pdg0009199':('dx',length),'pdg0006789':('W',energy),
        'pdg0004202':('F',DimensionalSignature(M=1,L=1,T=-2)),
        'pdg0005156':('m',DimensionalSignature(M=1)),
        'pdg0009140':('a',DimensionalSignature(L=1,T=-2)),
        'pdg0001943':('d',length),'pdg0004037':('x',length),
        'pdg0001357':('v',velocity),'pdg0005153':('v_0',velocity),
        'pdg0002473':('v_1',velocity),'pdg0004770':('v_2',velocity),
        'pdg0001352':('KE_2',energy),'pdg0001955':('KE_1',energy),
    }
    for identity,(latex,dimension) in scalar_expectations.items():
        if definitions[identity].latex!=latex or definitions[identity].dimension!=dimension:
            raise ValueError('Reviewed symbol identity/dimension differs')
    if definitions['pdg0004202'].dimension.multiply(length)!=energy or definitions['pdg0005156'].dimension.multiply(velocity.power(2))!=energy:
        raise ValueError('Work and kinetic-energy dimensional balance differs')
    ...
    return dict(approved=False, source_version_id=SOURCE_VERSION, source_content_hash=SOURCE_HASH,
                stored_source_equations=10, recovered_missing_equations=2, source_records=records, stored_evidence=snapshots,
                ...
                corrections=['Define accumulated work relative to zero at initial displacement; use definite integration with distinct dummy variables.',
                             'Correct simultaneous renaming d->x, v->v2, v0->v1; source feeds incorrectly use x for d and repeat v for v0.',
                             'Constant net force, positive constant mass and 1D signed motion explicit.',
                             'Divided-acceleration source branch requires nonzero a; separately prove a=0 through trajectory identity.'],
                implementation_sha256={p: hashlib.sha256((root/p).read_bytes()).hexdigest() for p in
                    ['scripts/validate_work_energy_source.py','sciona/physics_ingest/work_energy_proof.py']})
