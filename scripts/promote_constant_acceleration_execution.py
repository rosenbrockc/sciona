#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected constant_acceleration realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.constant_acceleration_execution import build_constant_acceleration_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_constant_acceleration_execution import validate
...
LIMITATIONS=[
    ...
    'Finite signed one-dimensional initial velocity and constant acceleration; nonnegative elapsed time.',
    'Signed displacement is not path length during reversal; final velocity retains sign.',
    'At zero time displacement is zero and average velocity is the continuous extension initial velocity.',
    'Zero acceleration supported by polynomial integration independently of source quotient branches.',
    'Exact rational arithmetic on converted float64 inputs; zeros/subnormals accepted, nonzero underflow and overflow rejected.',
    'No variable acceleration, inverse square-root sign selection or trajectory collision guarantee.',
    'Damaged source squared-velocity AST restored; source quotient domains explicit; no literal source AST parity.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    tests=json.loads((root/'docs/reviews/constant_acceleration_test_review.json').read_text())
    if tests['test_results']!=dict(tests=49,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    graph=build_constant_acceleration_execution();digest,nodes,edges=encode_execution_graph(graph)
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['constant_acceleration_execution.json','constant_acceleration_test_review.json','constant_acceleration_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('constant_acceleration-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'constant_acceleration-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.constant_acceleration_corrected_execution'
        ...
        description='Compute signed final velocity, displacement and average velocity for constant acceleration, including zero time and reversal. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'motion'},dict(version_id=version_id,node_id='motion',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='constant_acceleration-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['final_velocity','displacement','average_velocity']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; damaged squared-velocity AST and quotient domains reconstructed; zero-time/acceleration extensions independently integrated. Original draft unchanged.'))})
        title='OpenStax University Physics Volume1: Motion with Constant Acceleration'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Signed constant-acceleration kinematic relations and endpoint mean velocity.'})
            ...
            evidence_id=uuid5(selected,'constant_acceleration-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='constant_acceleration-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'constant-acceleration-signed-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Finite signed initial velocity and constant acceleration with nonnegative elapsed time. Signed displacement, not path length. Zero-duration average is the continuous extension initial velocity; no inverse sign or variable-acceleration claim.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
