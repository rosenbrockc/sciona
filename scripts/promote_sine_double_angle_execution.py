#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected sine_double_angle realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from scripts.validate_sine_double_angle_execution import validate

REFERENCE='openstax-sine-double-angle-community'
URL='https://openstax.org/books/precalculus-2e/pages/a-basic-functions-and-identities'
LIMITATIONS=[
    'Automated Tier 3 only; no human-reviewed Tier 1 or Tier 2 usage claim.',
    'Real dimensionless angles in radians; no degree or complex-input interpretation.',
    'Exact imaginary-unit source identity interpreted as I despite source variable classification; no arbitrary name coercion.',
    'Nonempty finite real inputs converted to float64, including scalar arrays; shape and inputs preserved.',
    'Product identity evaluated with invocation-local mpmath 450-digit precision; no float64 angle doubling or argument clipping.',
    'Subnormal outputs supported; nonzero underflow to zero rejected. Accuracy supported by retained corpus, not a universal correctly-rounded proof.',
    'Requires precision optional dependency mpmath 1.3.0. No phase-unwrapping, interval arithmetic or throughput claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    tests=json.loads((root/'docs/reviews/sine_double_angle_test_review.json').read_text())
    ...
    graph=build_sine_double_angle_execution();digest,nodes,edges=encode_execution_graph(graph)
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['sine_double_angle_execution.json','sine_double_angle_test_review.json','sine_double_angle_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('sine_double_angle-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'sine_double_angle-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.sine_double_angle_corrected_execution'
        ...
        description='Evaluate sin(2*x) through the product identity for real radians. All six source derivation steps verified under exact imaginary-unit interpretation. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'double_angle'},dict(version_id=version_id,node_id='double_angle',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='sine_double_angle-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['double_angle_sine']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; exact imaginary unit explicitly interpreted, all six steps checked. Original draft unchanged.'))})
        title='OpenStax Precalculus 2e: Basic Functions and Identities'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Lists the sine double-angle identity; the source exponential derivation and independent angle-addition check establish the same endpoint.'})
