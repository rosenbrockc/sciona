"""Atomically publish the source-compared Porto realization at automated Tier 3.
...
RUNNER = 'porto-execution-community.v1'
from scripts.review_porto_execution import LIMITATIONS
...
def review(root):
    from scripts.review_porto_execution import audit
    import sciona.atoms.ml.porto_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_porto_semantic_review.json').read_text())
    ...
    graph=build_porto_graph()
    digest,nodes,edges=encode_execution_graph(graph)
    full=json.loads((reviews/'competition_porto_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_porto_review_gates.json').read_text())
    ...
    for name in ('competition_porto_semantic_review.json','competition_porto_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_porto_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_porto_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_porto_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if {n.node_id for n in original.nodes} != {'denoising_autoencoder_features', 'missing_value_imputation', 'rank_average_ensemble', 'model_training', 'feature_concatenation'}:
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community independent Porto CPU ensemble: five swap-noise DAE neural models and one raw-prepared LightGBM with equal probability averaging. Synthetic qualification; historical configurations and competitive accuracy unqualified.',
        ...
        references=[]
        for reference,url,title in [('porto-winner-method-44629','https://www.kaggle.com/competitions/porto-seguro-safe-driver-prediction/writeups/michael-jahrer-1st-place-with-representation-learn','Porto winner representation-learning ensemble')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Primary provenance for swap-noise denoising autoencoders, neural hidden-feature models, raw-prepared LightGBM and equal probability averaging. Independent CPU implementation; historical settings and competitive accuracy remain unqualified.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_porto_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_porto_publication_gates.json':sha(gates_path)})
