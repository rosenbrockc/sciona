"""Atomically publish the source-compared Physical Operator realization at automated Tier 3.
...
RUNNER = 'physical_operator-execution-community.v1'
from scripts.review_physical_operator_execution import LIMITATIONS
...
def review(root):
    from scripts.review_physical_operator_execution import audit
    import sciona.atoms.ml.physical_operator_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_physical_operator_semantic_review.json').read_text())
    ...
    full=json.loads((reviews/'competition_physical_operator_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_physical_operator_review_gates.json').read_text())
    ...
    for name in ('competition_physical_operator_semantic_review.json','competition_physical_operator_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_physical_operator_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_physical_operator_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_physical_operator_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community periodic scalar diffusion pipeline with SI state assembly, learned Fourier operator, validation checkpoint selection, nonnegative mass projection and conservative smoothing. Synthetic evidence; no arbitrary PDE or historical benchmark qualification.',
        ...
        references=[]
        for reference,url,title in [('physical-operator-fno-paper','https://arxiv.org/abs/2010.08895','Fourier Neural Operator for Parametric Partial Differential Equations')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Conceptual Fourier-space learned operator provenance. Independent project architecture and synthetic periodic diffusion checks; no paper benchmark reproduction.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_physical_operator_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_physical_operator_publication_gates.json':sha(gates_path)})
