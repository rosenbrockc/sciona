#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected sound_scale realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.sound_scale_execution import build_sound_scale_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_sound_scale_execution import validate

REFERENCE='trachenko-sound-speed-2020'
URL='https://ccmmp.ph.qmul.ac.uk/~kostya/Speed%20of%20sound.pdf'
LIMITATIONS=[
    'Automated Tier3; no human-reviewed Tier1 designation.',
    'Conditional Rydberg bonding-energy scale and isotropic elasticity; not universal material-speed certification.',
    'A>=1, positive constants and bulk prefactor, nonnegative shear-to-bulk ratio; identical nonempty shapes.',
    'Upper scale bounds only the unit-prefactor bulk-only scaling curve; diagnostic models may exceed it.',
    'Retained prefactor and shear diagnostics expose approximations; material applicability remains caller-established.',
    'Independent float64 rounding with local450digit arithmetic; positive subnormals accepted, underflow0 and overflow rejected.',
    'Corrected factor2 and source symbol interpretation; no literal source parity claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/sound_scale_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/sound_scale_test_review.json').read_text())
    if tests['test_results']!=dict(tests=43,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['sound_scale_execution.json','sound_scale_test_review.json','sound_scale_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('sound_scale-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'sound_scale-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.sound_scale_corrected_execution'
        ...
        description='Compute corrected conditional sound-speed scaling, its A=1 scale, and prefactor/shear diagnostics. No universal material-speed bound claim; automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'scaling'},dict(version_id=version_id,node_id='scaling',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='sound_scale-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['speed_estimate','upper_scale','bulk_model_speed','longitudinal_model_speed','shear_factor']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; factor2 restored and approximation boundaries retained. Original draft unchanged.'))})
        title='Trachenko et al. (2020): Speed of sound from fundamental physical constants'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Equations4and9 retain factor2; bonding-energy and prefactor approximations limit physical interpretation.'})
            ...
            evidence_id=uuid5(selected,'sound_scale-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='sound_scale-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'sound-scale-conditional-model-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Conditional isotropic Rydberg bonding-energy model, A>=1, positive constants/prefactor and nonnegative G/K. Upper scale bounds only the unit-prefactor scaling curve; other outputs may exceed it. All outputs positive and representable; no universal physical bound.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
        for selected in [spec.version_id,version_id]:
            if db.execute('SELECT count(*) AS n FROM artifact_io_specs WHERE version_id=%s',(selected,)).fetchone()['n']!=12:
                raise ValueError('Unexpected canonical ports')
        if db.execute('SELECT count(*) AS n FROM atom_io_specs WHERE version_id=%s',(spec.version_id,)).fetchone()['n']!=12:
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=7,top_level_output_arity=5,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
