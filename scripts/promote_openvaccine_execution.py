"""Atomically publish the source-compared OpenVaccine realization at automated Tier 3.
...
from scripts.review_openvaccine_execution import SOURCES, COMMITS, LIMITATIONS
SOURCE_VERSION = SOURCES[0]['version_id']
SOURCE_HASH = SOURCES[0]['content_hash']
RUNNER = 'openvaccine-execution-community.v1'
...
def review(root):
    from scripts.review_openvaccine_execution import audit
    import sciona.atoms.ml.openvaccine_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_openvaccine_semantic_review.json').read_text())
    ...
    graph=build_openvaccine_graph()
    digest,nodes,edges=encode_execution_graph(graph)
    full=json.loads((reviews/'competition_openvaccine_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_openvaccine_review_gates.json').read_text())
    if gates['result']!='passed' or gates['checks']['injected_failures_rejected']!=13:
        raise ValueError('Negative semantic gates missing')
    check_hashes(root,gates['sha256'])
    evidence=dict(semantic['evidence_sha256'])
    for name in ('competition_openvaccine_semantic_review.json','competition_openvaccine_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_openvaccine_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_openvaccine_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    publication_gate_hash=sha(gates_path)
    evidence=dict(evidence,**{'competition_openvaccine_publication_gates.json':publication_gate_hash})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        for expected in SOURCES:
            item=db.execute('SELECT a.artifact_id,a.fqdn,a.status,a.is_publishable,v.content_hash FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s FOR SHARE OF a,v', (expected['version_id'],)).fetchone()
            if not item or item['content_hash']!=expected['content_hash'] or item['status']!='draft' or item['is_publishable']:
                raise ValueError('Original source identity/state differs')
            original=load_competition_graph(db,version_id=expected['version_id'])
            if len(original.nodes)!=expected['stages']:raise ValueError('Original source stage inventory differs')
            sources.append(item)
        source=sources[0]
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community corrected DasLab OpenVaccine reconstruction: pinned folding/features, shared autoencoder pretraining, twenty TensorFlow models with explicit splits, supervised/pseudo-label refinement, validation rollback and checkpoint-backed reverse-averaged prediction. Automated synthetic evidence; no original final-blend or historical accuracy claim',
        ...
        for source,expected in zip(sources,SOURCES):
            dependency=dict(dependent_version_id=version_id,dependency_artifact_fqdn=source['fqdn'],dependency_content_hash=expected['content_hash'],port_name='')
        ...
        references=[]
        for key,repository,title in [
            ('reconstruction','DasLab/KaggleOpenVaccine','Pinned DasLab OpenVaccine reconstruction (MIT)'),
            ('folding_engine','csfoo/contrafold-se','Pinned CONTRAfold-SE folding software (BSD notices retained)'),
            ('folding_parameters','WaymentSteeleLab/EternaFold','Pinned EternaFold coefficients (separate parameter BSD license retained)')]:
            reference='openvaccine-'+key+'-'+COMMITS[key][:12]
            url='https://github.com/'+repository+'/tree/'+COMMITS[key]
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Pinned OpenVaccine reconstruction, folding software and separately licensed coefficients; retained notices, explicit corrections and source-bound synthetic lifecycle evidence.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                execution_graph_sha256=digest, source_versions=SOURCES,
                source_commits=COMMITS, evidence_sha256=evidence, limitations=LIMITATIONS,
                implementation_sha256=json.loads((root / 'docs/reviews/competition_openvaccine_graph_execution.json').read_text())['code_sha256'],
        ...
        for table, field, selected, expected in [('artifact_cdg_bindings', 'version_id', version_id, 2),
                ('artifact_dependencies', 'dependent_version_id', version_id, 2)]:
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_openvaccine_publication_gates.json':sha(gates_path)})
