"""Read back served cornell versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_cornell_graph_execution as comparison


def verify(root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_cornell_graph_execution.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('5326bafb-d26f-5202-85c9-6f903df0fc77',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='cornell-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='cornell-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='0e75554c472b820a39e97f3807ebc13ebd7d915af068b36ca4d364997b9b1690', dependency_role='cdg', optional=False)]
    ...
    evidence_path=root/'docs/reviews/competition_cornell_graph_execution.json'
    ...
    with tempfile.TemporaryDirectory(prefix='sciona_cornell_served_synthetic_') as temp:
        with patch.dict(os.environ,{'SCIONA_CORNELL_SOURCE_DIR':os.environ.get('SCIONA_CORNELL_SOURCE_DIR','/private/tmp/sciona_cornell_source'),'SCIONA_CORNELL_DEPENDENCY_DIR':os.environ.get('SCIONA_CORNELL_DEPENDENCY_DIR','/private/tmp/sciona_cornell_dependencies/audiomentations')}), patch.object(comparison.runner,'RUNS_DIR',Path(temp)), patch.object(comparison.runner,'save_intermediate_value',side_effect=capture):
            status=asyncio.run(comparison.runner.CDGExecutionSession(None,'synthetic-cornell-served','case').execute({'payload':payload},cdg=graph))
    ...
    assert len(result['window_votes'])==6 and len(result['whole_record_decisions'])==264
    assert evidence_path.read_bytes()==before
    from scripts.promote_cornell_execution import review
    review(root)
    result=expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=2,
        served_cdg_count=served, catalog_graph_runtime_execution='passed', serialized_graph_cases=1, full_model_lifecycles=13, training_phases=17, continuation_phases=4,
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_cornell_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
