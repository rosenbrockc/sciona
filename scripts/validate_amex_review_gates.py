"""Reject weakened Amex evidence without changing evidence files or catalog."""
import hashlib
import json
from pathlib import Path
from unittest.mock import patch
import review_amex_execution as review
...
def main():
    ...
    def failure(suffix,name,mutate):
        target=f'competition_amex_{suffix}.json'
    ...
    failure('execution_source_review','wrong_blend',lambda d:d['stage_mapping'].__setitem__('blending','rank average'))
    failure('metric_comparison','missing_metric_comparison',lambda d:d.__setitem__('comparison_cases',0))
    failure('component_tests','missing_tests',lambda d:d.__setitem__('tests_passed',0))
    failure('tree_training','reduced_training_limits',lambda d:d.__setitem__('configured_rounds',1))
    ...
    paths=['scripts/review_amex_execution.py','scripts/validate_amex_review_gates.py']
    report=dict(format='amex-review-negative-gates.v1',result='passed',catalog_mutations=0,
    ...
    (review.ROOT/'docs/reviews/competition_amex_review_gates.json').write_text(json.dumps(report,indent=2)+'\n')
