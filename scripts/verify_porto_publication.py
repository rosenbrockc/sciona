"""Read back served porto versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_porto_graph_execution as comparison


def verify(root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_porto_graph_execution.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('ad3239bf-5a38-565a-81da-3d3cba9a02b0',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='porto-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='porto-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='8b42f3175fa6408c91c325e24784554cc480637a33673bd7a2128e2e657b8dd8', dependency_role='cdg', optional=False)]
    ...
    evidence_path=root/'docs/reviews/competition_porto_graph_execution.json'
    ...
    with tempfile.TemporaryDirectory(prefix='sciona_porto_served_synthetic_') as temp:
        with patch.object(comparison.runner,'RUNS_DIR',Path(temp)), patch.object(comparison.runner,'save_intermediate_value',side_effect=capture):
            status=asyncio.run(comparison.runner.CDGExecutionSession(None,'synthetic-porto-served','case').execute({'payload':payload},cdg=graph))
    assert status['status']=='completed'
    result=json.loads(json.dumps(captured['result'],allow_nan=False))
    import numpy as np
    scores=np.asarray(result['probabilities'])
    assert scores.shape==(8,) and np.isfinite(scores).all() and ((scores>=0)&(scores<=1)).all()
    assert result['models']==6 and result['dae_models']==5
    assert result['learned_widths']==[30,8,25,7,45] and result['score_kind']=='equal_probability_mean'
    assert np.mean((scores>=.5)==(np.arange(8)>=4))>=.875
    assert evidence_path.read_bytes()==before
    from scripts.promote_porto_execution import review
    review(root)
    result=expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=2,
        served_cdg_count=served, catalog_graph_runtime_execution='passed', serialized_graph_cases=1, models=6,dae_models=5,synthetic_query_rows=8,
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_porto_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
