"""Atomically publish the source-compared Contrails realization at automated Tier 3.
...
SOURCE_VERSION = 'dd8cc8a7-162b-52f1-a74d-256ce661f7ba'
SOURCE_HASH = '6f8962c26d57adbdead9c31a1ed95d7bc31d66bf4e042f5da564d0c8c6d23dee'
COMMIT = '08a15beb36f9cbed4c3990e74c625c1332b61fe8'
RUNNER = 'contrails-execution-community.v1'
LIMITATIONS = [
    'Corrected full two-branch MaxViT/U-Net implementation; original generic temporal-TTA description is not source-faithful.',
    'Shifted labels, augmentation masking, skipped decoder block, validation-break timing and terminal-checkpoint selection are preserved.',
    'Unnormalized fold/branch probability sum and strict0.5 final threshold; threshold sweep is diagnostic only.',
    'Source-sized arrays replace raw file codecs. Caller supplies the selected times/channels and explicit checkpoint provenance.',
    'CPU/current dependencies; no historical binary, seeded-training or competition accuracy equivalence claim.',
    'Explicit random initialization is execution evidence only. Encoder/full-model initial states must be supplied for pretrained use.',
    'Actual serialized full-model v43 evidence uses all4folds,one epoch,batch1/workers0; default35/40epoch schedules not benchmarked.',
    'v47 all10fold orchestration tested with stand-ins; the same full computational branches have real execution evidence.',
    'Tier3 automated Community approval; Tier2 community usage and Tier1 human certification not established.',
    'No observations, model weights or real templated dataset metadata bundled.'
]
...
COMPONENT_CHECKS = {
    'preprocessing': {'color_source_cases': 4, 'tta_source_cases': 40, 'tta_independent_oracles': 40, 'invalid_inputs_rejected': 6},
    'temporal': {'exact_panel_source_cases': 40},
    'losses': {'exact_source_value_and_gradient_cases': 12, 'independent_pytest_cases': 10},
    'decoders': {'forward_cases': 8, 'input_gradient_cases': 40, 'parameter_gradient_cases': 240},
    'scheduler': {'independent_schedule_points_two_parameter_groups': 16, 'final_branch_training_loop_ast_equal': True},
    'population': {'independent_fold_permutation_cases': 12, 'invalid_population_cases': 4},
    'prediction': {'preparation_cases': 4, 'ensemble_cases': 4, 'rle_cases': 3},
    'evaluation': {'exact_source_metric_cases': 6, 'undefined_score_rejection_and_mode_restoration': True},
    'checkpoints': {'full_model_restore': True, 'feature_encoder_restore': True, 'snapshot_independence': True, 'invalid_checkpoints_rejected_before_copy': 4},
    'fold_contract': {'source_score_ast_equal': True, 'independent_fold_tests': 2},
}
EXTRA_EVIDENCE = ['epoch', 'preparation', 'model_execution', 'model_source_parity',
                  'runtime_contract', 'source_pins', 'configuration_pins', 'notebook_pins',
                  'scope_review', 'graph']


def validate_component_evidence(root, full, semantic):
    reviews = root / 'docs/reviews'
    names = {'competition_contrails_' + s + '.json' for s in [*COMPONENT_CHECKS, *EXTRA_EVIDENCE]}
    if set(semantic['evidence_sha256']) != names:
        raise ValueError('Incomplete semantic evidence manifest')
    required_code = {str(p.relative_to(root)) for p in (root / 'sciona').glob('contrails_*.py')}
    if not required_code <= set(full['hashes']):
        raise ValueError('Incomplete current implementation hash coverage')
    check_hashes(root, full['hashes'])
    documents = {}
    for name in names:
        doc = json.loads((reviews / name).read_text())
        documents[name.removeprefix('competition_contrails_').removesuffix('.json')] = doc
        if doc.get('hashes'):
            check_hashes(root, doc['hashes'])
    for suffix, expected in COMPONENT_CHECKS.items():
        if any(documents[suffix]['checks'].get(key) != value for key, value in expected.items()):
            raise ValueError('Component coverage differs: ' + suffix)
    singles = {'preprocessing': 'preprocessing', 'temporal': 'temporal', 'losses': 'losses',
               'scheduler': 'scheduler', 'prediction': 'prediction', 'evaluation': 'evaluation', 'checkpoints': 'checkpoints'}
    for suffix, module in singles.items():
        if documents[suffix]['implementation_sha256'] != sha(root / ('sciona/contrails_' + module + '.py')):
            raise ValueError('Component implementation drift: ' + suffix)
    if documents['population']['population_sha256'] != sha(root / 'sciona/contrails_population.py'):
        raise ValueError('Population implementation drift')
    if documents['epoch']['exact_epoch_comparisons'] != 10 or documents['preparation']['exact_source_preparation_cases'] != 12:
        raise ValueError('Missing loop/preparation source comparisons')
    cases = documents['model_execution']['cases']
    if len(cases) != 2 or not all(c['full_eight_way_inference'] and c['finite_parameter_gradients'] > 400 for c in cases):
        raise ValueError('Missing full model execution')
    if len(documents['model_source_parity']['checks']) != 4 or not all(c['normalized_ast_equal'] for c in documents['model_source_parity']['checks']):
        raise ValueError('Model source parity absent')
    if documents['runtime_contract'].get('runtime_regression_tests_passed') != 8:
        raise ValueError('Runtime boundary coverage missing')
    pins = documents['source_pins']
    if pins['commit'] != COMMIT or pins['source_version_id'] != SOURCE_VERSION or pins['source_hash'] != SOURCE_HASH or pins['license'] != 'MIT':
        raise ValueError('Source provenance mismatch')
    license_hash = next(f['sha256'] for f in pins['files'] if f['path'] == 'LICENSE')
    if sha(root / 'docs/licenses/Contrails-MIT.txt') != license_hash:
    ...
    if defaults() != json.loads((root / 'sciona/contrails_defaults.json').read_text()):
        raise ValueError('Computational defaults differ')
    package = root.parent / 'sciona-atoms-dl/pyproject.toml'
    if sha(package) != documents['scope_review']['provider_package_sha256']:
        raise ValueError('Provider dependencies changed')
    evidence = {name: sha(reviews / name) for name in names}
    evidence['competition_contrails_semantic_review.json'] = sha(reviews / 'competition_contrails_semantic_review.json')
    return evidence


def review(root):
    import sciona.atoms.dl.contrails_execution
    graph=build_contrails_graph()
    digest,nodes,edges=encode_execution_graph(graph)
    reviews=root/'docs/reviews'
    full=json.loads((reviews/'competition_contrails_graph.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    expected=dict(provider_contracts=2,serialized_full_model_graphs=1,trained_folds=4,optimizer_updates=12,runner_rejections=1)
    if full['checks']!=expected:raise ValueError('Graph coverage differs')
    semantic=json.loads((reviews/'competition_contrails_semantic_review.json').read_text())
    ...
    evidence = validate_component_evidence(root, full, semantic)
...
def promote(root, apply=False):
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community Contrails execution: shifted targets and augmentation, two MaxViT/U-Net branches, selected fold training, terminal checkpoint spatial inference, unnormalized ensemble and mask encoding. Synthetic evidence; no competition accuracy claim.',
        ...
        reference = 'contrails-contrails-' + COMMIT[:12]
        url = 'https://github.com/junkoda/kaggle_contrails_solution/tree/' + COMMIT
        title = 'Pinned Contrails source implementation (MIT)'
        ...
        for identity, selected, node_id in targets:
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_contrails_graph.json').read_text())['hashes'],
