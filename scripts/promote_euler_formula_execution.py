#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected euler_formula realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.euler_formula_execution import build_euler_formula_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_euler_formula_execution import validate

REFERENCE='mit-1803-general-euler-formula'
URL='https://ocw.mit.edu/courses/18-03-differential-equations-spring-2010/5bd84a2e02dece25f25b5e0606b30392_MIT18_03S10_sup.pdf'
LIMITATIONS=[
    'Automated Tier3 only; no human-reviewed Tier1 or Tier2 claim.',
    'Provably finite real symbolic angle in radians; caller establishes dimensionless units and assumptions.',
    'General real-angle identity from product rule, initial value and FTC; no complex-log branch.',
    'Invalid source logarithm/integration steps replaced, not certified as literal transformations.',
    'Symbolic expressions and proof certificate; no floating-point approximation or complex-angle extension claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/euler_formula_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/euler_formula_test_review.json').read_text())
    ...
    graph=build_euler_formula_execution();digest,nodes,edges=encode_execution_graph(graph)
    if report['graph_digest']!=digest or report['full_runner_cases']!=6 or not report['general_angle_preserved'] or not report['certificate_verified'] or not report['cache_ast_strings_preserved']:raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['euler_formula_execution.json','euler_formula_test_review.json','euler_formula_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('euler_formula-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'euler_formula-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.euler_formula_corrected_execution'
        ...
        description='Construct general real-angle Euler formula and branch-free ODE proof certificate with input specialization. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'formula'},dict(version_id=version_id,node_id='formula',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='euler_formula-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['exponential_srepr','trigonometric_srepr','certificate_json']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; invalid logarithm and omitted integration constant reconstructed by normalized global real-line ODE proof. Original draft unchanged.'))})
        title='MIT18.03: Complex Exponential and Euler Formula'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Complex exponential characterized through differential equation and initial value; general Euler formula.'})
