#!/usr/bin/env python3
"""Intake exact Andriy provider drafts; never approve or enable publication."""
import argparse
...
def intake(root, reference_dir, library, apply=False):
    prerequisites = audit(root, reference_dir, library)
    ...
    references = {name: hashlib.sha256((root/'docs/reviews'/name).read_bytes()).hexdigest()
                  for name in ['andriy_population_parity.json', 'andriy_execution_parity.json',
                    'andriy_r_glm_parity.json', 'andriy_r_svm_parity.json', 'andriy_r_xgb_parity.json',
                    'andriy_raw_small_sample_failure.json']}
    counts = Counter()
    identities = {}
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('andriy-provider-intake.v1'))")
        ...
        for runtime, spec in sorted(specs.items()):
            ...
            evidence_id = uuid5(UUID(str(spec.version_id)), 'andriy-provider-intake.v1')
            counts['integrity_evidence'] += ensure_row(db, 'artifact_audit_evidence', {'evidence_id': evidence_id}, dict(
                ...
                runner_version='andriy-provider-intake.v1', details=Jsonb(dict(
                    runtime_fqdn=runtime, source_sha256=record['provider_sha256'], interfaces=record['interfaces'],
                    reference_report_sha256=references, dependency_runtimes=record['dependencies'],
                    scope='Exact provider identity/interface intake only. Source references and failed small-sample raw run retained. Separate semantic/runtime promotion review required; no publication approval.'))))
    ...
    return dict(applied=apply, counts=dict(counts), providers=len(records), served_providers=0,
        publication='pending_review', execution_prerequisites_complete=prerequisites['component_execution_prerequisites_complete'])
