"""Atomically publish the source-compared Cassava realization at automated Tier 3.
...
from scripts.review_cassava_execution import SOURCE_VERSION, SOURCE_HASH
ROOT = Path(__file__).resolve().parents[1]
LIBRARY_VERSIONS = dict(line.split('==', 1) for line in
    (ROOT / 'requirements/cassava-reference.txt').read_text().splitlines()
    if line and not line.startswith('#'))
RUNNER = 'cassava-execution-community.v1'
EXPECTED_GATES = {
    'semantic_review:proposed_tier', 'semantic_review:verdict',
    'semantic_review:review_source', 'graph_execution_validation:serialized_graph_sha256',
    'evidence_hash', 'installed_dependency',
}
from scripts.review_cassava_execution import LIMITATIONS as CORE_LIMITATIONS
LIMITATIONS = CORE_LIMITATIONS + json.loads(
    (ROOT / 'docs/reviews/competition_cassava_reference_scope_review.json').read_text())['conditions']
...
def review(root):
    from scripts.review_cassava_execution import final_review
    from importlib.metadata import version
    from sciona.atoms.ml import cassava_execution as provider
    expected = final_review()
    reviews = root / 'docs/reviews'
    semantic = json.loads((reviews / 'competition_cassava_semantic_review.json').read_text())
    ...
    if len(LIBRARY_VERSIONS) != 85 or any(version(name) != pin for name,pin in LIBRARY_VERSIONS.items()):
    ...
    full = json.loads((reviews / 'competition_cassava_graph_execution_validation.json').read_text())
    ...
    if len(boundary['input']) != 11 or [p.name for p in boundary['output']] != ['result']:
        raise ValueError('Unexpected execution boundary')
    evidence = {str(p.relative_to(root)):sha(p) for p in [
        reviews/'competition_cassava_semantic_review.json',
        reviews/'competition_cassava_graph_execution_validation.json',
        root/'requirements/cassava-reference.txt']}
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_cassava_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates.get('format')!='cassava-publication-gates.v1'
            ...
            or gates['validator_sha256']!=sha(root/'scripts/validate_cassava_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'docs/reviews/competition_cassava_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if {n.node_id for n in original.nodes} != {'image_augmentation','model_backbone','robust_loss','pseudo_labeling','ensembling'}:
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community corrected Cassava CPU workflow: fivefold ResNeXt, ViT and EfficientNet training, separate 14-epoch B4 refit and frozen CropNet inference. Source-specific losses and final score blend; explicit population corrections and caller-managed reference artifacts. No historical accuracy claim.',
        ...
        references=[]
        for reference,url,title in [('cassava-winner-four-family','https://www.kaggle.com/competitions/cassava-leaf-disease-classification/writeups/golddiggaz-1st-place-solution','Cassava first-place four-family solution')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Winning four-family solution provenance. Corrected CPU reconstruction with disjoint ViT validation, explicit final B4 normalization population and reviewed local model references; historical runtime and score equivalence unproven.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_cassava_graph_execution_validation.json').read_text())['sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'docs/reviews/competition_cassava_publication_gates.json':sha(gates_path)})
