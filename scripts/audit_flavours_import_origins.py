...
def main():
    import scripts.flavours_graph_execution
    import sciona.flavours_pipeline
    import numpy as np
    import xgboost as xgb
    from sciona.visualizer.runner import _ensure_atoms_imported
    _ensure_atoms_imported()
    with xgb.config_context(nthread=1):
        matrix = xgb.DMatrix(np.ones((10,3)), label=np.arange(10)%2, nthread=1)
        model = xgb.train({'nthread':1,'objective':'binary:logistic'}, matrix, 3)
        restored = xgb.Booster(params={'nthread':1})
        restored.load_model(model.save_raw())
        assert np.array_equal(model.predict(matrix), restored.predict(matrix))
    files = {}
    ...
    report = {'approved': False, 'catalog_mutations': 0,
        'scope': 'Shared runner/numerical startup and atom discovery and synthetic XGBoost training/checkpoint replay. Distribution-file attribution; not publisher authentication or exhaustive lazy-execution coverage.',
        'packages': dict(sorted(packages.items())), 'unmatched': unmatched,
        'first_party_module_files': first_party, 'stdlib_module_files': standard,
        'sha256': {'scripts/audit_flavours_import_origins.py': hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}}
    Path('docs/reviews/competition_flavours_import_origins.json').write_text(json.dumps(report, indent=2) + '\n')
