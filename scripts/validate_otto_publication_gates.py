"""Otto publication must reject review/evidence/dependency drift before writes."""
import hashlib
import json
...
from scripts import promote_otto_execution as publication
...
def main():
    ...
    for suffix,field,value in cases:
        target=ROOT/f'docs/reviews/competition_otto_{suffix}.json'
    ...
    with patch.object(publication,'sha',side_effect=lambda p:'invalid' if p.name=='competition_otto_environment_inventory.json' else real_sha(p)):
        try:publication.review(ROOT)
    ...
    report={'format':'otto-publication-gates.v1','result':'passed','rejected_gates':rejected,
            'publisher_sha256':real_sha(ROOT/'scripts/promote_otto_execution.py'),
            'validator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
            'limits':'Six injected incompatible inputs rejected by read-only review; no catalog mutations.'}
    (ROOT/'docs/reviews/competition_otto_publication_gates.json').write_text(json.dumps(report,indent=2)+'\n')
