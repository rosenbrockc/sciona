"""Reject weakened Video multilabel evidence without changing files or catalog."""
import hashlib
import json
from pathlib import Path
from unittest.mock import patch
import review_video_multilabel_execution as review
...
def main():
    ...
    def failure(suffix,name,mutate):
        target=f'competition_video_multilabel_{suffix}.json'
    ...
    paths=['scripts/review_video_multilabel_execution.py','scripts/validate_video_multilabel_review_gates.py']
    report=dict(format='video_multilabel-review-negative-gates.v1',result='passed',
    ...
    (review.ROOT/'docs/reviews/competition_video_multilabel_review_gates.json').write_text(json.dumps(report,indent=2)+'\n')
