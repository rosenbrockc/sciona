"""Validate normalized interval modes and boundary/integral certificate through serialized full runner."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.atoms.physics import dirichlet_mode as provider
from sciona.physics_ingest.dirichlet_mode_execution import SOURCE_VERSION,build_dirichlet_mode_execution
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg
from scripts.validate_dirichlet_mode_source import validate as validate_source
...
async def validate(root,symbol_file,rule_file,expression_file):
    ...
    from tests.physics_ingest.test_dirichlet_mode import cases,check_outputs
    inputs=cases()
    input_names=['width_srepr','mode_index_srepr','phase_srepr']
    names=['wave_srepr','opposite_wave_srepr','wavenumber_srepr','derivative_srepr','second_derivative_srepr','certificate_json']
    with tempfile.TemporaryDirectory(prefix='sciona-dirichlet-mode-') as directory:
        prior=runner.RUNS_DIR;runner.RUNS_DIR=Path(directory)
        try:
            for i,args in enumerate(inputs):
                run='case-'+str(i)
                result=await runner.CDGExecutionSession(None,'synthetic-dirichlet-mode',run).execute(dict(zip(input_names,map(sp.srepr,args))),cdg=restored)
                if result['status']!='completed':raise ValueError('Full runner failed')
                outputs=[]
                for name in names:
                    record=json.loads((Path(directory)/run/'mode'/('out_'+name+'.json')).read_text())
                ...
                cache=runner.load_cached_outputs(Path(directory)/run,'mode')
                if any(cache['out_'+n]!=s for n,s in zip(names,outputs)):raise ValueError('Cache changed output')
        finally:runner.RUNS_DIR=prior
    files=['sciona/physics_ingest/dirichlet_mode_execution.py','scripts/validate_dirichlet_mode_execution.py',
           'tests/physics_ingest/test_dirichlet_mode.py','tests/physics_ingest/test_dirichlet_mode_proof.py',
