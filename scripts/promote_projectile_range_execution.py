#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected projectile_range realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.projectile_range_execution import build_projectile_range_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_projectile_range_execution import validate

REFERENCE='openstax-level-ground-projectile-range'
...
LIMITATIONS=[
    ...
    'Fixed positive launch speed, constant positive gravity, no drag, equal launch and landing height.',
    'Angle in [0,float64(pi/2)] radians; zero angle is limiting flight-root extension.',
    'Angles retain actual float64 values: float64(pi/2) has a small positive range, not exact vertical zero.',
    'Maximum range uses exact mathematical pi/4 independently of the rounded returned optimizing angle.',
    'Local450digit arithmetic; nonzero output underflow and overflow reject; no universal correct-rounding claim.',
    'Boundary conditions, reversed source feed and mathematical pi explicitly reconstructed; no literal AST parity.',
    'No unequal-height, drag, curved-earth or variable-gravity optimization.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/projectile_range_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/projectile_range_test_review.json').read_text())
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['projectile_range_execution.json','projectile_range_test_review.json','projectile_range_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('projectile_range-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'projectile_range-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.projectile_range_corrected_execution'
        ...
        description='Compute ideal level-ground projectile flight time, horizontal range and global maximum range at fixed launch speed. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'flight'},dict(version_id=version_id,node_id='flight',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='projectile_range-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['flight_time','horizontal_range','maximizing_angle','maximum_range']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; boundary conditions, reversed angle substitution and nonzero flight-time branch reconstructed. Original draft unchanged.'))})
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Ideal no-drag level-ground projectile flight and fixed-speed maximum range.'})
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=3,top_level_output_arity=4,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
