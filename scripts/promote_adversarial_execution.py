"""Atomically publish the source-compared Adversarial realization at automated Tier 3.
...
SOURCE_VERSION = 'cebbfc7d-e32e-5f52-9ec2-e1de0d955501'
SOURCE_HASH = 'd6b977ae079c82dec43ffdf88f64fb9089c17e587c3ee4c36f5411aa0b28828b'
COMMITS = {'non_targeted':'5c68162e05de7afed2e3d33115df43e2a7a1c3da','targeted':'7da707fe568053a9e2735fcc692c8f0ad7e122a1'}
RUNNER = 'adversarial-execution-community.v1'
LIMITATIONS = ['Automated Tier3 Community execution reconstruction; no Tier1 human certification or Tier2 usage qualification. Original intake remains draft.', 'Intake cites the momentum paper and detailed algorithms but contains no explicit repository reference field. Author repository correspondence is an assessed provenance link, not an invented version-bound catalog URL.', 'Full source-shaped299RGB1001class backbones and auxiliary heads execute. CPU Torch/NCHW operations and initializers adapt unspecified historical TF binaries; numerical reduction and max-pool tie-gradient parity unverified.', 'Exact caller model states or explicit random initialization required. Synthetic tests do not establish pretrained attack effectiveness or historical checkpoint authenticity. Named-tensor conversion is not TensorFlow checkpoint binary decoding.', 'Undefined zero gradient/variance normalization explicitly rejects instead of source NaNs; no stabilizing epsilon. Synthetic padded targeted evidence uses supplied nonzero BN offsets, not a silent initialization change.', 'Source ten-entry zero padding,zero target padding,and real-entry trimming preserved. Evidence executes full10/20/40schedules and padded batches in all branches; real graph runner evidence covers five-model targeted-large.', 'Source SciPy reference imsave stretches per-image min/max and maps constantfloatimages to black. Only normalized outputs have epsilon projection guarantee; saved PNG pixel perturbation bound is NOT guaranteed.', 'TensorFlow1.4/SciPy0.19.1 are explicit reference versions, not asserted competition versions. PNG decoded pixels checked with current Pillow; historical bytes unverified.', 'Sequential per-model VJP recomputation preserves ensemble objective but can change floating summation order. Direct full two-model autograd comparison passed; historical TF whole-ensemble parity unverified.', 'Runtime arrays,states and PNGs are private caller data; only synthetic aggregate evidence retained. No downloads or payload logging. Graph validation captures intermediate persistence only in memory.', 'Installed numerical dependencies pinned; clean installation untested. Matcher supplies broader runner dependencies. RNG checks start after provider discovery; unrelated cold imports may affect NumPy state.']
...
def review(root):
    from scripts.review_adversarial_execution import audit
    import sciona.atoms.dl.adversarial_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_adversarial_semantic_review.json').read_text())
    ...
    full=json.loads((reviews/'competition_adversarial_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_adversarial_review_gates.json').read_text())
    if gates['result']!='passed' or gates['checks']['injected_failures_rejected']!=6:
        raise ValueError('Negative semantic gates missing')
    check_hashes(root,gates['sha256'])
    evidence=dict(semantic['evidence_sha256'])
    for name in ('competition_adversarial_semantic_review.json','competition_adversarial_review_gates.json'):
...
def promote(root, apply=False):
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community momentum adversarial attack: source8/5/2full-model ensembles, main/aux objectives,10/20/40iterations and padded RGB output. Explicit caller states; normalized epsilon projection precedes source PNG minmax scaling, which does not guarantee saved pixel bounds. Synthetic evidence; no pretrained effectiveness claim',
        ...
        references=[]
        for branch,repository in [('non_targeted','Non-Targeted-Adversarial-Attacks'),('targeted','Targeted-Adversarial-Attack')]:
            reference='adversarial-'+branch+'-'+COMMITS[branch][:12]
            url='https://github.com/dongyp13/'+repository+'/tree/'+COMMITS[branch]
            title='Pinned '+branch+' momentum attack software (Apache-2.0)'
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Author source correspondence assessed from cited paper and algorithms; no explicit repository field in original intake. Pinned software; automated synthetic evidence with documented adaptations.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                source_commits=COMMITS, evidence_sha256=evidence, limitations=LIMITATIONS,
                implementation_sha256=json.loads((root / 'docs/reviews/competition_adversarial_graph_execution.json').read_text())['sha256'],
