"""Atomically publish the source-compared Amex realization at automated Tier 3.
...
LIBRARY_VERSIONS = {'filelock': '3.25.2', 'fsspec': '2026.2.0', 'jinja2': '3.1.6', 'joblib': '1.5.3', 'lightgbm': '4.7.0', 'markupsafe': '3.0.3', 'mpmath': '1.3.0', 'narwhals': '2.26.0', 'networkx': '3.6.1', 'numpy': '2.4.2', 'pandas': '2.3.3', 'python-dateutil': '2.9.0.post0', 'pytz': '2026.1.post1', 'scikit-learn': '1.8.0', 'scipy': '1.17.1', 'setuptools': '81.0.0', 'six': '1.17.0', 'sympy': '1.14.0', 'threadpoolctl': '3.6.0', 'torch': '2.11.0', 'typing-extensions': '4.15.0', 'tzdata': '2025.3'}
RUNNER = 'amex-execution-community.v1'
from scripts.review_amex_execution import LIMITATIONS
...
def review(root):
    from scripts.review_amex_execution import audit
    import sciona.atoms.ml.amex_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_amex_semantic_review.json').read_text())
    ...
    graph=build_amex_graph()
    digest,nodes,edges=encode_execution_graph(graph)
    full=json.loads((reviews/'competition_amex_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_amex_review_gates.json').read_text())
    ...
    for name in ('competition_amex_semantic_review.json','competition_amex_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_amex_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_amex_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_amex_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if {n.node_id for n in original.nodes} != {'memory_optimization', 'time_series_aggregation', 'difference_features', 'model_training', 'blending'}:
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community independent Amex CPU pipeline: raw preprocessing, manual and sequence features, 15 cross-fitted DART models, 10 neural models, and literal four-way blend. Synthetic qualification; no historical parity or competitive accuracy claim.',
        ...
        references=[]
        for reference,url,title in [('amex-winner-source-03b86f5','https://github.com/jxzly/Kaggle-American-Express-Default-Prediction-1st-solution/tree/03b86f584bb61a34e31eb6343bd33ae753a92c02','Amex winner preprocessing and ensemble software')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Primary provenance for raw preprocessing, manual and sequence features, customer-grouped DART, neural training and literal four-way blending. Independent mathematical reconstruction; original software reuse grant and historical native parity remain unqualified.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_amex_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_amex_publication_gates.json':sha(gates_path)})
