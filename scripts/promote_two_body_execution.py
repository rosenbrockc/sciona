#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected two_body realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.two_body_execution import build_two_body_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_two_body_execution import validate

REFERENCE='openstax-calculus-v3-3-4'
URL='https://openstax.org/books/calculus-volume-3/pages/3-4-motion-in-space'
LIMITATIONS=[
    'Automated Tier 3 only; no Tier 1 certification or Tier 2 usage claim.',
    'Corrected source inference rules and explicit positive domain; no literal source-proof parity claim.',
    'Isolated Newtonian point masses in a circular orbit; r is separation, not either barycentric radius.',
    'Positive finite real float64 inputs, identical nonempty array shapes, positive scalar SI G.',
    'Caller establishes model applicability and SI units; no relativistic or perturbed-orbit correction.',
    'Binary64 pi and per-element Decimal evaluation; subnormals may lose relative precision; no throughput claim.',
]
...
async def promote(root,symbol_file,rule_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/two_body_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/two_body_test_review.json').read_text())
    if tests['test_results']!=dict(tests=68,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    graph=build_two_body_execution();digest,nodes,edges=encode_execution_graph(graph)
    if report['graph_digest']!=digest or report['full_runner_cases']!=5 or report['synthetic_numeric_cases']!=208 or report['maximum_ulp_error']>1:raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['two_body_execution.json','two_body_test_review.json','two_body_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('two_body-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'two_body-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.two_body_corrected_execution'
        ...
        description='Compute circular Newtonian two-body period from positive SI separation, both masses and explicit G. Corrected source derivation with stable high-precision evaluation. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'period'},dict(version_id=version_id,node_id='period',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='two_body-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['period_seconds']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; corrected inference operations and explicit domain are separately reviewed. Not an invoked dependency or source-parity claim.'))})
        title='OpenStax Calculus Volume 3: Motion in Space'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Equation 3.30 total-mass orbital period; circular separation and positive-domain premises separately reviewed.'})
