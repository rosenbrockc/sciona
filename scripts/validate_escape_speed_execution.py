#!/usr/bin/env python3
"""Execute escape threshold through reused infall provider through the serialized CDG runner."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.escape_speed_execution import SOURCE_VERSION,build_escape_speed_execution
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg
from scripts.validate_escape_speed_source import validate as validate_source
from tests.physics_ingest.test_escape_speed_execution import reference, verify_reused_provider


async def validate(root,symbol_file,rule_file,expression_file):
    provider_pin=verify_reused_provider(root)
    ...
    with tempfile.TemporaryDirectory(prefix='sciona-escape-speed-runner-') as directory:
        prior=runner.RUNS_DIR;runner.RUNS_DIR=Path(directory)
        try:
            for i,args in enumerate(cases):
                run='case-'+str(i)
                result=await runner.CDGExecutionSession(None,'synthetic-escape-speed',run).execute(
                ...
                for name,expected in zip(['escape_speed','inward_counterpart_velocity','required_launch_energy','potential_energy'],expected_arrays):
                    actual=np.load(Path(directory)/run/('escape/out_'+name+'.npy'))
                    np.testing.assert_array_equal(actual,expected)
        finally:runner.RUNS_DIR=prior
    files=['sciona/physics_ingest/escape_speed_execution.py','scripts/validate_escape_speed_execution.py',
           'tests/physics_ingest/test_infall_speed.py','tests/physics_ingest/test_escape_speed_proof.py','tests/physics_ingest/test_escape_speed_execution.py','sciona/physics_ingest/infall_speed_proof.py','sciona/services/execution_graph_codec.py','sciona/visualizer/runner.py']
    return dict(approved=False,synthetic_only=True,graph_digest=digest,full_runner_cases=len(cases),synthetic_states=29,
                maximum_ulp_error=0,reused_provider=provider_pin,source_proof=proof,provider_sha256=hashlib.sha256(Path(provider.__file__).read_bytes()).hexdigest(),
