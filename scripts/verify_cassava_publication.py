"""Read back served Cassava versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_cassava_graph_execution as comparison
from scripts import cassava_graph_execution as graph_execution


def verify(root):
    graph_execution.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_cassava_graph_execution_validation.json').read_text())
    semantic = json.loads((root / 'docs/reviews/competition_cassava_semantic_review.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('735c71d1-6f8a-565b-a5c3-f1bcf3a89285',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='cassava-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        def check_review(identity, legacy=False):
            ...
            for table, key in references:
                ...
                assert rows == [dict(ref_id='cassava-winner-four-family',
                    url='https://www.kaggle.com/competitions/cassava-leaf-disease-classification/writeups/golddiggaz-1st-place-solution',
                    verified=True)]
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='cassava-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='c74ce39f2cea94fe168690a36ac59be792dad72a02f705424dea7a8f24cc2218', dependency_role='cdg', optional=False)]
    ...
    evidence_path = root / 'docs/reviews/competition_cassava_graph_execution_validation.json'
    ...
    assert report['passed'] and report['actual_runner_nodes'] == 2
    assert report['serialized_graph_sha256'] == record['content_hash']
    assert report['trainable_model_fits'] == 16 and report['efficientnet_refit_epochs'] == 14
    assert report['torch_checkpoint_prediction_replays_exact'] and report['final_scalar_assembly_exact']
    assert evidence_path.read_bytes() == before
    from scripts.promote_cassava_execution import review
    review(root)
    return dict(read_only=True, synthetic_only=True, trust_tier=3,
        ...
        trainable_model_fits=16, frozen_models=1, graph_sha256=record['content_hash'],
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_cassava_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
