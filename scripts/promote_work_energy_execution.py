#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected work_energy realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.work_energy_execution import build_work_energy_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_work_energy_execution import validate

REFERENCE='openstax-work-energy-community'
URL='https://openstax.org/books/university-physics-volume-1/pages/7-3-work-energy-theorem'
LIMITATIONS=[
    'Automated Tier 3 only; no human-reviewed Tier 1 or Tier 2 usage claim.',
    'Classical constant positive mass and signed one-dimensional endpoint velocities; source constant NET force.',
    'Initial/final kinetic energy and signed accumulated net work in joules; not arbitrary individual-force work, heat, rotational, relativistic or variable-mass energy.',
    'Caller establishes endpoint/force consistency; no trajectory reconstruction or certification.',
    'Explicit source work reference, corrected endpoint renaming and separate zero-acceleration proof; original unchanged.',
    'Matching nonempty finite real float64-compatible shapes; positive mass; exact rational arithmetic before independent output rounding.',
    'All outputs finite representable; exact zeros and subnormals supported, nonzero underflow/overflow rejected. No throughput claim.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/work_energy_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/work_energy_test_review.json').read_text())
    ...
    graph=build_work_energy_execution();digest,nodes,edges=encode_execution_graph(graph)
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['work_energy_execution.json','work_energy_test_review.json','work_energy_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('work_energy-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'work_energy-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.work_energy_corrected_execution'
        ...
        description='Compute endpoint kinetic energies and signed net translational work from constant mass and signed 1D velocities, with exact arithmetic and reviewed seven-step source proof. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'energy'},dict(version_id=version_id,node_id='energy',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='work_energy-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['initial_kinetic_energy','final_kinetic_energy','net_work']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; work reference and endpoint identities reconstructed, nonzero and zero acceleration branches verified. Original draft unchanged.'))})
        title='OpenStax University Physics Volume 1: Work-Energy Theorem'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Establishes that net work on a constant-mass particle equals the change in translational kinetic energy.'})
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=3,top_level_output_arity=3,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
