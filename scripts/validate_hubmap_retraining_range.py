"""Raw labeled and pseudo-labeled slides through source-equivalent retraining."""
import argparse
...
from sciona.hubmap_retraining import run_retraining, prepare_retraining
...
def validate(root,source,library):
    ...
    slides=[]
    groups=['synthetic-train-a','synthetic-train-b','synthetic-valid']
    for i in range(3):
        image=np.random.RandomState(263+i).randint(0,256,(35,37,3)).astype(np.uint8)
        mask=np.zeros((35,37),dtype=np.uint8)
        if i!=1:mask[::3]=1
        slides.append((image,mask))
    # Preparation and partition have their own independent source comparisons.
    # Here the reference consumes that validated boundary, while the candidate
    # performs preparation within its raw training entry point.
    pseudo_image=np.random.RandomState(359).randint(0,256,(35,37,3)).astype(np.uint8)
    pseudo_mask=np.zeros((35,37),dtype=np.uint8);pseudo_mask[::2]=1
    pseudo_sources=[[(pseudo_image,pseudo_mask)]]
    training,validation=prepare_retraining(slides,groups,['synthetic-valid'],pseudo_sources,tile_size=32)
    images=training['images_bgr'];rles=training['rles']
    present=training['present'];bins=training['bins']
    validation_images=validation['images_bgr'];validation_rles=validation['rles']
    ...
    actual=run_retraining(candidate,optimizers[0],schedulers[0],
        slides,groups,['synthetic-valid'],pseudo_sources,tile_size=32,
    ...
    try:
        ...
        frame=pd.DataFrame(dict(is_masked=present,binned=bins,_position=np.arange(len(images))))
        ...
        for epoch in [18,19]:
            ...
            validation=valid_source(reference,batches,len(validation_rles),.5)
    ...
    paths=sorted(root.glob('sciona/hubmap_*.py'))+sorted(root.glob('scripts/validate_hubmap_*.py'))+[
        root/'docs/reviews'/p for p in ['competition_hubmap_source_pins.json','competition_hubmap_loader_pins.json','competition_hubmap_augmentation_pins.json','competition_hubmap_partition_pins.json']]
    return dict(approved=False,synthetic_only=True,epochs=[r['epoch'] for r in expected],
        ...
        limitations=['Initial and retraining source loops are AST-identical as separately verified; source loop consumes pseudo-augmented population here.',
            'Reference starts at separately source-validated preparation/partition boundary; candidate executes raw preparation within training.',
            'Raw synthetic RGB/mask arrays, runtime group labels and explicit initial weights/RNG; TIFF IO and inference remain.',
...
if __name__=='__main__':
    ...
    (root/'docs/reviews/competition_hubmap_retraining_range.json').write_text(json.dumps(report,indent=2)+'\n')
