"""Read back served santander versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_santander_graph_execution as comparison


def verify(root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_santander_graph_execution.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('743c3225-058e-5a7e-ab18-ae5dd1c2b7d4',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='santander-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='santander-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='162915709dcde12f33a88f1d693b169aa8509dbd7e5b29755f89a728d388ee11', dependency_role='cdg', optional=False)]
    ...
    evidence_path=root/'docs/reviews/competition_santander_graph_execution.json'
    ...
    with tempfile.TemporaryDirectory(prefix='sciona_santander_served_synthetic_') as temp:
        with patch.object(comparison.runner,'RUNS_DIR',Path(temp)), patch.object(comparison.runner,'save_intermediate_value',side_effect=capture):
            status=asyncio.run(comparison.runner.CDGExecutionSession(None,'synthetic-santander-served','case').execute({'payload':payload},cdg=graph))
    ...
    assert result['selection_counts']=={'neural':dict(positive=5000,negative=3000,labeled_rows=8060,query_reference_rows=8500),'tree':dict(positive=2700,negative=2000,labeled_rows=4760,query_reference_rows=8500)}
    assert result['score_kind']=='rank_blend' and result['reference_policy']=='retain_selected'
    assert evidence_path.read_bytes()==before
    from scripts.promote_santander_execution import review
    review(root)
    result=expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=2,
        served_cdg_count=served, catalog_graph_runtime_execution='passed', serialized_graph_cases=1, models=30,synthetic_query_rows=8500,full_default_pseudo_counts=True,
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_santander_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
