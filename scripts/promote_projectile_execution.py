#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected projectile realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.projectile_execution import build_projectile_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_projectile_execution import validate
...
LIMITATIONS=[
    ...
    'Ideal no-drag motion with constant downward acceleration in a common Cartesian SI frame; no collision, terrain or varying-gravity handling.',
    'Finite real identical nonempty array shapes, no broadcasting; nonzero horizontal initial velocity, nonnegative gravity and nonnegative elapsed time.',
    'Two missing expression snapshots recovered from the exact ingestion-pinned source file; original source artifacts remain unchanged.',
    'Initial vertical-velocity identity and linear gravity explicitly corrected from reviewed LaTeX and symbol definitions; no source-AST parity claim.',
    'Exact rational arithmetic on converted float64 inputs before independent time/height rounding; height uses exact time.',
    'Exact zeros and subnormal outputs supported; nonzero underflow to zero and overflow rejected.',
    'Generic runner summary statistics can overflow for extreme inputs; saved arrays and computed outputs are independently verified.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    tests=json.loads((root/'docs/reviews/projectile_test_review.json').read_text())
    if tests['test_results']!=dict(tests=26,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    if report['graph_digest']!=digest or report['full_runner_cases']!=6 or report['synthetic_points']!=29 or report['maximum_ulp_error']!=0:raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['projectile_execution.json','projectile_test_review.json','projectile_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('projectile-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'projectile-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.projectile_corrected_execution'
        ...
        description='Evaluate corrected ideal-projectile elapsed time and height at a horizontal coordinate. Exact converted-input arithmetic and reviewed time-elimination proof. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'trajectory'},dict(version_id=version_id,node_id='trajectory',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='projectile-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['elapsed_time','height']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; missing expressions recovered from pinned source and incorrect velocity/gravity AST terms explicitly reconstructed. Original draft remains unchanged.'))})
        title='OpenStax University Physics Volume 1: Projectile Motion'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Time elimination from ideal constant-acceleration projectile kinematics with initial velocity and linear gravity.'})
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=6,top_level_output_arity=2,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
