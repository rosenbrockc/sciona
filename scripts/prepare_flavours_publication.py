...
from scripts.review_flavours_execution import ROOT, SOURCE_VERSION, SOURCE_HASH, STAGE_MAPPING, audit, sha
...
def prepare():
    ...
    return {'approved': False, 'applied': False, 'catalog_mutations': 0,
        ...
        'sha256': {'scripts/prepare_flavours_publication.py': sha(Path(__file__)),
                   'scripts/build_flavours_execution_graph.py': sha(ROOT / 'scripts/build_flavours_execution_graph.py'),
                   'docs/reviews/competition_flavours_source_triage.json': sha(ROOT / 'docs/reviews/competition_flavours_source_triage.json')}}


if __name__ == '__main__':
    plan = prepare()
    (ROOT / 'docs/reviews/competition_flavours_publication_plan.json').write_text(json.dumps(plan, indent=2) + '\n')
