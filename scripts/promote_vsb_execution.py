"""Atomically publish the source-compared VSB realization at automated Tier 3.
...
RUNNER = 'vsb-execution-community.v1'
from scripts.review_vsb_execution import LIMITATIONS
...
def review(root):
    from scripts.review_vsb_execution import audit
    import sciona.atoms.ml.vsb_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_vsb_semantic_review.json').read_text())
    ...
    for name in ('competition_vsb_semantic_review.json','competition_vsb_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_vsb_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_vsb_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_vsb_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if {n.node_id for n in original.nodes} != {'peak_feature_extraction', 'dl_model', 'signal_to_matrix_transform', 'weighted_blend', 'peak_detection', 'wavelet_denoising', 'tabular_model'}:
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community independent VSB CPU pipeline: recursive baseline, directional peaks, phase aggregates, 125 repeated LightGBM fits and signal-level threshold decisions. Synthetic qualification; no untouched-test or historical accuracy claim.',
        ...
        references=[]
        for reference,url,title in [('vsb-winner-notebook-v3','https://www.kaggle.com/code/mark4h/vsb-1st-place-solution','VSB winner signal feature and repeated-tree pipeline')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Primary provenance for recursive baseline removal, custom peaks, phase aggregates, repeated LightGBM and signal-level thresholding. Independent mathematical reconstruction; original notebook reuse grant and historical native parity remain unqualified.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_vsb_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_vsb_publication_gates.json':sha(gates_path)})
