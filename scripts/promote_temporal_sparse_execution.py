"""Atomically publish the source-compared Temporal Sparse realization at automated Tier 3.
...
LIBRARY_VERSIONS = {'joblib': '1.5.3', 'numpy': '2.4.2', 'scikit-learn': '1.8.0', 'scipy': '1.17.1', 'threadpoolctl': '3.6.0'}
RUNNER = 'temporal_sparse-execution-community.v1'
from scripts.review_temporal_sparse_execution import LIMITATIONS
...
def review(root):
    from scripts.review_temporal_sparse_execution import audit
    import sciona.atoms.ml.temporal_sparse_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_temporal_sparse_semantic_review.json').read_text())
    ...
    full=json.loads((reviews/'competition_temporal_sparse_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_temporal_sparse_review_gates.json').read_text())
    ...
    for name in ('competition_temporal_sparse_semantic_review.json','competition_temporal_sparse_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_temporal_sparse_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_temporal_sparse_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_temporal_sparse_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        references=[]
        for reference,url,title in [('temporal-sparse-sgd','https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.SGDRegressor.html','Scikit-learn online sparse SGD regression'),('temporal-sparse-hashing','https://scikit-learn.org/stable/modules/generated/sklearn.feature_extraction.FeatureHasher.html','Scikit-learn fixed-width feature hashing')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Library API provenance for sparse online regression and hashing. Independent causal feature, private stream, frozen validation and inverse-transform execution; synthetic evidence only.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_temporal_sparse_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_temporal_sparse_publication_gates.json':sha(gates_path)})
