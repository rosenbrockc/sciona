"""Read back served TrackML versions and execute the catalog graph against source."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_trackml_lifecycle as comparison


def verify(root, source_root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_trackml_lifecycle.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('11c41ece-c648-50b4-b211-7e319ba36dea',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='trackml-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        assert len(bindings) == len(graph.nodes) == 3
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='trackml-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        assert dependency == [dict(dependency_artifact_fqdn=fqdn.removesuffix('.execution'), dependency_content_hash='c0b732d94cd161a98dfc8a24a70a1805453a3de2d5b4d71ee7719fda4c639183', dependency_role='cdg', optional=False)]
    ...
    with patch.object(comparison, 'build_trackml_graph', return_value=graph):
        result = comparison.validate(root, source_root)
    assert result == expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=3,
        served_cdg_count=served, catalog_graph_source_comparison='passed', serialized_graph_cases=result['checks']['serialized_graph_cases'],
...
if __name__ == '__main__':
    ...
    (root / 'docs/reviews/competition_trackml_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
