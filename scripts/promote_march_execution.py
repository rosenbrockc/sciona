"""Atomically publish the source-compared March realization at automated Tier 3.
...
LIBRARY_VERSIONS = {'scikit-learn':'1.8.0','networkx':'3.6.1'}
RUNNER = 'march-execution-community.v1'
from scripts.review_march_execution import LIMITATIONS
...
def review(root):
    from scripts.review_march_execution import audit
    import sciona.atoms.ml.march_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_march_semantic_review.json').read_text())
    ...
    graph=build_march_graph()
    digest,nodes,edges=encode_execution_graph(graph)
    full=json.loads((reviews/'competition_march_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_march_review_gates.json').read_text())
    ...
    for name in ('competition_march_semantic_review.json','competition_march_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_march_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_march_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_march_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community generic season-statistics, PageRank, chronological Elo and matchup-feature pipeline with logistic fitting, held-out sigmoid calibration and orientation-symmetric probabilities. Synthetic evidence; no historical winning recipe or accuracy claim',
        ...
        for reference,url,title in [
            ('march-sklearn-calibration-1.8.0','https://scikit-learn.org/1.8/modules/generated/sklearn.calibration.CalibratedClassifierCV.html','sklearn held-out calibration documentation (1.8.0)'),
            ('march-networkx-pagerank-3.6.1','https://networkx.org/documentation/stable/reference/algorithms/generated/networkx.algorithms.link_analysis.pagerank_alg.pagerank.html','NetworkX PageRank documentation; reviewed package 3.6.1')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Versioned library behavior supporting the explicit generic matchup pipeline; retained BSD notices and independent synthetic evidence, not a historical winning solution.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_march_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_march_publication_gates.json':sha(gates_path)})
