#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected quadratic realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.quadratic_execution import build_quadratic_execution,PRIMITIVE
from sciona.physics_ingest.quadratic_corrected_proof import SOURCE_VERSION
from scripts.audit_physics_branch_steps import validate as validate_source_diagnostics
SOURCE_HASH='d66ede579e855129e780446bba640b51c5ec10cf481988eca10eadd76629e5d1'
...
from scripts.validate_quadratic_execution import validate
...
LIMITATIONS=[
    'Automated Tier 3 only; no Tier 1 certification or Tier 2 usage claim.',
    'Explicitly corrected derivation; no parity claim with the defective original source proof.',
    'Finite real float64-converted coefficients, identical nonempty shapes, nonzero a and nonnegative discriminant.',
    'Lower then upper root for either sign of a; repeated roots appear twice. No linear fallback or complex-root solution.',
    'Consistent polynomial units and physical applicability are caller responsibilities.',
    'Scalar-per-element reference implementation; no throughput or universal correct-rounding claim.',
]



async def promote(root,symbol_file,rule_file,apply=False):
    report=await validate(root)
    diagnostics=validate_source_diagnostics(root,symbol_file,rule_file)
    if diagnostics!=json.loads((root/'docs/reviews/physics_branch_step_diagnostics.json').read_text()):raise ValueError('Source diagnostics differ')
    retained=json.loads((root/'docs/reviews/quadratic_corrected_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/quadratic_test_review.json').read_text())
    if tests['test_results']!=dict(tests=31,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    directory=root.parent/'sciona-atoms'
    specs=[s for s in _parse_registered_atoms(repo=ProviderRepo('sciona-atoms',directory),artifact_root=directory/'src/sciona/atoms') if s.import_module+'.'+s.source_symbol==PRIMITIVE]
    ...
    if report['graph_digest']!=digest or report['full_runner_cases']!=5 or report['synthetic_polynomials']!=305 or report['maximum_ulp_error']>3:raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['quadratic_corrected_execution.json','quadratic_test_review.json','quadratic_correction_review.md','physics_branch_step_diagnostics.json']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('quadratic-corrected-community.v1'))")
        ...
        failures=db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='pdg-branch-diagnostics.v1' AND NOT passed FOR SHARE",(SOURCE_VERSION,)).fetchall()
        if len(failures)!=1 or failures[0]['details']['graph']!=diagnostics['graphs'][0]:raise ValueError('Verified original defects must remain recorded')
        graph_id=uuid5(source['artifact_id'],'quadratic-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.quadratic_corrected_execution'
        ...
        description='Solve a real quadratic with both ordered roots using an explicitly corrected proof. Exact float64 discriminant classification and high-precision stable root evaluation. Automated Tier 3; no source-parity claim.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'roots'},dict(version_id=version_id,node_id='roots',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='quadratic-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['lower_root','upper_root']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory defective-source provenance; corrected proof supersedes its mathematics only in this new realization. Not an invoked dependency or source-parity claim.'))})
        title='OpenStax Algebra and Trigonometry 2e: Quadratic Equations'
        created+=ensure_row(db,'references_registry',{'ref_id':REFERENCE},dict(ref_id=REFERENCE,ref_type='book',title=title,url=URL))
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Completing the square and the quadratic formula for nonzero leading coefficient; automated corrected-proof review.'})
            ...
            evidence_id=uuid5(selected,'quadratic-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='quadratic-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['corrected_proof'],source_diagnostics=diagnostics,source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'quadratic-real-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Finite real coefficients in consistent units with identical nonempty shapes, a nonzero, discriminant nonnegative; both roots representable as finite float64, including exact zero roots.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
        for selected in [spec.version_id,version_id]:
            if db.execute('SELECT count(*) AS n FROM artifact_io_specs WHERE version_id=%s',(selected,)).fetchone()['n']!=5:
                raise ValueError('Unexpected canonical ports')
        if db.execute('SELECT count(*) AS n FROM atom_io_specs WHERE version_id=%s',(spec.version_id,)).fetchone()['n']!=5:
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=3,top_level_output_arity=2,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
...
if __name__=='__main__':
    ...
    for name in ['symbol-file','rule-file']:parser.add_argument('--'+name,type=Path,required=True)
    parser.add_argument('--apply',action='store_true')
    args=parser.parse_args()
    print(json.dumps(asyncio.run(promote(Path(__file__).resolve().parents[1],args.symbol_file,args.rule_file,args.apply))))
