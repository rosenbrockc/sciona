...
def main():
    ...
    report = {'approved': False, 'all_compared_payloads_match': all(not p['failures'] for p in packages.values()),
        ...
        'sha256': {p: hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in [
            str(audit_path), 'scripts/verify_flavours_publisher_wheels.py', 'requirements/flavours-reference-macos-arm64.lock']}}
    Path('docs/reviews/competition_flavours_publisher_payload_validation.json').write_text(json.dumps(report, indent=2) + '\n')
