#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected free_schrodinger realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.free_schrodinger_execution import build_free_schrodinger_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_free_schrodinger_execution import validate

REFERENCE='openstax-free-schrodinger'
URL='https://openstax.org/books/university-physics-volume-3/pages/7-3-the-schrodinger-equation'
LIMITATIONS=[
    'Automated Tier3; no human-reviewed Tier1 designation.',
    'Three-dimensional Cartesian free nonrelativistic scalar mode, U=0, constant parameters.',
    'Provably positive finite real mass and hbar, finite real momentum Tuple, finite complex amplitude.',
    'Reserved real x,y,z,t coordinates; parameters must not depend on them; conflicting symbol assumptions rejected.',
    'Zero momentum and amplitude allowed; parsed floating constants become exact binary-value rationals.',
    'Wave, energy, gradient, Laplacian and time derivative with seven checked differential/Hamiltonian identities.',
    'Source wavelength inversion, missing hbar power, vector operators, ASTs and bindings reconstructed; no literal parity.',
    'Plane-wave consistency and finite-superposition linearity do not derive quantum dynamics from classical laws.',
    'No infinite-space normalization, arbitrary-wavefunction, boundary, potential, spin or relativistic claim.',
    'Symbolic outputs; caller establishes units and assumptions. No numeric PDE solver or infinite-superposition guarantee.',
]
...
async def promote(root,symbol_file,rule_file,expression_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/free_schrodinger_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/free_schrodinger_test_review.json').read_text())
    ...
    if report['graph_digest']!=digest or report['full_runner_cases']!=6 or not report['general_argument_preserved'] or not report['certificate_verified'] or not report['cache_ast_strings_preserved']:raise ValueError('Execution gates differ')
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['free_schrodinger_execution.json','free_schrodinger_test_review.json','free_schrodinger_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('free_schrodinger-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'free_schrodinger-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.free_schrodinger_corrected_execution'
        ...
        description='Construct a full three-dimensional free-particle wave, energy, gradient, Laplacian, time derivative and checkable Hamiltonian certificate. Automated Tier3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'wave'},dict(version_id=version_id,node_id='wave',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='free_schrodinger-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['wave_srepr','energy_srepr','gradient_json','laplacian_srepr','time_derivative_srepr','certificate_json']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; wavelength reciprocal, hbar-squared Laplacian coefficient, vector derivatives, source ASTs and gradient binding corrected. Free nonrelativistic plane-wave consistency with finite-superposition linearity. Original draft unchanged.'))})
        title='OpenStax University Physics Volume3: The Schrodinger Equation'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Free nonrelativistic Schrodinger equation with zero potential, complex plane-wave solution and linear superposition.'})
            ...
            evidence_id=uuid5(selected,'free_schrodinger-corrected-community.v1')
            created+=ensure_row(db,'artifact_audit_evidence',{'evidence_id':evidence_id},dict(evidence_id=evidence_id,artifact_id=identity,version_id=selected,audit_type='semantic_audit',passed=True,status='completed',source_kind='automated',runner_version='free_schrodinger-corrected-community.v1',details=Jsonb(dict(kind=kind,publication_tier=3,review_source='automated',evidence_sha256=evidence_hashes,provider_version_id=str(spec.version_id),provider_content_hash=spec.content_hash,execution_graph_sha256=digest,source_version_id=SOURCE_VERSION,source_content_hash=SOURCE_HASH,corrected_proof=report['source_proof'],source_parity_claim=False,limitations=LIMITATIONS))))
            bound_id=uuid5(selected,'free-schrodinger-constant-mode-domain.v1')
            created+=ensure_row(db,'artifact_validity_bounds',{'bound_id':bound_id},dict(bound_id=bound_id,artifact_id=identity,version_id=selected,scope='version',bound_kind='regime',validity_statement='Positive constant mass/hbar, constant real three-component momentum and finite complex amplitude; fixed real x,y,z,t. Free nonrelativistic scalar plane wave, zero potential, zero momentum/amplitude supported. Compatible units required; no boundary or infinite-space normalized-state claim.',evidence_ref_key=REFERENCE,review_status='automated_pass',metadata=Jsonb(dict(scope='Automated reviewed domain; caller establishes physical applicability.'))))
        for selected in [spec.version_id,version_id]:
            if db.execute('SELECT count(*) AS n FROM artifact_io_specs WHERE version_id=%s',(selected,)).fetchone()['n']!=10:
                raise ValueError('Unexpected canonical ports')
        if db.execute('SELECT count(*) AS n FROM atom_io_specs WHERE version_id=%s',(spec.version_id,)).fetchone()['n']!=10:
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=4,top_level_output_arity=6,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
