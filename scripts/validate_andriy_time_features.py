...
def main():
    ...
    hashes = json.loads((root/'docs/reviews/andriy_time_source_hashes.json').read_text())
    cases = []
    with TemporaryDirectory(prefix='sciona-andriy-time-') as temporary:
        ...
        (directory/'check.m').write_text("load('input.mat'); result=zeros(rows(windows),10); for k=1:rows(windows); x=windows(k,:)'; [a,m,c]=hjorth(x); [z1,z2,v1,v2]=raw_analysis(x'); result(k,:)=[zero_crossing(x), minmax(x), RMS_amplitude(x), a,m,c,z1,z2,v1,v2]; end; reference_version=version; save('-mat7-binary','output.mat','result','reference_version');\n")
        for seed in [10211,10212,10213]:
            for samples in [17,3840,7680]:
    ...
    report=dict(source_revision='00f937cc7710977dc812d9fc675864e2b8288658',source_hashes=hashes,
        provider_sha256=hashlib.sha256(Path(inspect.getsourcefile(andriy_time_features)).read_bytes()).hexdigest(),
        validation_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        test_sha256=hashlib.sha256((root/'tests/test_andriy_time_features.py').read_bytes()).hexdigest(),
        ...
        limitations=['Unmodified numerical source helpers in current Octave; no original MATLAB runtime claim.',
                     'Ten helper outputs only, not the complete102-feature family; low-range masking and full model execution remain separate.'])
