"""Actual serialized graph execution at complete pretrained training budgets.

Uses the same synthetic population and numerical/checkpoint checks as the
standalone pipeline validator, through the actual runner and both providers.
"""
import os
...
from scripts.cassava_graph_execution import execute_through_graph as execute
...
def main():
    ...
    paths += ['scripts/validate_cassava_graph_execution.py', 'scripts/cassava_graph_execution.py',
              'scripts/build_cassava_execution_graph.py', 'sciona/visualizer/runner.py', 'docs/reviews/competition_cassava_timm_pretrained_validation.json',
              'docs/reviews/competition_cassava_cropnet_artifact.json']
    report = {'approved': False, 'passed': True, 'synthetic_only': True,
        'actual_runner_nodes': 2, 'graph_codec_roundtrip': True,
        'serialized_graph_sha256': encode_execution_graph(build_graph())[0],
    ...
    Path('docs/reviews/competition_cassava_graph_execution_validation.json').write_text(json.dumps(report, indent=2) + '\n')
    print('PASS complete pretrained four-family serialized graph', flush=True)
