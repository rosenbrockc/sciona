#!/usr/bin/env python3
"""Atomically intake and approve the reviewed corrected isobaric_energy realization at Tier 3."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.physics_ingest.isobaric_energy_execution import build_isobaric_energy_execution,PRIMITIVE,SOURCE_VERSION,SOURCE_HASH
...
from scripts.validate_isobaric_energy_execution import validate

REFERENCE='mit-8044-2013-isobaric-internal-energy'
URL='https://ocw.mit.edu/courses/8-044-statistical-physics-i-spring-2013/f7b8910bfe299d327f60ae36e856718c_MIT8_044S13_ps4.pdf'
LIMITATIONS=[
    'Automated Tier 3 only; no human-reviewed Tier 1 or Tier 2 usage claim.',
    'Local derivative dU/dT at fixed pressure, not Cp or dH/dT; mechanical pressure is distinct from internal pressure.',
    'Caller supplies consistent local Cv, internal pressure, positive volume and thermal expansion at the same state and fixed composition/amount.',
    'Differentiable U(T,V) and V(T,p) required; no material equation-of-state or thermodynamic stability inference.',
    'Source internal-pressure units are explicitly corrected from force to pressure using its dU/dV definition; original metadata and ASTs remain unchanged.',
    'Finite real identical nonempty shapes without broadcasting; signed internal pressure and expansion supported.',
    'Exact rational arithmetic on float64-converted inputs before one output rounding; exact zeros/subnormals supported, nonzero underflow to zero and overflow rejected.',
    'No finite-temperature-change integration or throughput claim.',
]
...
async def promote(root,symbol_file,rule_file,apply=False):
    ...
    retained=json.loads((root/'docs/reviews/isobaric_energy_execution.json').read_text())
    if report!=retained:raise ValueError('Retained execution evidence differs from fresh validation')
    tests=json.loads((root/'docs/reviews/isobaric_energy_test_review.json').read_text())
    if tests['test_results']!=dict(tests=27,failures=0,errors=0,skipped=0):raise ValueError('Complete tests required')
    ...
    graph=build_isobaric_energy_execution();digest,nodes,edges=encode_execution_graph(graph)
    ...
    evidence_hashes={p:hashlib.sha256((root/'docs/reviews'/p).read_bytes()).hexdigest() for p in ['isobaric_energy_execution.json','isobaric_energy_test_review.json','isobaric_energy_correction_review.md']}
    ...
    with psycopg.connect(dotenv_values(root/'.env')['SCIONA_DATA_CATALOG_DATABASE_URL'],row_factory=dict_row) as db:
        db.execute("SELECT pg_advisory_xact_lock(hashtext('isobaric_energy-corrected-community.v1'))")
        ...
        graph_id=uuid5(source['artifact_id'],'isobaric_energy-corrected-execution.v1');version_id=uuid5(graph_id,digest);fqdn=source['fqdn']+'.isobaric_energy_corrected_execution'
        ...
        description='Compute the local temperature derivative of internal energy at fixed pressure from Cv, internal pressure, volume and expansion. Corrected pressure dimensions and explicit differential chain rule. Automated Tier 3.'
        ...
        created+=ensure_row(db,'artifact_cdg_bindings',{'version_id':version_id,'node_id':'energy_derivative'},dict(version_id=version_id,node_id='energy_derivative',bound_artifact_fqdn=spec.fqdn,bound_version_content_hash=spec.content_hash,binding_confidence=1.,binding_source='isobaric_energy-corrected-community.v1',status='active',alternatives=Jsonb([]),evidence_summary=Jsonb(dict(runtime_fqdn=PRIMITIVE,provider_version_id=str(spec.version_id),output_aliases_by_ordinal=['internal_energy_temperature_derivative']))))
        ...
        created+=ensure_row(db,'artifact_dependencies',dependency,{**dependency,'dependency_role':'cdg','optional':False,'binding_metadata':Jsonb(dict(scope='Mandatory original-source provenance; differential semantics and internal-pressure dimensions explicitly reconstructed from source definitions. Original draft unchanged.'))})
        title='MIT Statistical Physics I: Heat Capacity at Constant Pressure in a Simple Fluid'
        ...
        for identity,selected,kind in [(atom_id,UUID(str(spec.version_id)),'provider'),(graph_id,version_id,'execution')]:
            ...
            for table,key in tables:
                created+=ensure_row(db,table,{key:identity,'ref_key':REFERENCE},{key:identity,'ref_id':REFERENCE,'ref_key':REFERENCE,'title':title,'url':URL,'source':'llm_extracted','verified':True,'confidence':'high','relevance_note':'Distinguishes dU/dT at constant pressure from Cp, which also includes mechanical expansion work for a simple fluid.'})
        ...
        db.execute("UPDATE artifacts SET status='approved',is_publishable=true,verified_leaf_coverage=1,leaf_count=1,top_level_input_arity=4,top_level_output_arity=1,topo_hash=%s WHERE artifact_id=%s",(topo,graph_id))
