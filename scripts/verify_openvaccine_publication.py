"""Read back served openvaccine versions and execute the catalog graph against the validated derived runtime."""
import argparse
import asyncio
import hashlib
import importlib
...
from scripts import validate_openvaccine_graph_execution as comparison


def verify(root):
    comparison.runner._ensure_atoms_imported()
    expected = json.loads((root / 'docs/reviews/competition_openvaccine_graph_execution.json').read_text())

    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
            options='-c default_transaction_read_only=on -c statement_timeout=30000') as db:
        fqdn=db.execute('SELECT a.fqdn FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',('9510a335-f191-5642-9ae1-20459e7d4eba',)).fetchone()['fqdn']+'.execution'
        ...
        approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='openvaccine-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (record['version_id'],)).fetchall()
        ...
        for binding in bindings:
            ...
            for table in ['artifact_io_specs', 'atom_io_specs']:
                ...
                assert ports == wanted
            approval = db.execute("SELECT details FROM artifact_audit_evidence WHERE version_id=%s AND runner_version='openvaccine-execution-community.v1' AND passed AND status='completed' AND source_kind='automated'", (atom['version_id'],)).fetchall()
            assert len(approval) == 1 and approval[0]['details']['publication_tier'] == 3
        ...
        from scripts.review_openvaccine_execution import SOURCES
        wanted=[]
        for original in SOURCES:
            source=db.execute('SELECT a.fqdn,a.status,a.is_publishable,v.content_hash FROM artifacts a JOIN artifact_versions v USING(artifact_id) WHERE v.version_id=%s',(original['version_id'],)).fetchone()
            assert source['status']=='draft' and not source['is_publishable'] and source['content_hash']==original['content_hash']
            wanted.append(dict(dependency_artifact_fqdn=source['fqdn'],dependency_content_hash=source['content_hash'],dependency_role='cdg',optional=False))
        assert sorted(dependency,key=lambda d:d['dependency_artifact_fqdn'])==sorted(wanted,key=lambda d:d['dependency_artifact_fqdn'])
    ...
    evidence_path=root/'docs/reviews/competition_openvaccine_graph_execution.json'
    before=evidence_path.read_bytes()
    payload=comparison.payload()
    comparison.setup.tf.config.threading.set_intra_op_parallelism_threads(2)
    comparison.setup.tf.config.threading.set_inter_op_parallelism_threads(2)
    captured={}
    def capture(directory,node,name,value):
        if node=='execute' and name=='out_result':captured['result']=value
    with tempfile.TemporaryDirectory(prefix='sciona_openvaccine_served_synthetic_') as temp:
        with patch.object(comparison.runner,'RUNS_DIR',Path(temp)), patch.object(comparison.runner,'save_intermediate_value',side_effect=capture):
            status=asyncio.run(comparison.runner.CDGExecutionSession(None,'synthetic-openvaccine-served','case').execute({'payload':payload},cdg=graph))
    ...
    assert all(comparison.setup.np.asarray(result[k]).shape==(2,14,5) and comparison.setup.np.isfinite(result[k]).all() for k in ['predictions','teacher_mean','teacher_std'])
    assert evidence_path.read_bytes()==before
    from scripts.promote_openvaccine_execution import review
    review(root)
    result=expected
    return dict(read_only=True, synthetic_only=True, trust_tier=3, provider_versions_verified=2,
        served_cdg_count=served, catalog_graph_runtime_execution='passed', serialized_graph_cases=1, full_model_members=20, refinement_rounds=1, mandatory_intake_dependencies=2,
...
if __name__ == '__main__':
    ...
    report = verify(root)
    (root / 'docs/reviews/competition_openvaccine_publication_verification.json').write_text(json.dumps(report, indent=2) + '\n')
