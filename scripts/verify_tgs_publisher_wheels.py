...
def main():
    audit_path = Path('docs/reviews/competition_tgs_published_wheel_audit.json')
    ...
    report = {'approved': False, 'all_compared_payloads_match': all(not p['failures'] for p in packages.values()),
        ...
        'sha256': {p: hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in [
            str(audit_path), 'scripts/verify_tgs_publisher_wheels.py', 'requirements/tgs-reference-macos-arm64.lock']}}
    Path('docs/reviews/competition_tgs_publisher_payload_validation.json').write_text(json.dumps(report, indent=2) + '\n')
