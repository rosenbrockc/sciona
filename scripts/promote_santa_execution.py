"""Atomically publish the source-compared Santa realization at automated Tier 3.
...
RUNNER = 'santa-execution-community.v1'
from scripts.review_santa_execution import LIMITATIONS
...
def review(root):
    from scripts.review_santa_execution import audit
    import sciona.atoms.ml.santa_execution
    try:
        expected_semantic=audit()
    except (AssertionError,KeyError) as error:
        raise ValueError('Required source/computation evidence failed') from error
    reviews=root/'docs/reviews'
    semantic=json.loads((reviews/'competition_santa_semantic_review.json').read_text())
    ...
    full=json.loads((reviews/'competition_santa_graph_execution.json').read_text())
    if full['serialized_graph_sha256']!=digest or (len(nodes),len(edges))!=(2,1):
        raise ValueError('Serialized graph identity differs')
    gates=json.loads((reviews/'competition_santa_review_gates.json').read_text())
    ...
    for name in ('competition_santa_semantic_review.json','competition_santa_review_gates.json'):
...
def promote(root, apply=False):
    ...
    gates_path=root/'docs/reviews/competition_santa_publication_gates.json'
    gates=json.loads(gates_path.read_text())
    if (gates['result']!='passed' or len(gates['rejected_gates'])!=6
            or gates['publisher_sha256']!=sha(Path(__file__))
            or gates['validator_sha256']!=sha(root/'scripts/validate_santa_publication_gates.py')):
        raise ValueError('Publication gate evidence missing or stale')
    evidence=dict(evidence,**{'competition_santa_publication_gates.json':sha(gates_path)})
    created = 0
    ...
    with psycopg.connect(dotenv_values(root / '.env')['SCIONA_DATA_CATALOG_DATABASE_URL'], row_factory=dict_row,
        ...
        if existing_graph and (existing_graph['artifact_id'] != graph_id or existing_graph['status'] != 'approved' or not existing_graph['is_publishable']):
            ...
            description='Community binary image/tabular pipeline with handcrafted RGB descriptors, fold-local metadata preparation, fused dropout MLPs, label smoothing and held-out F1 threshold. Synthetic evidence; no historical or empirical accuracy claim.',
        ...
        references=[]
        for reference,url,title in [('santa2020-winner-method','https://www.kaggle.com/c/santa-2020/discussion/218453','Santa 2020 winner two-model threshold method'),('santa2020-official-game','https://github.com/Kaggle/kaggle-environments/blob/master/kaggle_environments/envs/mab/mab.py','Official simultaneous reward and shared bandit decay interpreter')]:
        ...
        for identity, selected, node_id in targets:
            ...
            for reference,url,title in references:
                for table,key in ref_tables:
                        ...
                        'relevance_note':'Primary provenance for corrected discrete threshold inference, opponent modeling, two-regressor exploration blend and simultaneous reward/decay rules. Independent implementation and synthetic verification; no original weights or historical accuracy claim.'})
            ...
            details = dict(kind='provider' if node_id is not None else 'execution', publication_tier=3, review_source='automated',
                ...
                implementation_sha256=json.loads((root / 'docs/reviews/competition_santa_graph_execution.json').read_text())['code_sha256'],
        ...
        rechecked_evidence=dict(rechecked[-1],**{'competition_santa_publication_gates.json':sha(gates_path)})
