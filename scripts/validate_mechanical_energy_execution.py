#!/usr/bin/env python3
"""Execute constant-force mechanical energy through the serialized CDG runner."""
import argparse
import asyncio
import hashlib
import json
...
from sciona.atoms.physics import mechanical_energy as provider
from sciona.physics_ingest.mechanical_energy_execution import SOURCE_VERSION,build_mechanical_energy_execution
from sciona.services.execution_graph_codec import encode_execution_graph
from sciona.services.catalog_artifact_retrieval import _artifact_document_to_cdg
from scripts.validate_mechanical_energy_source import validate as validate_source
from tests.physics_ingest.test_mechanical_energy import reference
...
async def validate(root,symbol_file,rule_file,expression_file):
    ...
    cases=[(np.exp(rng.uniform(-80,80,(2,3,4))),rng.normal(size=(2,3,4)),
    ...
    cases += [tuple(np.array(v,dtype=float) for v in row) for row in [
        (2,-4,3,2,2),(2,0,-3,-2,4),(2,4,1,2,0),
        (1e-308,1e-308,0,0,1),(1e-308,-2,0,1e308,1)]]
        ...
        try:
            for i,args in enumerate(cases):
                ...
                result=await runner.CDGExecutionSession(None,'synthetic-constant-acceleration',run).execute(
                    dict(mass=args[0],force=args[1],initial_position=args[2],initial_velocity=args[3],elapsed_time=args[4]),cdg=restored)
                ...
                for name,expected in zip(['final_position','final_velocity','kinetic_energy','potential_energy','total_energy','work'],expected_arrays):
                    actual=np.load(Path(directory)/run/('energy/out_'+name+'.npy'))
                    np.testing.assert_array_equal(actual,expected)
        finally:runner.RUNS_DIR=prior
    files=['sciona/physics_ingest/mechanical_energy_execution.py','scripts/validate_mechanical_energy_execution.py',
           'tests/physics_ingest/test_mechanical_energy.py','tests/physics_ingest/test_mechanical_energy_proof.py','sciona/services/execution_graph_codec.py','sciona/visualizer/runner.py']
