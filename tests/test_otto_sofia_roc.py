...
from sciona import otto_sofia_roc as module
from sciona.otto_sofia_roc_crossfit import crossfit_sofia

CONTROLS=dict(forest=dict(ntree=32,mtry=4,nodesize=1),triples=[(0,1,2),(3,4,5)],log_scope='nonnegative_blocks',r_library=str(Path(__file__).resolve().parents[1]/'.venv/otto-r-library'),sofia=dict(regularization=.01,iterations=10000,rank_probability=.5))
...
def test_explicit_log_scope_matches_independent_block_oracle(population,monkeypatch):
    ...
    expected=lambda z:np.column_stack((np.log1p(z[:,:13]),z[:,13:16],np.log1p(z[:,16:])))
    np.testing.assert_allclose(a,expected(reference))
    np.testing.assert_allclose(b,expected(query))
...
def test_all_column_log_domain_rejects_signed_embedding(population):
    x,y,f,ids,q,qids,e=population
    options={k:v for k,v in CONTROLS.items() if k!='sofia'};options['log_scope']='all_columns'
    assert (e.coordinates<=-1).any()
    with pytest.raises(ValueError,match='domain'):
        module.assemble(x,y,ids,q,qids,embedding=e,seed=12,**options)
