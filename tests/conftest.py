...
def _default_supabase_project_root(repo_root: Path) -> Path:
    infra_root = (repo_root / ".." / "sciona-infra").resolve()
    if (infra_root / "supabase" / "config.toml").exists():
        return infra_root
    return repo_root


def _supabase_project_root(repo_root: Path) -> Path:
    configured = os.environ.get("SCIONA_SUPABASE_PROJECT_ROOT", "").strip()
    if configured:
        return Path(configured).expanduser().resolve()
    return _default_supabase_project_root(repo_root)


@pytest.fixture(scope="session")
def supabase_project_root(repo_root: Path) -> Path:
    return _supabase_project_root(repo_root)
...
def _supabase_project_id(project_root: Path) -> str:
...
@pytest.fixture(scope="session")
def supabase_local_env(supabase_project_root: Path) -> Iterator[dict[str, str]]:
    ...
    project_id = _supabase_project_id(supabase_project_root)
    started_here = False
    status = _run_subprocess(
        ["supabase", "status", "-o", "env"],
    ...
    if os.environ.get("SCIONA_TEST_SUPABASE_RESET_LOCAL", "0") == "1":
        ...
        recovered_status = _wait_for_supabase_ready(supabase_project_root)
...
@pytest.fixture(scope="session")
def sciona_atoms_root(repo_root: Path) -> Path:
    configured = os.environ.get("SCIONA_TEST_SCIONA_ATOMS_ROOT", "")
    if configured:
        return Path(configured).expanduser().resolve()
    return (repo_root / ".." / "sciona-atoms").resolve()
...
@pytest.fixture(scope="session")
def sciona_atoms_declarations(sciona_atoms_root: Path) -> list:
    if not sciona_atoms_root.exists():
        pytest.skip(f"sciona-atoms repo not found at {sciona_atoms_root}")
    return build_sciona_atoms_declarations(sciona_atoms_root)
...
# ---------------------------------------------------------------------------
# Synthetic-family fixtures (see tests/fixtures/synthetic_family/)
# ---------------------------------------------------------------------------
#
# These fixtures let engine-level tests exercise the ExpansionEngine,
# GraphRewriter, and Principal proposal plumbing without pulling in any
# real family's rule set, runtime atoms, or provider-repo dependencies.


@pytest.fixture
def synthetic_family_rule_set():  # type: ignore[no-untyped-def]
    from tests.fixtures.synthetic_family import SyntheticFamilyExpansionRuleSet

    return SyntheticFamilyExpansionRuleSet()


@pytest.fixture
def synthetic_cdg():  # type: ignore[no-untyped-def]
    from tests.fixtures.synthetic_family import synthetic_cdg as _build

    return _build()


@pytest.fixture
def synthetic_expansion_context():  # type: ignore[no-untyped-def]
    from sciona.principal.expansion import ExpansionContext

    return ExpansionContext(
        intermediates={"synth_quality_score": 0.2, "synth_error_rate": 0.3}
    )
