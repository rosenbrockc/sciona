...
from sciona.cli import _load_skill_index_or_empty


def test_load_skill_index_can_be_disabled(monkeypatch, tmp_path: Path, capsys):
    monkeypatch.setenv("SCIONA_DISABLE_SKILL_INDEX", "1")
    ...
    monkeypatch.setattr("sciona.architect.embedder.SkillIndex.load", _fail)

    index = _load_skill_index_or_empty(config)

    assert index is not None
    assert "SCIONA_DISABLE_SKILL_INDEX" in capsys.readouterr().err


def test_load_skill_index_can_be_disabled_by_execution_mode(
    monkeypatch, tmp_path: Path, capsys
):
    monkeypatch.delenv("SCIONA_DISABLE_SKILL_INDEX", raising=False)
    config = SimpleNamespace(skill_index_dir=tmp_path)
...
def test_load_skill_index_uses_persisted_index_when_enabled(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("sciona.architect.embedder.SkillIndex.load", _load)
