...
async def test_run_release_validation_fails_when_nonbaseline_regressions_exist(
    ...
    assert manifest["warnings"]["warning_summary"] == (
        "runtime=0 catalog=0 benchmark=1 top=flow_comparison_failures=2"
    )
    assert manifest["warnings"]["top_runtime_warning"] == ""
    ...
    assert bench["health_summary"] == (
        ...
        "failures=subcheck=prompt_tuning failure=prompt_tuned_failures=1"
    )
    assert bench["warning_summary"] == (
...
async def test_run_release_validation_fails_when_runtime_complexity_budget_exceeded(
    ...
    assert manifest["status"] == "failed"
    assert manifest["warnings"]["warning_summary"] == (
        "runtime=2 top=provider_count=6 exceeds budget 4 "
        "catalog=0 benchmark=1 top=flow_comparison_failures=2"
    )
    ...
    assert bench["health_summary"] == (
        "warnings=subcheck=comparison_failures warning=flow_comparison_failures=2 "
        "failures=subcheck=runtime_budget failure=provider_count=6 exceeds budget 4"
    )
    assert bench["warning_summary"] == (
        "subcheck=comparison_failures warning=flow_comparison_failures=2"
    )
