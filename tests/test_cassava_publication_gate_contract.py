...
import pytest
from scripts import promote_cassava_execution as publisher
...
def gate_record(tmp_path, monkeypatch):
    ...
    script = tmp_path / 'scripts/validate_cassava_publication_gates.py'
    script.parent.mkdir()
    ...
    record = dict(format='cassava-publication-gates.v1', result='passed',
...
def test_invalid_gate_record_stops_before_transaction(gate_record, fault):
    ...
    (root / 'docs/reviews/competition_cassava_publication_gates.json').write_text(json.dumps(record))
