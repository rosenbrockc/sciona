...
def test_invalid_contract_before_runtime(kind):
    ...
    with pytest.raises(ValueError):
        andriy_r_glm_segment_probabilities(train,labels,pred,valid)


def test_training_class_order_is_explicit():
    train,labels,pred,valid=inputs()
    labels[0]=np.array([0,1])
    with pytest.raises(ValueError,match='positive-first'):
        andriy_r_glm_segment_probabilities(train,labels,pred,valid)
