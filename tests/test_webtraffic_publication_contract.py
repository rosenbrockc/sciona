...
import pytest
from scripts.promote_webtraffic_execution import review
...
def test_review_rejects_invalid_evidence(change):
    ...
    def edited(path,*args,**kwargs):
        ...
        if path.name=='competition_webtraffic_graph.json':
            ...
            else:data['implementation_sha256']['sciona/webtraffic_payload.py']='0'*64
...
def test_review_rejects_unapproved_semantic_scope():
    ...
    def edited(path,*args,**kwargs):
        text=original(path,*args,**kwargs)
        if path.name=='competition_webtraffic_semantic_review.json':
