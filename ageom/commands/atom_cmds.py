...
async def _cmd_atom_publish(args: argparse.Namespace) -> None:
    ...
    from ageom.commands.login_cmds import _load_token

    token, default_api_url = _load_token()
    if not token:
        print("Error: not authenticated. Run `ageom login` first.", file=sys.stderr)
        sys.exit(1)

    api_url = (getattr(args, "api_url", None) or default_api_url or "https://api.ageom.dev").rstrip("/")
    ...
    if py_files:
        try:
            from ageom.architect.atom_similarity import fingerprint_function
