...
def _get_config_path() -> Path:
    ...
    return Path.home() / ".ageom" / "config.json"


def _save_token(token: str, api_url: str) -> Path:
    """Persist the JWT token to ~/.ageom/config.json."""
...
async def _cmd_login(args: argparse.Namespace) -> None:
    """Authenticate with the AGEOM platform via GitHub device flow."""
    try:
        import httpx
    except ImportError:
    ...
    api_url = (args.api_url or "https://api.ageom.dev").rstrip("/")
