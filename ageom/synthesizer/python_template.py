...
def generate_pipeline_py(
    ...
    *,
    ...
    lines.append("from ageom.principal.datasets._parser import DataSetCollection")
    lines.append("from ageom.principal.eval_spec import compute_evaluation_payload, load_evaluation_spec")
    lines.append("from ageom.principal.datasets.io import read as read_dataset_template")
    lines.append("")
    ...
    lines.append("    cache_dir = Path(__file__).resolve().parents[2] / '.ageom_generated_adapters'")
    lines.append("    cache_dir.mkdir(parents=True, exist_ok=True)")
    lines.append("    target = cache_dir / ('.ageom_' + '_'.join(selected_groups or ('all',)) + '_runner.yml')")
    ...
    lines.append("            _SIGNAL_ALIASES = {'signal': ['signal', 'ecg', 'ppg', 'wave']}")
    lines.append("            for _alias_key, _alias_tokens in _SIGNAL_ALIASES.items():")
    lines.append("                if (str(column) == 'value' or alias == 'value') and _alias_key not in flat and any(token in group_lower for token in _alias_tokens):")
    lines.append("                    flat[_alias_key] = value")
    lines.append("        sampling_rate = _infer_sampling_rate(frame)")
    lines.append("        if sampling_rate is not None:")
    lines.append("            flat.setdefault('sampling_rate', sampling_rate)")
    ...
    lines.append("    if args.params and hasattr(atoms, '_AGEOM_PARAMS'):")
    lines.append("        atoms._AGEOM_PARAMS = json.loads(Path(args.params).read_text())")
